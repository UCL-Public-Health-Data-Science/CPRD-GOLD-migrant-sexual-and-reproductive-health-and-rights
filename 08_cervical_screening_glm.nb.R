...
## Cleaning and analysing cervical_screening data for Neha's SRHR overview chapter in females of reproductive age (15-49yo) 
...
## ensure reference for prac region is london
...
## restrict to screening age 25 +
srh_annual_counts_final <- srh_annual_counts_final %>% filter(eventyear_age >= 25)
srh_annual_counts_final_extra <- srh_annual_counts_final_extra %>% filter(eventyear_age >= 25)
...
cervical_screening_count_overall_whole_cohort <- srh_annual_counts_final %>% tally(cervical_screening_n) %>% rename(n_events = n)
cervical_screening_summary_overall_whole_cohort <- srh_annual_counts_final %>% 
  summarise(mean = mean(cervical_screening_n), sd = sd(cervical_screening_n), median = median(cervical_screening_n), iqr = IQR(cervical_screening_n), 
            min = min(cervical_screening_n), max = max(cervical_screening_n))
cervical_screening_count_summary_overall_whole_cohort <- merge(cervical_screening_count_overall_whole_cohort, cervical_screening_summary_overall_whole_cohort)
cervical_screening_count_summary_overall_whole_cohort

## Overall by migrant status
cervical_screening_count_overall_migrant_status <- srh_annual_counts_final %>% group_by(migrant_status) %>%
  tally(cervical_screening_n) %>% rename(n_events = n)
cervical_screening_summary_overall_migrant_status <- srh_annual_counts_final %>% group_by(migrant_status) %>%
  summarise(mean = mean(cervical_screening_n), sd = sd(cervical_screening_n), median = median(cervical_screening_n), iqr = IQR(cervical_screening_n), 
            min = min(cervical_screening_n), max = max(cervical_screening_n))
cervical_screening_count_summary_overall_migrant_status <- left_join(cervical_screening_count_overall_migrant_status, cervical_screening_summary_overall_migrant_status,
                                                                 by = c("migrant_status"="migrant_status" ))

## Overall by migcertainty
cervical_screening_count_overall_migcertainty <- srh_annual_counts_final %>% group_by(migcertainty) %>%
  tally(cervical_screening_n) %>% rename(n_events = n)
cervical_screening_summary_overall_migcertainty <- srh_annual_counts_final %>% group_by(migcertainty) %>%
  summarise(mean = mean(cervical_screening_n), sd = sd(cervical_screening_n), median = median(cervical_screening_n), iqr = IQR(cervical_screening_n), 
            min = min(cervical_screening_n), max = max(cervical_screening_n))
cervical_screening_count_summary_overall_migcertainty <- left_join(cervical_screening_count_overall_migcertainty, cervical_screening_summary_overall_migcertainty,
...
cervical_screening_count_summary_overall_final <- full_join(cervical_screening_count_summary_overall_whole_cohort, cervical_screening_count_summary_overall_migrant_status, 
                                                        by = c("n_events"="n_events", "mean" = "mean",
                                                               "sd"="sd", "median"="median", "iqr"="iqr", "min"="min", "max"="max")) %>% 
  full_join(cervical_screening_count_summary_overall_migcertainty, by = c("n_events"="n_events", "mean" = "mean",
                                                                      "sd"="sd", "median"="median", "iqr"="iqr", "min"="min", "max"="max",
                                                                      "migrant_status" = "migcertainty")) 
cervical_screening_count_summary_overall_final$migrant_status <- fct_explicit_na(cervical_screening_count_summary_overall_final$migrant_status, na_level = "whole_cohort")
cervical_screening_count_summary_overall_final <- cervical_screening_count_summary_overall_final %>% relocate(migrant_status)
cervical_screening_count_summary_overall_final

write_csv(cervical_screening_count_summary_overall_final,"filepath")

## Annual - whole cohort 
cervical_screening_count_annual_whole_cohort <- srh_annual_counts_final %>% group_by(eventyear) %>%
  tally(cervical_screening_n) %>% rename(n_events = n)
cervical_screening_summary_annual_whole_cohort <- srh_annual_counts_final %>% group_by(eventyear) %>%
  summarise(mean = mean(cervical_screening_n), sd = sd(cervical_screening_n), median = median(cervical_screening_n), iqr = IQR(cervical_screening_n), 
            min = min(cervical_screening_n), max = max(cervical_screening_n))
cervical_screening_count_summary_annual_whole_cohort <- left_join(cervical_screening_count_annual_whole_cohort, cervical_screening_summary_annual_whole_cohort,
                                                              by = c("eventyear"="eventyear"))


## Annual - by migrant status
cervical_screening_count_annual_migrant_status <- srh_annual_counts_final %>% group_by(migrant_status, eventyear) %>%
  tally(cervical_screening_n) %>% rename(n_events = n)
cervical_screening_summary_annual_migrant_status <- srh_annual_counts_final %>% group_by(migrant_status, eventyear) %>%
  summarise(mean = mean(cervical_screening_n), sd = sd(cervical_screening_n), median = median(cervical_screening_n), iqr = IQR(cervical_screening_n), 
            min = min(cervical_screening_n), max = max(cervical_screening_n))
cervical_screening_count_summary_annual_migrant_status <- left_join(cervical_screening_count_annual_migrant_status, cervical_screening_summary_annual_migrant_status,
                                                                by = c("migrant_status"="migrant_status" , "eventyear"="eventyear"))

## Annual - by migcertainty 
cervical_screening_count_annual_migcertainty <- srh_annual_counts_final %>% group_by(migcertainty, eventyear) %>%
  tally(cervical_screening_n) %>% rename(n_events = n)
cervical_screening_summary_annual_migcertainty <- srh_annual_counts_final %>% group_by(migcertainty, eventyear) %>%
  summarise(mean = mean(cervical_screening_n), sd = sd(cervical_screening_n), median = median(cervical_screening_n), iqr = IQR(cervical_screening_n), 
            min = min(cervical_screening_n), max = max(cervical_screening_n))
cervical_screening_count_summary_annual_migcertainty <- left_join(cervical_screening_count_annual_migcertainty, cervical_screening_summary_annual_migcertainty,
                                                              by = c("migcertainty"="migcertainty" , "eventyear"="eventyear"))



## Join to give annual for whole cohort + migrant status
cervical_screening_count_summary_annual_final <- full_join(cervical_screening_count_summary_annual_whole_cohort, cervical_screening_count_summary_annual_migrant_status, 
                                                       by = c("eventyear" = "eventyear", "n_events"="n_events", "mean" = "mean",
                                                              "sd"="sd", "median"="median", "iqr"="iqr", "min"="min", "max"="max")) 
cervical_screening_count_summary_annual_final <-   full_join(cervical_screening_count_summary_annual_final, cervical_screening_count_summary_annual_migcertainty,
                                                         by = c("eventyear" = "eventyear", "n_events"="n_events", "mean" = "mean",
                                                                "sd"="sd", "median"="median", "iqr"="iqr", "min"="min", "max"="max",
                                                                "migrant_status" = "migcertainty"))

cervical_screening_count_summary_annual_final$migrant_status <- fct_explicit_na(cervical_screening_count_summary_annual_final$migrant_status, na_level = "whole_cohort") 
cervical_screening_count_summary_annual_final <- cervical_screening_count_summary_annual_final %>% 
  relocate(migrant_status)


## Export cervical_screening summary measures
write_csv(cervical_screening_count_summary_annual_final, "filepath")


## distribution of cervical_screening counts
library(ggforce)
cervical_screening_distribution <- ggplot(srh_annual_counts_final) + geom_histogram(aes(x = cervical_screening_n), binwidth = 1) +
  facet_zoom(xlim = c(1, 8), ylim = c(0,750000)) +
  ggtitle("Distribution of cervical_screening counts") +
  labs(y="Frequency", x = "cervical_screening counts")
print(cervical_screening_distribution)
...
cervical_screeningcount_mvnm <- aggregate(cervical_screening_n ~ migrant_status, srh_annual_counts_final_extra, sum) 
IR_mvnm <- inner_join(cervical_screeningcount_mvnm,pyears_mvnm_overall, by= c("migrant_status" = "migrant_status"))
IR_mvnm_output <- pois.exact(IR_mvnm$cervical_screening_n, IR_mvnm$pyears_ey, conf.level=0.95)
...
x <- glm.nb(cervical_screening_n ~ as.factor(migrant_status) + offset(log(pyears_ey)), data = srh_annual_counts_final_extra)
names <- names(coef(x))
estimate<- exp(coef(x))
...
d<- left_join(c, reproductive_cohort_final, by = c("patid" = "patid")) %>% 
  rename(cervical_screening_n = n.x) %>% 
...
cervical_screeningcount_agecat <- aggregate(cervical_screening_n ~ eventyear_agecat, srh_annual_counts_final_extra, sum) 
IR_agecat <- inner_join(cervical_screeningcount_agecat,pyears_agecat_overall, by= c("eventyear_agecat" = "eventyear_agecat"))
IR_agecat_output <- pois.exact(IR_agecat$cervical_screening_n, IR_agecat$pyears_ey, conf.level=0.95)
...
x <- glm.nb(cervical_screening_n ~ as.factor(eventyear_agecat) + offset(log(pyears_agecat)), data = d)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
cervical_screeningcount_prac_region <- aggregate(cervical_screening_n ~ prac_region, srh_annual_counts_final_extra, sum) 
IR_prac_region <- inner_join(cervical_screeningcount_prac_region,pyears_prac_region_overall, by= c("prac_region" = "prac_region"))
IR_prac_region_output <- pois.exact(IR_prac_region$cervical_screening_n, IR_prac_region$pyears_ey, conf.level=0.95)
...
x <- glm.nb(cervical_screening_n ~ as.factor(prac_region) + offset(log(pyears_ey)), data = srh_annual_counts_final_extra)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
cervical_screeningcount_imd <- aggregate(cervical_screening_n ~ imd, srh_annual_counts_final, sum) 
IR_imd <- inner_join(cervical_screeningcount_imd,pyears_imd_overall, by= c("imd" = "imd"))
IR_imd_output <- pois.exact(IR_imd$cervical_screening_n, IR_imd$pyears, conf.level=0.95)
...
x <- glm.nb(cervical_screening_n ~ as.factor(imd) + offset(log(pyears_ey)), data = srh_annual_counts_final_extra)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
cervical_screening_count_annual <- aggregate(cervical_screening_n ~ eventyear, srh_annual_counts_final_extra, sum) 
IR_annual <- inner_join(cervical_screening_count_annual,pyears_annual, by= c("eventyear" = "eventyear"))
IR_annual_output <- pois.exact(IR_annual$cervical_screening_n, IR_annual$pyears_ey, conf.level=0.95)
...
## Generate IRR via negative binomial regression univariabe for  year 
x <- glm.nb(cervical_screening_n ~ as.factor(eventyear) + offset(log(pyears_ey)), data = srh_annual_counts_final_extra)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
x <- glm.nb(cervical_screening_n ~ as.factor(migrant_status) + as.factor(eventyear) + as.factor(eventyear_agecat) + as.factor(imd) + as.factor(prac_region) + offset(log(pyears_ey)) , 
...
cervical_screening_glm_adj_unadj <- glm_adj_unadj
save(cervical_screening_glm_adj_unadj, file = "filepath")
...
cervical_screeningcount_dp <- aggregate(cervical_screening_n ~ migcertainty, srh_annual_counts_final_extra, sum) 
IR_dp <- inner_join(cervical_screeningcount_dp,pyears_dp_overall, by= c("migcertainty" = "migcertainty"))
IR_dp_output <- pois.exact(IR_dp$cervical_screening_n, IR_dp$pyears_ey, conf.level=0.95)
...
x <- glm.nb(cervical_screening_n ~ as.factor(migcertainty) + offset(log(pyears_ey)), data = srh_annual_counts_final_extra)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
## Generate IRR via negative binomial regression
srh_annual_counts_final_extra$migcertainty <- relevel(srh_annual_counts_final_extra$migcertainty, "Non-migrant")
x <- glm.nb(cervical_screening_n ~ as.factor(migcertainty) + as.factor(eventyear) + as.factor(eventyear_agecat) +  as.factor(imd) + as.factor(prac_region) + offset(log(pyears_ey)), 
            data = srh_annual_counts_final_extra)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
...
cervical_screening_glm_dp_adj_unadj <- glm_dp_adj_unadj
save(cervical_screening_glm_dp_adj_unadj, file = "filepath")
...
cervical_screeningcount_mvnm_white_british<- aggregate(cervical_screening_n ~ migrant_status, white_british, sum) 
IR_mvnm_white_british <- inner_join(cervical_screeningcount_mvnm_white_british,pyears_mvnm_white_british, by= c("migrant_status" = "migrant_status"))
IR_mvnm_white_british_output <- pois.exact(IR_mvnm_white_british$cervical_screening_n, IR_mvnm_white_british$pyears_ey, conf.level=0.95)
...
x <- glm.nb(cervical_screening_n ~ as.factor(migrant_status) + offset(log(pyears_ey)), data = white_british)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
cervical_screeningcount_mvnm_white_nonbritish<- aggregate(cervical_screening_n ~ migrant_status, white_nonbritish, sum) 
IR_mvnm_white_nonbritish <- inner_join(cervical_screeningcount_mvnm_white_nonbritish,pyears_mvnm_white_nonbritish, by= c("migrant_status" = "migrant_status"))
IR_mvnm_white_nonbritish_output <- pois.exact(IR_mvnm_white_nonbritish$cervical_screening_n, IR_mvnm_white_nonbritish$pyears_ey, conf.level=0.95)
...
x <- glm.nb(cervical_screening_n ~ as.factor(migrant_status) + offset(log(pyears_ey)), data = white_nonbritish)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
cervical_screeningcount_mvnm_mixed<- aggregate(cervical_screening_n ~ migrant_status, mixed, sum) 
IR_mvnm_mixed <- inner_join(cervical_screeningcount_mvnm_mixed,pyears_mvnm_mixed, by= c("migrant_status" = "migrant_status"))
IR_mvnm_mixed_output <- pois.exact(IR_mvnm_mixed$cervical_screening_n, IR_mvnm_mixed$pyears_ey, conf.level=0.95)
...
x <- glm.nb(cervical_screening_n ~ as.factor(migrant_status) + offset(log(pyears_ey)), data = mixed)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
cervical_screeningcount_mvnm_asian<- aggregate(cervical_screening_n ~ migrant_status, asian, sum) 
IR_mvnm_asian <- inner_join(cervical_screeningcount_mvnm_asian,pyears_mvnm_asian, by= c("migrant_status" = "migrant_status"))
IR_mvnm_asian_output <- pois.exact(IR_mvnm_asian$cervical_screening_n, IR_mvnm_asian$pyears_ey, conf.level=0.95)
...
x <- glm.nb(cervical_screening_n ~ as.factor(migrant_status) + offset(log(pyears_ey)), data = asian)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
cervical_screeningcount_mvnm_black<- aggregate(cervical_screening_n ~ migrant_status, black, sum) 
IR_mvnm_black <- inner_join(cervical_screeningcount_mvnm_black,pyears_mvnm_black, by= c("migrant_status" = "migrant_status"))
IR_mvnm_black_output <- pois.exact(IR_mvnm_black$cervical_screening_n, IR_mvnm_black$pyears_ey, conf.level=0.95)
...
x <- glm.nb(cervical_screening_n ~ as.factor(migrant_status) + offset(log(pyears_ey)), data = black)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
cervical_screeningcount_mvnm_other<- aggregate(cervical_screening_n ~ migrant_status, other, sum) 
IR_mvnm_other <- inner_join(cervical_screeningcount_mvnm_other,pyears_mvnm_other, by= c("migrant_status" = "migrant_status"))
IR_mvnm_other_output <- pois.exact(IR_mvnm_other$cervical_screening_n, IR_mvnm_other$pyears_ey, conf.level=0.95)
...
x <- glm.nb(cervical_screening_n ~ as.factor(migrant_status) + offset(log(pyears_ey)), data = other)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
glm_mig_other$names <- recode(glm_mig_other$names, "(Intercept)" = "Non-migrant" ,"as.factor(migrant_status)Migrant" = "Other Migrant")
...
cervical_screening_glm_adj_unadj_ethnicity <- glm_adj_unadj
save(cervical_screening_glm_adj_unadj_ethnicity, file = "filepath")
...
## restrict to screening age 25 +
exact_match_srh_annual_counts_final_4to1 <- exact_match_srh_annual_counts_final_4to1 %>% filter(eventyear_age >= 25)
exact_match_srh_annual_counts_final_extra_4to1 <- exact_match_srh_annual_counts_final_extra_4to1 %>% filter(eventyear_age >= 25)
...
cervical_screeningcount_mvnm <- aggregate(cervical_screening_n ~ migrant_status, exact_match_srh_annual_counts_final_extra_4to1, sum) 
IR_mvnm <- inner_join(cervical_screeningcount_mvnm,pyears_mvnm_overall, by= c("migrant_status" = "migrant_status"))
IR_mvnm_output <- pois.exact(IR_mvnm$cervical_screening_n, IR_mvnm$pyears_ey, conf.level=0.95)
IR_mvnm_output <- IR_mvnm_output %>%
  mutate(incidence_rate100=rate*100)
...
x <- glm.nb(cervical_screening_n ~ as.factor(migrant_status) + offset(log(pyears_ey)), data = exact_match_srh_annual_counts_final_extra_4to1)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
cervical_screening_glm_adj_unadj_em <- glm_adj_unadj
save(cervical_screening_glm_adj_unadj_em, file = "filepath")
...
unadj_irr_migrant_status <- forestplot(tabletext, mean = glm_adj_unadj$estimate, lower=glm_adj_unadj$lower, upper=glm_adj_unadj$upper,
                                       ...
                                       ci.vertices = TRUE,
                                       boxsize = 0.05,
                                       title = "cervical_screening (Exact Matched Cohort)",
                                       graphwidth = unit(80, 'mm'))
dev.off()
...
pyears_dp_overall <- pyears_dp_overall %>% mutate(across(is.numeric, ~ round(.,)))
cervical_screeningcount_dp <- aggregate(cervical_screening_n ~ migcertainty, exact_match_srh_annual_counts_final_extra_4to1, sum) 
IR_dp <- inner_join(cervical_screeningcount_dp,pyears_dp_overall, by= c("migcertainty" = "migcertainty"))
IR_dp_output <- pois.exact(IR_dp$cervical_screening_n, IR_dp$pyears_ey, conf.level=0.95)
IR_dp_output <- IR_dp_output %>%
  mutate(incidence_rate100=rate*100)
...
x <- glm.nb(cervical_screening_n ~ as.factor(migcertainty) + offset(log(pyears_ey)), data = exact_match_srh_annual_counts_final_extra_4to1)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
cervical_screening_glm_dp_adj_unadj_em <- glm_dp_adj_unadj
save(cervical_screening_glm_dp_adj_unadj_em, file = "filepath")
...
irrs_migcertainty <- forestplot(tabletext, mean = glm_dp_adj_unadj$estimate, lower=glm_dp_adj_unadj$lower, upper=glm_dp_adj_unadj$upper,
                                ...
                                ci.vertices = TRUE,
                                boxsize = 0.05,
                                title = "Cervical_screening Overall",
                                graphwidth = unit(80, 'mm'))
dev.off()
...
cervical_screening_glm_adj_unadj_ethnicity_em <- glm_adj_unadj
save(cervical_screening_glm_adj_unadj_ethnicity_em, file = "filepath")
