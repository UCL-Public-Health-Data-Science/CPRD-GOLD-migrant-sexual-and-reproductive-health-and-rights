...
## Cleaning and analysing abortion data for Neha's SRHR overview chapter in females of reproductive age (15-49yo) 
...
abortion_count_overall_whole_cohort <- srh_annual_counts_final %>% tally(abortion_n) %>% rename(n_events = n)
abortion_summary_overall_whole_cohort <- srh_annual_counts_final %>% 
  summarise(mean = mean(abortion_n), sd = sd(abortion_n), median = median(abortion_n), iqr = IQR(abortion_n), 
            min = min(abortion_n), max = max(abortion_n))
abortion_count_summary_overall_whole_cohort <- merge(abortion_count_overall_whole_cohort, abortion_summary_overall_whole_cohort)
abortion_count_summary_overall_whole_cohort

## Overall by migrant status
abortion_count_overall_migrant_status <- srh_annual_counts_final %>% group_by(migrant_status) %>%
  tally(abortion_n) %>% rename(n_events = n)
abortion_summary_overall_migrant_status <- srh_annual_counts_final %>% group_by(migrant_status) %>%
  summarise(mean = mean(abortion_n), sd = sd(abortion_n), median = median(abortion_n), iqr = IQR(abortion_n), 
            min = min(abortion_n), max = max(abortion_n))
abortion_count_summary_overall_migrant_status <- left_join(abortion_count_overall_migrant_status, abortion_summary_overall_migrant_status,
                                                           by = c("migrant_status"="migrant_status" ))

## Overall by migcertainty
abortion_count_overall_migcertainty <- srh_annual_counts_final %>% group_by(migcertainty) %>%
  tally(abortion_n) %>% rename(n_events = n)
abortion_summary_overall_migcertainty <- srh_annual_counts_final %>% group_by(migcertainty) %>%
  summarise(mean = mean(abortion_n), sd = sd(abortion_n), median = median(abortion_n), iqr = IQR(abortion_n), 
            min = min(abortion_n), max = max(abortion_n))
abortion_count_summary_overall_migcertainty <- left_join(abortion_count_overall_migcertainty, abortion_summary_overall_migcertainty,
                                                           by = c("migcertainty"="migcertainty" ))

## Join for overall whole cohort + by migrant status
abortion_count_summary_overall_final <- full_join(abortion_count_summary_overall_whole_cohort, abortion_count_summary_overall_migrant_status, 
                                                  by = c("n_events"="n_events", "mean" = "mean",
                                                         "sd"="sd", "median"="median", "iqr"="iqr", "min"="min", "max"="max")) %>% 
  full_join(abortion_count_summary_overall_migcertainty, by = c("n_events"="n_events", "mean" = "mean",
                                                                "sd"="sd", "median"="median", "iqr"="iqr", "min"="min", "max"="max",
                                                                "migrant_status" = "migcertainty")) 
abortion_count_summary_overall_final$migrant_status <- fct_explicit_na(abortion_count_summary_overall_final$migrant_status, na_level = "whole_cohort")
abortion_count_summary_overall_final <- abortion_count_summary_overall_final %>% relocate(migrant_status)
abortion_count_summary_overall_final

write_csv(abortion_count_summary_overall_final, "results/abortion/1_summary_measures_overall.csv" )

## Annual - whole cohort 
abortion_count_annual_whole_cohort <- srh_annual_counts_final %>% group_by(eventyear) %>%
  tally(abortion_n) %>% rename(n_events = n)
abortion_summary_annual_whole_cohort <- srh_annual_counts_final %>% group_by(eventyear) %>%
  summarise(mean = mean(abortion_n), sd = sd(abortion_n), median = median(abortion_n), iqr = IQR(abortion_n), 
            min = min(abortion_n), max = max(abortion_n))
abortion_count_summary_annual_whole_cohort <- left_join(abortion_count_annual_whole_cohort, abortion_summary_annual_whole_cohort,
                                                        by = c("eventyear"="eventyear"))

## Annual - by migrant status
abortion_count_annual_migrant_status <- srh_annual_counts_final %>% group_by(migrant_status, eventyear) %>%
  tally(abortion_n) %>% rename(n_events = n)
abortion_summary_annual_migrant_status <- srh_annual_counts_final %>% group_by(migrant_status, eventyear) %>%
  summarise(mean = mean(abortion_n), sd = sd(abortion_n), median = median(abortion_n), iqr = IQR(abortion_n), 
            min = min(abortion_n), max = max(abortion_n))
abortion_count_summary_annual_migrant_status <- left_join(abortion_count_annual_migrant_status, abortion_summary_annual_migrant_status,
                                                          by = c("migrant_status"="migrant_status" , "eventyear"="eventyear"))

## Annual - by migcertainty 
abortion_count_annual_migcertainty <- srh_annual_counts_final %>% group_by(migcertainty, eventyear) %>%
  tally(abortion_n) %>% rename(n_events = n)
abortion_summary_annual_migcertainty <- srh_annual_counts_final %>% group_by(migcertainty, eventyear) %>%
  summarise(mean = mean(abortion_n), sd = sd(abortion_n), median = median(abortion_n), iqr = IQR(abortion_n), 
            min = min(abortion_n), max = max(abortion_n))
abortion_count_summary_annual_migcertainty <- left_join(abortion_count_annual_migcertainty, abortion_summary_annual_migcertainty,
                                                          by = c("migcertainty"="migcertainty" , "eventyear"="eventyear"))

## Join to give annual for whole cohort + migrant status
abortion_count_summary_annual_final <- full_join(abortion_count_summary_annual_whole_cohort, abortion_count_summary_annual_migrant_status, 
                                                 by = c("eventyear" = "eventyear", "n_events"="n_events", "mean" = "mean",
                                                        "sd"="sd", "median"="median", "iqr"="iqr", "min"="min", "max"="max")) 
  abortion_count_summary_annual_final <-   full_join(abortion_count_summary_annual_final, abortion_count_summary_annual_migcertainty,
            by = c("eventyear" = "eventyear", "n_events"="n_events", "mean" = "mean",
                   "sd"="sd", "median"="median", "iqr"="iqr", "min"="min", "max"="max",
                   "migrant_status" = "migcertainty"))

abortion_count_summary_annual_final$migrant_status <- fct_explicit_na(abortion_count_summary_annual_final$migrant_status, na_level = "whole_cohort") 
abortion_count_summary_annual_final <- abortion_count_summary_annual_final %>% 
  relocate(migrant_status)

## Export abortion summary measures
write_csv(abortion_count_summary_annual_final, "filepath")

## distribution of consultation counts
library(ggforce)
abortion_distribution <- ggplot(srh_annual_counts_final) + geom_histogram(aes(x = abortion_n), binwidth = 1) +
  facet_zoom(xlim = c(1, 8), ylim = c(0,26000)) +
  ggtitle("Distribution of abortion counts") +
  labs(y="Frequency", x = "Abortion counts")
print(abortion_distribution)
...
abortioncount_mvnm <- aggregate(abortion_n ~ migrant_status, srh_annual_counts_final_extra, sum) 
IR_mvnm <- inner_join(abortioncount_mvnm,pyears_mvnm_overall, by= c("migrant_status" = "migrant_status"))
IR_mvnm_output <- pois.exact(IR_mvnm$abortion_n, IR_mvnm$pyears_ey, conf.level=0.95)
...
x <- glm.nb(abortion_n ~ as.factor(migrant_status) + offset(log(pyears_ey)), data = srh_annual_counts_final_extra)
names <- names(coef(x))
estimate<- exp(coef(x))
...
abortioncount_agecat <- aggregate(abortion_n ~ eventyear_agecat, srh_annual_counts_final_extra, sum) 
IR_agecat <- inner_join(abortioncount_agecat,pyears_agecat_overall, by= c("eventyear_agecat" = "eventyear_agecat"))
IR_agecat_output <- pois.exact(IR_agecat$abortion_n, IR_agecat$pyears_ey, conf.level=0.95)
...
x <- glm.nb(abortion_n ~ as.factor(eventyear_agecat) + offset(log(pyears_agecat)), data = d)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
abortioncount_prac_region <- aggregate(abortion_n ~ prac_region, srh_annual_counts_final_extra, sum) 
IR_prac_region <- inner_join(abortioncount_prac_region,pyears_prac_region_overall, by= c("prac_region" = "prac_region"))
IR_prac_region_output <- pois.exact(IR_prac_region$abortion_n, IR_prac_region$pyears_ey, conf.level=0.95)
...
x <- glm.nb(abortion_n ~ as.factor(prac_region) + offset(log(pyears_ey)), data = srh_annual_counts_final_extra)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
abortioncount_imd <- aggregate(abortion_n ~ imd, srh_annual_counts_final_extra, sum) 
IR_imd <- inner_join(abortioncount_imd,pyears_imd_overall, by= c("imd" = "imd"))
IR_imd_output <- pois.exact(IR_imd$abortion_n, IR_imd$pyears_ey, conf.level=0.95)
...
x <- glm.nb(abortion_n ~ as.factor(imd) + offset(log(pyears_ey)), data = srh_annual_counts_final_extra)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
abortion_count_annual <- aggregate(abortion_n ~ eventyear, srh_annual_counts_final_extra, sum) 
IR_annual <- inner_join(abortion_count_annual,pyears_annual, by= c("eventyear" = "eventyear"))
IR_annual_output <- pois.exact(IR_annual$abortion_n, IR_annual$pyears_ey, conf.level=0.95)
...
## Generate IRR via negative binomial regression univariabe for  year 
x <- glm.nb(abortion_n ~ as.factor(eventyear) + offset(log(pyears_ey)), data = srh_annual_counts_final_extra)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
x <- glm.nb(abortion_n ~ as.factor(migrant_status) + as.factor(eventyear) + as.factor(eventyear_agecat) + as.factor(imd) + as.factor(prac_region) + offset(log(pyears_ey)) , 
...
abortion_glm_adj_unadj <- glm_adj_unadj
save(abortion_glm_adj_unadj, file = "filepath")
...
abortioncount_dp <- aggregate(abortion_n ~ migcertainty, srh_annual_counts_final_extra, sum) 
IR_dp <- inner_join(abortioncount_dp,pyears_dp_overall, by= c("migcertainty" = "migcertainty"))
IR_dp_output <- pois.exact(IR_dp$abortion_n, IR_dp$pyears_ey, conf.level=0.95)
...
x <- glm.nb(abortion_n ~ as.factor(migcertainty) + offset(log(pyears_ey)), data = srh_annual_counts_final_extra)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
## Generate IRR via negative binomial regression
srh_annual_counts_final_extra$migcertainty <- relevel(srh_annual_counts_final_extra$migcertainty, "Non-migrant")
x <- glm.nb(abortion_n ~ as.factor(migcertainty) + as.factor(eventyear) + as.factor(eventyear_agecat) +  as.factor(imd) + as.factor(prac_region) + offset(log(pyears_ey)), 
         data = srh_annual_counts_final_extra)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
...
abortion_glm_dp_adj_unadj <- glm_dp_adj_unadj
save(abortion_glm_dp_adj_unadj, file = "filepath")
...
abortioncount_mvnm_white_british<- aggregate(abortion_n ~ migrant_status, white_british, sum) 
IR_mvnm_white_british <- inner_join(abortioncount_mvnm_white_british,pyears_mvnm_white_british, by= c("migrant_status" = "migrant_status"))
IR_mvnm_white_british_output <- pois.exact(IR_mvnm_white_british$abortion_n, IR_mvnm_white_british$pyears_ey, conf.level=0.95)
...
x <- glm.nb(abortion_n ~ as.factor(migrant_status) + offset(log(pyears_ey)), data = white_british)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
abortioncount_mvnm_white_nonbritish<- aggregate(abortion_n ~ migrant_status, white_nonbritish, sum) 
IR_mvnm_white_nonbritish <- inner_join(abortioncount_mvnm_white_nonbritish,pyears_mvnm_white_nonbritish, by= c("migrant_status" = "migrant_status"))
IR_mvnm_white_nonbritish_output <- pois.exact(IR_mvnm_white_nonbritish$abortion_n, IR_mvnm_white_nonbritish$pyears_ey, conf.level=0.95)
...
x <- glm.nb(abortion_n ~ as.factor(migrant_status) + offset(log(pyears_ey)), data = white_nonbritish)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
abortioncount_mvnm_mixed<- aggregate(abortion_n ~ migrant_status, mixed, sum) 
IR_mvnm_mixed <- inner_join(abortioncount_mvnm_mixed,pyears_mvnm_mixed, by= c("migrant_status" = "migrant_status"))
IR_mvnm_mixed_output <- pois.exact(IR_mvnm_mixed$abortion_n, IR_mvnm_mixed$pyears_ey, conf.level=0.95)
...
x <- glm.nb(abortion_n ~ as.factor(migrant_status) + offset(log(pyears_ey)), data = mixed)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
abortioncount_mvnm_asian<- aggregate(abortion_n ~ migrant_status, asian, sum) 
IR_mvnm_asian <- inner_join(abortioncount_mvnm_asian,pyears_mvnm_asian, by= c("migrant_status" = "migrant_status"))
IR_mvnm_asian_output <- pois.exact(IR_mvnm_asian$abortion_n, IR_mvnm_asian$pyears_ey, conf.level=0.95)
...
x <- glm.nb(abortion_n ~ as.factor(migrant_status) + offset(log(pyears_ey)), data = asian)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
abortioncount_mvnm_black<- aggregate(abortion_n ~ migrant_status, black, sum) 
IR_mvnm_black <- inner_join(abortioncount_mvnm_black,pyears_mvnm_black, by= c("migrant_status" = "migrant_status"))
IR_mvnm_black_output <- pois.exact(IR_mvnm_black$abortion_n, IR_mvnm_black$pyears_ey, conf.level=0.95)
...
x <- glm.nb(abortion_n ~ as.factor(migrant_status) + offset(log(pyears_ey)), data = black)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
abortioncount_mvnm_other<- aggregate(abortion_n ~ migrant_status, other, sum) 
IR_mvnm_other <- inner_join(abortioncount_mvnm_other,pyears_mvnm_other, by= c("migrant_status" = "migrant_status"))
IR_mvnm_other_output <- pois.exact(IR_mvnm_other$abortion_n, IR_mvnm_other$pyears_ey, conf.level=0.95)
...
x <- glm.nb(abortion_n ~ as.factor(migrant_status) + offset(log(pyears_ey)), data = other)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
abortion_glm_adj_unadj_ethnicity <- glm_adj_unadj
save(abortion_glm_adj_unadj_ethnicity, file = "filepath")
...
abortioncount_mvnm <- aggregate(abortion_n ~ migrant_status, exact_match_srh_annual_counts_final_extra_4to1, sum) 
IR_mvnm <- inner_join(abortioncount_mvnm,pyears_mvnm_overall, by= c("migrant_status" = "migrant_status"))
IR_mvnm_output <- pois.exact(IR_mvnm$abortion_n, IR_mvnm$pyears_ey, conf.level=0.95)
IR_mvnm_output <- IR_mvnm_output %>%
  mutate(incidence_rate100=rate*100)
...
x <- glm.nb(abortion_n ~ as.factor(migrant_status) + offset(log(pyears_ey)), data = exact_match_srh_annual_counts_final_extra_4to1)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
glm_all_mvnm$names <- recode(glm_all_mvnm$names,"(Intercept)" = "Non-migrant" ,
                             ...
                             "as.factor(prac_region)South West" = "South West",
                             "as.factor(prac_region)South Central" = "South Central",
                             "as.factor(prac_region)London" = "London",
                             "as.factor(prac_region)South East Coast" = "South East Coast")
glm_all_mvnm$estimate <- as.numeric(glm_all_mvnm$estimate)
glm_all_mvnm <- glm_all_mvnm %>% rename(lower = "2.5 %") %>% rename(upper = "97.5 %") 
glm_all_mvnm$lower <- as.numeric(glm_all_mvnm$lower)
...
abortion_glm_adj_unadj_em <- glm_adj_unadj
save(abortion_glm_adj_unadj_em, file = "filepath")
...
unadj_irr_migrant_status <- forestplot(tabletext, mean = glm_adj_unadj$estimate, lower=glm_adj_unadj$lower, upper=glm_adj_unadj$upper,
                                       ...
                                       ci.vertices = TRUE,
                                       boxsize = 0.05,
                                       title = "Abortion (Exact Matched Cohort)",
                                       graphwidth = unit(80, 'mm'))
dev.off()
...
pyears_dp_overall <- pyears_dp_overall %>% mutate(across(is.numeric, ~ round(.,)))
abortioncount_dp <- aggregate(abortion_n ~ migcertainty, exact_match_srh_annual_counts_final_extra_4to1, sum) 
IR_dp <- inner_join(abortioncount_dp,pyears_dp_overall, by= c("migcertainty" = "migcertainty"))
IR_dp_output <- pois.exact(IR_dp$abortion_n, IR_dp$pyears_ey, conf.level=0.95)
IR_dp_output <- IR_dp_output %>%
  mutate(incidence_rate100=rate*100)
...
x <- glm.nb(abortion_n ~ as.factor(migcertainty) + offset(log(pyears_ey)), data = exact_match_srh_annual_counts_final_extra_4to1)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
abortion_glm_dp_adj_unadj_em <- glm_dp_adj_unadj
save(abortion_glm_dp_adj_unadj_em, file = "filepath")
...
abortion_glm_adj_unadj_ethnicity_em <- glm_adj_unadj
save(abortion_glm_adj_unadj_ethnicity_em, file = "filepath")
