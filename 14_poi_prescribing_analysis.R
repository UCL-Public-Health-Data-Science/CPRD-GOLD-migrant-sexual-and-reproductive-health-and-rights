...
## poi Analysis for Neha's contraception prescribing chapter in females of reproductive age (15-49yo) 
...
poipresc_annual_counts_final$prac_region <- relevel(poipresc_annual_counts_final$prac_region, "London")
poipresc_annual_counts_final_extra$prac_region <- relevel(poipresc_annual_counts_final_extra$prac_region, "London")
...
poipresc_annual_counts_final_extra_preg_max <- poipresc_annual_counts_final_extra %>% filter(pdays_ey_pregmax > 0) 
# recalculate pdays total based on counts data and join
poipresc_annual_counts_final_extra_preg_max_pdays <- poipresc_annual_counts_final_extra_preg_max %>% group_by(patid) %>% tally(pdays_ey_pregmax) %>% rename(poi_pdays_max = n )
poipresc_annual_counts_final_extra_preg_max_pyears <- poipresc_annual_counts_final_extra_preg_max %>% group_by(patid) %>% tally(pyears_ey_pregmax) %>% rename(poi_pyears_max = n )
poipresc_annual_counts_final_extra_preg_max_days_pyears <- full_join(poipresc_annual_counts_final_extra_preg_max_pdays, poipresc_annual_counts_final_extra_preg_max_pyears, by = c("patid" = "patid"))
# select patients in cohort file based on preg max assumption and add on recalculated pdays/pyears
poi_cohort_final_pregmax <- poi_cohort_final %>% filter(patid %in% poipresc_annual_counts_final_extra_preg_max$patid) %>% dplyr::select(-poi_pdays_max, -poi_pdays_min, -poi_pyears_max, -poi_pyears_min)
poi_cohort_final_pregmax <- left_join(poi_cohort_final_pregmax, poipresc_annual_counts_final_extra_preg_max_days_pyears, by = c("patid" = "patid"))

## Totals ---

## Whole cohort 
total_cohort_n <- count(poi_cohort_final_pregmax)
total_poi_pyears_max <- sum(poi_cohort_final_pregmax$poi_pyears_max)
total_wholecohort <- bind_cols(total_cohort_n, total_poi_pyears_max)
colnames(total_wholecohort) <- c("individuals", "poi_pyears_max")
total_wholecohort <- total_wholecohort %>% mutate(group = "Whole cohort") 

## By migrant status
total_mvnm_n <- poi_cohort_final_pregmax %>% group_by(migrant_status) %>% 
  count() %>% 
  rename(individuals = n) %>% 
  rename(group = migrant_status)
total_mvnm_poi_pyears_max <- poi_cohort_final_pregmax %>% group_by(migrant_status) %>%
  tally(poi_pyears_max) %>%
  rename(poi_pyears_max = n) %>% 
  rename(group = migrant_status)
total_mvnm <- full_join(total_mvnm_n, total_mvnm_poi_pyears_max,
                        by = c("group" = "group"))

## By migrant certainty
total_dp_n <- poi_cohort_final_pregmax %>% group_by(migcertainty) %>% 
  count() %>% 
  rename(individuals = n) %>% 
  rename(group = migcertainty)
total_dp_poi_pyears_max <- poi_cohort_final_pregmax %>% group_by(migcertainty) %>%
  tally(poi_pyears_max) %>%
  rename(poi_pyears_max = n) %>% 
  rename(group = migcertainty)
total_dp <- full_join(total_dp_n, total_dp_poi_pyears_max,
                      by = c("group" = "group"))

## Totals for export
totals <- full_join(total_mvnm, total_dp,
                    by = c("group" = "group", "individuals" = "individuals", "poi_pyears_max" = "poi_pyears_max")) %>% 
  full_join(total_wholecohort, by = c("group" = "group", "individuals" = "individuals", "poi_pyears_max" = "poi_pyears_max")) %>% 
...
totals_percent <- dplyr::select(totals_percent, c(group, individuals, poi_pyears_max))
...
poi_cohort_final_pregmax_age <- poi_cohort_final_pregmax %>%
  mutate(age_cohort_entry = calc_age(dob, poi_cohort_entry))
mean_ages_overall <- psych::describe(poi_cohort_final_pregmax_age$age_cohort_entry, IQR = TRUE)
mean_ages_overall

nm <- poi_cohort_final_pregmax_age %>% filter(migrant_status == "Non-migrant")
nm_age <- psych::describe(nm$age_cohort_entry, IQR = TRUE)
nm_age

m <- poi_cohort_final_pregmax_age %>% filter(migrant_status == "Migrant")
m_age <- psych::describe(m$age_cohort_entry, IQR = TRUE)
m_age

d <- poi_cohort_final_pregmax_age %>% filter(migcertainty == "Definite")
d_age <- psych::describe(d$age_cohort_entry, IQR = TRUE)
d_age

p <- poi_cohort_final_pregmax_age %>% filter(migcertainty == "Probable")
...
poi_cohort_final_pregmax_agecat <- poi_cohort_final_pregmax_age %>%
  mutate(agecat_cohort_entry = age_cohort_entry)
poi_cohort_final_pregmax_agecat <- poi_cohort_final_pregmax_agecat %>%
...
poi_cohort_final_pregmax_agecat$agecat_cohort_entry <- factor(poi_cohort_final_pregmax_agecat$agecat_cohort_entry, levels = c(1,2,3,4,5,6,7),
                                                      labels = c("15-19", "20-24", "25-29", "30-34", "35-39", "40-44", "45-49"))

agecat <- poi_cohort_final_pregmax_agecat %>% 
  group_by(agecat_cohort_entry) %>% count() 
agecat <- agecat %>% mutate(percent = (n/sum(agecat$n))*100) %>%
  mutate(group = "Whole Cohort")
agecat_mvnm <- poi_cohort_final_pregmax_agecat %>% 
...
agecat_dp <- poi_cohort_final_pregmax_agecat %>% 
...
poi_pyears_max_agecat <- aggregate(poi_pyears_max ~ agecat_cohort_entry, poi_cohort_final_pregmax_agecat, sum) %>% 
  mutate(group = "Whole Cohort")
poi_pyears_max_agecat_mvnm <- aggregate(poi_pyears_max ~ agecat_cohort_entry + migrant_status, poi_cohort_final_pregmax_agecat, sum) %>% 
  rename(group = migrant_status)
poi_pyears_max_agecat_dp <- aggregate(poi_pyears_max ~ agecat_cohort_entry + migcertainty, poi_cohort_final_pregmax_agecat, sum) %>% 
  rename(group = migcertainty)
poi_pyears_max_agecat_all <- poi_pyears_max_agecat_mvnm %>% full_join(poi_pyears_max_agecat_dp,by = c("agecat_cohort_entry"="agecat_cohort_entry",
                                                                                                      "group"="group","poi_pyears_max"="poi_pyears_max")) %>% 
  full_join(poi_pyears_max_agecat,by = c("agecat_cohort_entry"="agecat_cohort_entry",
                                         "group"="group","poi_pyears_max"="poi_pyears_max")) %>%
  rename(agecat = agecat_cohort_entry) 

poi_pyears_max_agecat_export <- full_join(poi_pyears_max_agecat_all, age_cat_all, 
                                          by = c("agecat"="agecat","group"="group")) %>% 
  mutate(across(is.numeric, ~ round(.,0))) %>% 
  dplyr::select(c(group, agecat, n_percent, poi_pyears_max)) 

write_csv(poi_pyears_max_agecat_export, "filepath")


## Ethnicity ---


poi_cohort_final_pregmax$ethnicat6 <- poi_cohort_final_pregmax$ethnicat6 %>% fct_explicit_na(na_level = "Not known")
levels(poi_cohort_final_pregmax$ethnicat6)

ethnicat6 <- poi_cohort_final_pregmax %>% 
  group_by(ethnicat6) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
ethnicat6_mvnm <- poi_cohort_final_pregmax %>% 
...
ethnicat6_dp <- poi_cohort_final_pregmax %>% 
...
poi_pyears_max_ethnicat6 <- poi_cohort_final_pregmax %>% group_by(ethnicat6) %>% tally(poi_pyears_max) %>% 
  mutate(group = "Whole Cohort")
poi_pyears_max_ethnicat6_mvnm <- poi_cohort_final_pregmax %>% group_by(migrant_status, ethnicat6) %>% tally(poi_pyears_max) %>% 
  rename(group = migrant_status)
poi_pyears_max_ethnicat6_dp <- poi_cohort_final_pregmax %>% group_by(migcertainty, ethnicat6) %>% tally(poi_pyears_max) %>% 
  rename(group = migcertainty)
poi_pyears_max_ethnicat6_all <- poi_pyears_max_ethnicat6_mvnm %>% full_join(poi_pyears_max_ethnicat6_dp,by = c("ethnicat6"="ethnicat6",
                                                                                                               "group"="group","n"="n")) %>% 
  full_join(poi_pyears_max_ethnicat6,by = c("ethnicat6"="ethnicat6",
                                            "group"="group","n"="n")) %>%
  rename(poi_pyears_max = n) %>% 
  mutate(across(is.numeric, ~ round(.,0)))

poi_pyears_max_ethnicat6_export <- full_join(poi_pyears_max_ethnicat6_all, ethnicat6_all, 
                                             by = c("ethnicat6"="ethnicat6","group"="group")) %>% 
  relocate(group,ethnicat6,n_percent,poi_pyears_max)  %>% 
  arrange(group)

write_csv(poi_pyears_max_ethnicat6_export, "filepath")


## practice region ---

poi_cohort_final_pregmax$prac_region <- poi_cohort_final_pregmax$prac_region %>% fct_explicit_na(na_level = "Not known")
levels(poi_cohort_final_pregmax$prac_region)

prac_region <- poi_cohort_final_pregmax %>% 
  group_by(prac_region) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
prac_region_mvnm <- poi_cohort_final_pregmax %>% 
...
prac_region_dp <- poi_cohort_final_pregmax %>% 
...
poi_pyears_max_prac_region <- poi_cohort_final_pregmax %>% group_by(prac_region) %>% tally(poi_pyears_max) %>% 
  mutate(group = "Whole Cohort")
poi_pyears_max_prac_region_mvnm <- poi_cohort_final_pregmax %>% group_by(migrant_status, prac_region) %>% tally(poi_pyears_max) %>% 
  rename(group = migrant_status)
poi_pyears_max_prac_region_dp <- poi_cohort_final_pregmax %>% group_by(migcertainty, prac_region) %>% tally(poi_pyears_max) %>% 
  rename(group = migcertainty)
poi_pyears_max_prac_region_all <- poi_pyears_max_prac_region_mvnm %>% full_join(poi_pyears_max_prac_region_dp,by = c("prac_region"="prac_region",
                                                                                                                     "group"="group","n"="n")) %>% 
  full_join(poi_pyears_max_prac_region,by = c("prac_region"="prac_region",
                                              "group"="group","n"="n")) %>%
  rename(poi_pyears_max = n) %>% 
  mutate(across(is.numeric, ~ round(.,0)))

poi_pyears_max_prac_region_export <- full_join(poi_pyears_max_prac_region_all, prac_region_all, 
                                               by = c("prac_region"="prac_region","group"="group")) %>% 
  relocate(group,prac_region,n_percent,poi_pyears_max)  %>% 
  arrange(group)

write_csv(poi_pyears_max_prac_region_export, "filepath")


## IMD ---

poi_cohort_final_pregmax$imd<- poi_cohort_final_pregmax$imd%>% fct_explicit_na(na_level = "Not known")
levels(poi_cohort_final_pregmax$imd)

imd<- poi_cohort_final_pregmax %>% 
  group_by(imd) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
imd_mvnm <- poi_cohort_final_pregmax %>% 
...
imd_dp <- poi_cohort_final_pregmax %>% 
...
poi_pyears_max_imd <- poi_cohort_final_pregmax %>% group_by(imd) %>% tally(poi_pyears_max) %>% 
  mutate(group = "Whole Cohort")
poi_pyears_max_imd_mvnm <- poi_cohort_final_pregmax %>% group_by(migrant_status, imd) %>% tally(poi_pyears_max) %>% 
  rename(group = migrant_status)
poi_pyears_max_imd_dp <- poi_cohort_final_pregmax %>% group_by(migcertainty, imd) %>% tally(poi_pyears_max) %>% 
  rename(group = migcertainty)
poi_pyears_max_imd_all <- poi_pyears_max_imd_mvnm %>% full_join(poi_pyears_max_imd_dp,by = c("imd"="imd",
                                                                                             "group"="group","n"="n")) %>% 
  full_join(poi_pyears_max_imd,by = c("imd"="imd",
                                      "group"="group","n"="n")) %>%
  rename(poi_pyears_max = n) %>% 
  mutate(across(is.numeric, ~ round(.,0)))

poi_pyears_max_imd_export <- full_join(poi_pyears_max_imd_all, imd_all, 
                                       by = c("imd"="imd","group"="group")) %>% 
  relocate(group,imd,n_percent,poi_pyears_max)  %>% 
  arrange(group)

write_csv(poi_pyears_max_imd_export, "filepath")



## Year of cohort entry ---

poi_cohort_final_pregmax_yearofentry <- poi_cohort_final_pregmax %>% mutate(yearofentry = year(poi_cohort_entry))
mean_yearofentry_overall <- psych::describe(poi_cohort_final_pregmax_yearofentry$yearofentry, IQR = TRUE)
mean_yearofentry_overall

nm <- poi_cohort_final_pregmax_yearofentry %>% filter(migrant_status == "Non-migrant")
nm_yearofentry <- psych::describe(nm$yearofentry, IQR = TRUE)
nm_yearofentry

m <- poi_cohort_final_pregmax_yearofentry %>% filter(migrant_status == "Migrant")
m_yearofentry <- psych::describe(m$yearofentry, IQR = TRUE)
m_yearofentry

d <- poi_cohort_final_pregmax_yearofentry %>% filter(migcertainty == "Definite")
d_yearofentry  <- psych::describe(d$yearofentry, IQR = TRUE)
d_yearofentry 

p <- poi_cohort_final_pregmax_yearofentry %>% filter(migcertainty == "Probable")
...
poi_cohort_final_pregmax_yearofentry$yearofentry <- factor(poi_cohort_final_pregmax_yearofentry$yearofentry,  levels = c(2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018), 
                                                   labels = c("2009", "2010", "2011", "2012", "2013", "2014", "2015", "2016", "2017", "2018"))
yearofentry <- poi_cohort_final_pregmax_yearofentry %>% 
  group_by(yearofentry) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
yearofentry_mvnm <- poi_cohort_final_pregmax_yearofentry %>% 
...
yearofentry_dp <- poi_cohort_final_pregmax_yearofentry %>% 
...
poi_pyears_max_yearofentry <- poi_cohort_final_pregmax_yearofentry %>% group_by(yearofentry) %>% tally(poi_pyears_max) %>% 
  mutate(group = "Whole Cohort")
poi_pyears_max_yearofentry_mvnm <- poi_cohort_final_pregmax_yearofentry %>% group_by(migrant_status, yearofentry) %>% tally(poi_pyears_max) %>% 
  rename(group = migrant_status)
poi_pyears_max_yearofentry_dp <- poi_cohort_final_pregmax_yearofentry %>% group_by(migcertainty, yearofentry) %>% tally(poi_pyears_max) %>% 
  rename(group = migcertainty)
poi_pyears_max_yearofentry_all <- poi_pyears_max_yearofentry_mvnm %>% full_join(poi_pyears_max_yearofentry_dp,by = c("yearofentry"="yearofentry",
                                                                                                                     "group"="group","n"="n")) %>% 
  full_join(poi_pyears_max_yearofentry,by = c("yearofentry"="yearofentry",
                                              "group"="group","n"="n")) %>%
  rename(poi_pyears_max = n) %>% 
  mutate(across(is.numeric, ~ round(.,0)))

poi_pyears_max_yearofentry_export <- full_join(poi_pyears_max_yearofentry_all, yearofentry_all, 
                                               by = c("group"="group","yearofentry"="yearofentry")) %>% 
  relocate(group,yearofentry,n_percent,poi_pyears_max)  %>% 
  arrange(group)

write_csv(poi_pyears_max_yearofentry_export, "filepath")


## Year ---

eventyear<- poipresc_annual_counts_final_extra_preg_max %>% 
  group_by(eventyear) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
eventyear_mvnm <- poipresc_annual_counts_final_extra_preg_max  %>% 
...
eventyear_dp <- poipresc_annual_counts_final_extra_preg_max  %>% 
...
pyears_eventyear <- poipresc_annual_counts_final_extra_preg_max  %>% group_by(eventyear) %>% tally(pyears_ey_pregmax) %>% 
  mutate(group = "Whole Cohort")
pyears_eventyear_mvnm <- poipresc_annual_counts_final_extra_preg_max  %>% group_by(migrant_status, eventyear) %>% tally(pyears_ey_pregmax) %>% 
  rename(group = migrant_status)
pyears_eventyear_dp <- poipresc_annual_counts_final_extra_preg_max  %>% group_by(migcertainty, eventyear) %>% tally(pyears_ey_pregmax) %>% 
  rename(group = migcertainty)
...
pyears_eventyear_export <- full_join(pyears_eventyear_all, eventyear_all, 
  ...
  relocate(group,eventyear,n_percent,poi_pyears_max)  %>% 
  arrange(group)
...
mean_poi_pyears_max_overall <- psych::describe(poi_cohort_final_pregmax$poi_pyears_max, IQR = TRUE)
mean_poi_pyears_max_overall

nm <- poi_cohort_final_pregmax %>% filter(migrant_status == "Non-migrant")
nm_poi_pyears_max <- psych::describe(nm$poi_pyears_max, IQR = TRUE)
nm_poi_pyears_max

m <- poi_cohort_final_pregmax %>% filter(migrant_status == "Migrant")
m_poi_pyears_max <- psych::describe(m$poi_pyears_max, IQR = TRUE)
m_poi_pyears_max

d <- poi_cohort_final_pregmax %>% filter(migcertainty == "Definite")
d_poi_pyears_max <- psych::describe(d$poi_pyears_max, IQR = TRUE)
d_poi_pyears_max

p <- poi_cohort_final_pregmax %>% filter(migcertainty == "Probable")
p_poi_pyears_max <- psych::describe(p$poi_pyears_max, IQR = TRUE)
p_poi_pyears_max

## join export mean + median ages + sd + IQR
mean_poi_pyears_max_overall <- as.data.frame((mean_poi_pyears_max_overall))
mean_poi_pyears_max_overall <- dplyr::select(mean_poi_pyears_max_overall, c(mean , sd, median, IQR)) %>% mutate(group = "Whole cohort") 
nm_poi_pyears_max <- as.data.frame((nm_poi_pyears_max))
nm_poi_pyears_max <- dplyr::select(nm_poi_pyears_max, c(mean , sd, median, IQR)) %>% mutate(group = "Non-migrant") 
m_poi_pyears_max <- as.data.frame((m_poi_pyears_max))
m_poi_pyears_max <- dplyr::select(m_poi_pyears_max, c(mean , sd, median, IQR)) %>% mutate(group = "Migrant") 
d_poi_pyears_max <- as.data.frame((d_poi_pyears_max))
d_poi_pyears_max <- dplyr::select(d_poi_pyears_max, c(mean , sd, median, IQR)) %>% mutate(group = "Definite") 
p_poi_pyears_max <- as.data.frame((p_poi_pyears_max))
p_poi_pyears_max <- dplyr::select(p_poi_pyears_max, c(mean , sd, median, IQR)) %>% mutate(group = "Probable") 
all_mean_poi_pyears_max <- full_join(mean_poi_pyears_max_overall, nm_poi_pyears_max,
                                     by = c( "group" = "group", "mean" = "mean", "sd" = "sd" , "median" = "median", "IQR" = "IQR")) %>% 
  full_join(m_poi_pyears_max, by = c( "group" = "group", "mean" = "mean", "sd" = "sd" , "median" = "median", "IQR" = "IQR") ) %>%
  full_join(d_poi_pyears_max, by = c( "group" = "group", "mean" = "mean", "sd" = "sd", "median" = "median",  "IQR" = "IQR" ) ) %>%
  full_join(p_poi_pyears_max, by = c( "group" = "group", "mean" = "mean", "sd" = "sd" , "median" = "median", "IQR" = "IQR") )  %>%
  mutate(across(is.numeric, ~ round(.,2)))
all_mean_poi_pyears_max$mean_sd <- paste(all_mean_poi_pyears_max$mean, all_mean_poi_pyears_max$sd, sep =", ")
all_mean_poi_pyears_max$median_iqr <- paste(all_mean_poi_pyears_max$median, all_mean_poi_pyears_max$IQR, sep =", ")
all_mean_poi_pyears_max <- dplyr::select(all_mean_poi_pyears_max, c(group, mean_sd, median_iqr))
write_csv(all_mean_poi_pyears_max, "filepath" )
...
## change cohort final to be based on preg max assumption
# drop records with 0 or less pdays_ey_pregmax in counts
exact_match_poipresc_annual_counts_final_extra_4to1_preg_max <- exact_match_poipresc_annual_counts_final_extra_4to1 %>% filter(pdays_ey_pregmax > 0) 
# recalculate pdays total based on counts data and join
exact_match_poipresc_annual_counts_final_extra_4to1_preg_max_pdays <- exact_match_poipresc_annual_counts_final_extra_4to1_preg_max %>% group_by(patid) %>% tally(pdays_ey_pregmax) %>% rename(poi_pdays_max = n )
exact_match_poipresc_annual_counts_final_extra_4to1_preg_max_pyears <- exact_match_poipresc_annual_counts_final_extra_4to1_preg_max %>% group_by(patid) %>% tally(pyears_ey_pregmax) %>% rename(poi_pyears_max = n )
exact_match_poipresc_annual_counts_final_extra_4to1_preg_max_days_pyears <- full_join(exact_match_poipresc_annual_counts_final_extra_4to1_preg_max_pdays, exact_match_poipresc_annual_counts_final_extra_4to1_preg_max_pyears, by = c("patid" = "patid"))
# select patients in cohort file based on preg max assumption and add on recalculated pdays/pyears
exact_match_poi_cohort_final_4to1_pregmax <- exact_match_poi_cohort_final_4to1 %>% filter(patid %in% exact_match_poipresc_annual_counts_final_extra_4to1_preg_max$patid) %>% dplyr::select(-poi_pdays_max, -poi_pdays_min, -poi_pyears_max, -poi_pyears_min)
exact_match_poi_cohort_final_4to1_pregmax <- left_join(exact_match_poi_cohort_final_4to1_pregmax, exact_match_poipresc_annual_counts_final_extra_4to1_preg_max_days_pyears, by = c("patid" = "patid"))

## Totals ---

## Whole cohort 
total_cohort_n <- count(exact_match_poi_cohort_final_4to1_pregmax)
total_poi_pyears_max <- sum(exact_match_poi_cohort_final_4to1_pregmax$poi_pyears_max)
total_wholecohort <- bind_cols(total_cohort_n, total_poi_pyears_max)
colnames(total_wholecohort) <- c("individuals", "poi_pyears_max")
total_wholecohort <- total_wholecohort %>% mutate(group = "Whole cohort") 

## By migrant status
total_mvnm_n <- exact_match_poi_cohort_final_4to1_pregmax %>% group_by(migrant_status) %>% 
  count() %>% 
  rename(individuals = n) %>% 
  rename(group = migrant_status)
total_mvnm_poi_pyears_max <- exact_match_poi_cohort_final_4to1_pregmax %>% group_by(migrant_status) %>%
  tally(poi_pyears_max) %>%
  rename(poi_pyears_max = n) %>% 
  rename(group = migrant_status)
total_mvnm <- full_join(total_mvnm_n, total_mvnm_poi_pyears_max,
...
## By migrant certainty
total_dp_n <- exact_match_poi_cohort_final_4to1_pregmax %>% group_by(migcertainty) %>% 
  count() %>% 
  rename(individuals = n) %>% 
  rename(group = migcertainty)
total_dp_poi_pyears_max <- exact_match_poi_cohort_final_4to1_pregmax %>% group_by(migcertainty) %>%
  tally(poi_pyears_max) %>%
  rename(poi_pyears_max = n) %>% 
  rename(group = migcertainty)
total_dp <- full_join(total_dp_n, total_dp_poi_pyears_max,
...
exact_match_poi_cohort_final_4to1_pregmax_age <- exact_match_poi_cohort_final_4to1_pregmax %>%
  mutate(age_cohort_entry = calc_age(dob, poi_cohort_entry))
mean_ages_overall <- psych::describe(exact_match_poi_cohort_final_4to1_pregmax_age$age_cohort_entry, IQR = TRUE)
mean_ages_overall

nm <- exact_match_poi_cohort_final_4to1_pregmax_age %>% filter(migrant_status == "Non-migrant")
nm_age <- psych::describe(nm$age_cohort_entry, IQR = TRUE)
nm_age

m <- exact_match_poi_cohort_final_4to1_pregmax_age %>% filter(migrant_status == "Migrant")
m_age <- psych::describe(m$age_cohort_entry, IQR = TRUE)
m_age

d <- exact_match_poi_cohort_final_4to1_pregmax_age %>% filter(migcertainty == "Definite")
d_age <- psych::describe(d$age_cohort_entry, IQR = TRUE)
d_age

p <- exact_match_poi_cohort_final_4to1_pregmax_age %>% filter(migcertainty == "Probable")
p_age <- psych::describe(p$age_cohort_entry, IQR = TRUE)
p_age
...
## Age category - No individuals and p years per group 
exact_match_poi_cohort_final_4to1_pregmax_agecat <- exact_match_poi_cohort_final_4to1_pregmax_age %>%
  mutate(agecat_cohort_entry = age_cohort_entry)
exact_match_poi_cohort_final_4to1_pregmax_agecat <- exact_match_poi_cohort_final_4to1_pregmax_agecat %>%
  mutate(agecat_cohort_entry=replace(agecat_cohort_entry, between (agecat_cohort_entry, 15, 19), 1)) %>%
  mutate(agecat_cohort_entry=replace(agecat_cohort_entry, between (agecat_cohort_entry, 20, 24), 2)) %>%
  mutate(agecat_cohort_entry=replace(agecat_cohort_entry, between (agecat_cohort_entry, 25, 29), 3)) %>%
  mutate(agecat_cohort_entry=replace(agecat_cohort_entry, between (agecat_cohort_entry, 30, 34), 4)) %>%
  ...
  mutate(agecat_cohort_entry=replace(agecat_cohort_entry, between (agecat_cohort_entry, 40, 44), 6)) %>%
  mutate(agecat_cohort_entry=replace(agecat_cohort_entry, between (agecat_cohort_entry, 45, 49), 7)) 
exact_match_poi_cohort_final_4to1_pregmax_agecat$agecat_cohort_entry <- factor(exact_match_poi_cohort_final_4to1_pregmax_agecat$agecat_cohort_entry, levels = c(1,2,3,4,5,6,7),
                                                              labels = c("15-19", "20-24", "25-29", "30-34", "35-39", "40-44", "45-49"))

agecat <- exact_match_poi_cohort_final_4to1_pregmax_agecat %>% 
  group_by(agecat_cohort_entry) %>% count() 
agecat <- agecat %>% mutate(percent = (n/sum(agecat$n))*100) %>%
  mutate(group = "Whole Cohort")
agecat_mvnm <- exact_match_poi_cohort_final_4to1_pregmax_agecat %>% 
  group_by(agecat_cohort_entry, migrant_status) %>% count() %>% 
  rename(group = migrant_status)
agecat_m <- agecat_mvnm %>% filter(group == "Migrant")
agecat_m <- agecat_m %>% mutate(percent = (n/sum(agecat_m$n))*100)
agecat_nm <- agecat_mvnm %>% filter(group == "Non-migrant")
agecat_nm <- agecat_nm %>% mutate(percent = (n/sum(agecat_nm$n))*100)
agecat_dp <- exact_match_poi_cohort_final_4to1_pregmax_agecat %>% 
  group_by(agecat_cohort_entry, migcertainty) %>% count() %>% 
  rename(group = migcertainty)
agecat_d <- agecat_dp %>% filter(group == "Definite")
agecat_d <- agecat_d %>% mutate(percent = (n/sum(agecat_d$n))*100)
...
poi_pyears_max_agecat <- aggregate(poi_pyears_max ~ agecat_cohort_entry, exact_match_poi_cohort_final_4to1_pregmax_agecat, sum) %>% 
  mutate(group = "Whole Cohort")
poi_pyears_max_agecat_mvnm <- aggregate(poi_pyears_max ~ agecat_cohort_entry + migrant_status, exact_match_poi_cohort_final_4to1_pregmax_agecat, sum) %>% 
  rename(group = migrant_status)
poi_pyears_max_agecat_dp <- aggregate(poi_pyears_max ~ agecat_cohort_entry + migcertainty, exact_match_poi_cohort_final_4to1_pregmax_agecat, sum) %>% 
  rename(group = migcertainty)
poi_pyears_max_agecat_all <- poi_pyears_max_agecat_mvnm %>% full_join(poi_pyears_max_agecat_dp,by = c("agecat_cohort_entry"="agecat_cohort_entry",
                                                                                                      "group"="group","poi_pyears_max"="poi_pyears_max")) %>% 
  full_join(poi_pyears_max_agecat,by = c("agecat_cohort_entry"="agecat_cohort_entry",
...
exact_match_poi_cohort_final_4to1_pregmax$ethnicat6 <- exact_match_poi_cohort_final_4to1_pregmax$ethnicat6 %>% fct_explicit_na(na_level = "Not known")
levels(exact_match_poi_cohort_final_4to1_pregmax$ethnicat6)

ethnicat6 <- exact_match_poi_cohort_final_4to1_pregmax %>% 
  group_by(ethnicat6) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
ethnicat6_mvnm <- exact_match_poi_cohort_final_4to1_pregmax %>% 
  group_by(ethnicat6, migrant_status) %>% count() %>% 
  rename(group = migrant_status) %>% 
  full_join(total_mvnm, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1))) %>%
  dplyr::select(c(ethnicat6, group, n, percent))
ethnicat6_dp <- exact_match_poi_cohort_final_4to1_pregmax %>% 
  group_by(ethnicat6, migcertainty) %>% count() %>% 
  rename(group = migcertainty) %>% 
  full_join(total_dp, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
...
poi_pyears_max_ethnicat6 <- exact_match_poi_cohort_final_4to1_pregmax %>% group_by(ethnicat6) %>% tally(poi_pyears_max) %>% 
  mutate(group = "Whole Cohort")
poi_pyears_max_ethnicat6_mvnm <- exact_match_poi_cohort_final_4to1_pregmax %>% group_by(migrant_status, ethnicat6) %>% tally(poi_pyears_max) %>% 
  rename(group = migrant_status)
poi_pyears_max_ethnicat6_dp <- exact_match_poi_cohort_final_4to1_pregmax %>% group_by(migcertainty, ethnicat6) %>% tally(poi_pyears_max) %>% 
  rename(group = migcertainty)
poi_pyears_max_ethnicat6_all <- poi_pyears_max_ethnicat6_mvnm %>% full_join(poi_pyears_max_ethnicat6_dp,by = c("ethnicat6"="ethnicat6",
                                                                                                               "group"="group","n"="n")) %>% 
  full_join(poi_pyears_max_ethnicat6,by = c("ethnicat6"="ethnicat6",
...
exact_match_poi_cohort_final_4to1_pregmax$prac_region <- exact_match_poi_cohort_final_4to1_pregmax$prac_region %>% fct_explicit_na(na_level = "Not known")
levels(exact_match_poi_cohort_final_4to1_pregmax$prac_region)

prac_region <- exact_match_poi_cohort_final_4to1_pregmax %>% 
  group_by(prac_region) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
prac_region_mvnm <- exact_match_poi_cohort_final_4to1_pregmax %>% 
  group_by(prac_region, migrant_status) %>% count() %>% 
  rename(group = migrant_status) %>% 
  full_join(total_mvnm, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1))) %>%
  dplyr::select(c(prac_region, group, n, percent))
prac_region_dp <- exact_match_poi_cohort_final_4to1_pregmax %>% 
  group_by(prac_region, migcertainty) %>% count() %>% 
  rename(group = migcertainty) %>% 
  full_join(total_dp, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
...
poi_pyears_max_prac_region <- exact_match_poi_cohort_final_4to1_pregmax %>% group_by(prac_region) %>% tally(poi_pyears_max) %>% 
  mutate(group = "Whole Cohort")
poi_pyears_max_prac_region_mvnm <- exact_match_poi_cohort_final_4to1_pregmax %>% group_by(migrant_status, prac_region) %>% tally(poi_pyears_max) %>% 
  rename(group = migrant_status)
poi_pyears_max_prac_region_dp <- exact_match_poi_cohort_final_4to1_pregmax %>% group_by(migcertainty, prac_region) %>% tally(poi_pyears_max) %>% 
  rename(group = migcertainty)
poi_pyears_max_prac_region_all <- poi_pyears_max_prac_region_mvnm %>% full_join(poi_pyears_max_prac_region_dp,by = c("prac_region"="prac_region",
                                                                                                                     "group"="group","n"="n")) %>% 
  full_join(poi_pyears_max_prac_region,by = c("prac_region"="prac_region",
...
exact_match_poi_cohort_final_4to1_pregmax$imd<- exact_match_poi_cohort_final_4to1_pregmax$imd%>% fct_explicit_na(na_level = "Not known")
levels(exact_match_poi_cohort_final_4to1_pregmax$imd)

imd<- exact_match_poi_cohort_final_4to1_pregmax %>% 
  group_by(imd) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
imd_mvnm <- exact_match_poi_cohort_final_4to1_pregmax %>% 
  group_by(imd, migrant_status) %>% count() %>% 
  rename(group = migrant_status) %>% 
  full_join(total_mvnm, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1))) %>%
  dplyr::select(c(imd, group, n, percent))
imd_dp <- exact_match_poi_cohort_final_4to1_pregmax %>% 
  group_by(imd, migcertainty) %>% count() %>% 
  rename(group = migcertainty) %>% 
  full_join(total_dp, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
...
poi_pyears_max_imd <- exact_match_poi_cohort_final_4to1_pregmax %>% group_by(imd) %>% tally(poi_pyears_max) %>% 
  mutate(group = "Whole Cohort")
poi_pyears_max_imd_mvnm <- exact_match_poi_cohort_final_4to1_pregmax %>% group_by(migrant_status, imd) %>% tally(poi_pyears_max) %>% 
  rename(group = migrant_status)
poi_pyears_max_imd_dp <- exact_match_poi_cohort_final_4to1_pregmax %>% group_by(migcertainty, imd) %>% tally(poi_pyears_max) %>% 
  rename(group = migcertainty)
poi_pyears_max_imd_all <- poi_pyears_max_imd_mvnm %>% full_join(poi_pyears_max_imd_dp,by = c("imd"="imd",
                                                                                             "group"="group","n"="n")) %>% 
  full_join(poi_pyears_max_imd,by = c("imd"="imd",
...
exact_match_poi_cohort_final_4to1_pregmax_yearofentry <- exact_match_poi_cohort_final_4to1_pregmax %>% mutate(yearofentry = year(poi_cohort_entry))
mean_yearofentry_overall <- psych::describe(exact_match_poi_cohort_final_4to1_pregmax_yearofentry$yearofentry, IQR = TRUE)
mean_yearofentry_overall

nm <- exact_match_poi_cohort_final_4to1_pregmax_yearofentry %>% filter(migrant_status == "Non-migrant")
nm_yearofentry <- psych::describe(nm$yearofentry, IQR = TRUE)
nm_yearofentry

m <- exact_match_poi_cohort_final_4to1_pregmax_yearofentry %>% filter(migrant_status == "Migrant")
m_yearofentry <- psych::describe(m$yearofentry, IQR = TRUE)
m_yearofentry

d <- exact_match_poi_cohort_final_4to1_pregmax_yearofentry %>% filter(migcertainty == "Definite")
d_yearofentry  <- psych::describe(d$yearofentry, IQR = TRUE)
d_yearofentry 

p <- exact_match_poi_cohort_final_4to1_pregmax_yearofentry %>% filter(migcertainty == "Probable")
p_yearofentry <- psych::describe(p$yearofentry, IQR = TRUE)
p_yearofentry
...
exact_match_poi_cohort_final_4to1_pregmax_yearofentry$yearofentry <- factor(exact_match_poi_cohort_final_4to1_pregmax_yearofentry$yearofentry,  levels = c(2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018), 
                                                           labels = c("2009", "2010", "2011", "2012", "2013", "2014", "2015", "2016", "2017", "2018"))
yearofentry <- exact_match_poi_cohort_final_4to1_pregmax_yearofentry %>% 
  group_by(yearofentry) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
yearofentry_mvnm <- exact_match_poi_cohort_final_4to1_pregmax_yearofentry %>% 
  group_by(yearofentry, migrant_status) %>% count() %>% 
  rename(group = migrant_status) %>% 
  full_join(total_mvnm, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1))) %>%
  dplyr::select(c(yearofentry, group, n, percent))
yearofentry_dp <- exact_match_poi_cohort_final_4to1_pregmax_yearofentry %>% 
  group_by(yearofentry, migcertainty) %>% count() %>% 
  rename(group = migcertainty) %>% 
  full_join(total_dp, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
...
poi_pyears_max_yearofentry <- exact_match_poi_cohort_final_4to1_pregmax_yearofentry %>% group_by(yearofentry) %>% tally(poi_pyears_max) %>% 
  mutate(group = "Whole Cohort")
poi_pyears_max_yearofentry_mvnm <- exact_match_poi_cohort_final_4to1_pregmax_yearofentry %>% group_by(migrant_status, yearofentry) %>% tally(poi_pyears_max) %>% 
  rename(group = migrant_status)
poi_pyears_max_yearofentry_dp <- exact_match_poi_cohort_final_4to1_pregmax_yearofentry %>% group_by(migcertainty, yearofentry) %>% tally(poi_pyears_max) %>% 
  rename(group = migcertainty)
poi_pyears_max_yearofentry_all <- poi_pyears_max_yearofentry_mvnm %>% full_join(poi_pyears_max_yearofentry_dp,by = c("yearofentry"="yearofentry",
                                                                                                                     "group"="group","n"="n")) %>% 
  full_join(poi_pyears_max_yearofentry,by = c("yearofentry"="yearofentry",
...
eventyear<- exact_match_poipresc_annual_counts_final_extra_4to1_preg_max %>% 
  group_by(eventyear) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
eventyear_mvnm <- exact_match_poipresc_annual_counts_final_extra_4to1_preg_max  %>% 
  group_by(eventyear, migrant_status) %>% count() %>% 
  rename(group = migrant_status) %>% 
  full_join(total_mvnm, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1))) %>%
  dplyr::select(c(eventyear, group, n, percent))
eventyear_dp <- exact_match_poipresc_annual_counts_final_extra_4to1_preg_max  %>% 
  group_by(eventyear, migcertainty) %>% count() %>% 
  rename(group = migcertainty) %>% 
  full_join(total_dp, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
...
pyears_eventyear <- exact_match_poipresc_annual_counts_final_extra_4to1_preg_max  %>% group_by(eventyear) %>% tally(pyears_ey_pregmax) %>% 
  mutate(group = "Whole Cohort")
pyears_eventyear_mvnm <- exact_match_poipresc_annual_counts_final_extra_4to1_preg_max  %>% group_by(migrant_status, eventyear) %>% tally(pyears_ey_pregmax) %>% 
  rename(group = migrant_status)
pyears_eventyear_dp <- exact_match_poipresc_annual_counts_final_extra_4to1_preg_max  %>% group_by(migcertainty, eventyear) %>% tally(pyears_ey_pregmax) %>% 
  rename(group = migcertainty)
pyears_eventyear_all <- pyears_eventyear_mvnm %>% full_join(pyears_eventyear_dp,by = c("eventyear"="eventyear",
                                                                                       "group"="group","n"="n")) %>% 
  full_join(pyears_eventyear,by = c("eventyear"="eventyear",
...
mean_poi_pyears_max_overall <- psych::describe(exact_match_poi_cohort_final_4to1_pregmax$poi_pyears_max, IQR = TRUE)
mean_poi_pyears_max_overall

nm <- exact_match_poi_cohort_final_4to1_pregmax %>% filter(migrant_status == "Non-migrant")
nm_poi_pyears_max <- psych::describe(nm$poi_pyears_max, IQR = TRUE)
nm_poi_pyears_max

m <- exact_match_poi_cohort_final_4to1_pregmax %>% filter(migrant_status == "Migrant")
m_poi_pyears_max <- psych::describe(m$poi_pyears_max, IQR = TRUE)
m_poi_pyears_max

d <- exact_match_poi_cohort_final_4to1_pregmax %>% filter(migcertainty == "Definite")
d_poi_pyears_max <- psych::describe(d$poi_pyears_max, IQR = TRUE)
d_poi_pyears_max

p <- exact_match_poi_cohort_final_4to1_pregmax %>% filter(migcertainty == "Probable")
p_poi_pyears_max <- psych::describe(p$poi_pyears_max, IQR = TRUE)
p_poi_pyears_max
...
poipresc_annual_counts_final_extra_preg_min <- poipresc_annual_counts_final_extra %>% filter(pdays_ey_pregmin > 0) 
# recalculate pdays total based on counts data and join
poipresc_annual_counts_final_extra_preg_min_pdays <- poipresc_annual_counts_final_extra_preg_min %>% group_by(patid) %>% tally(pdays_ey_pregmin) %>% rename(poi_pdays_min = n )
poipresc_annual_counts_final_extra_preg_min_pyears <- poipresc_annual_counts_final_extra_preg_min %>% group_by(patid) %>% tally(pyears_ey_pregmin) %>% rename(poi_pyears_min = n )
poipresc_annual_counts_final_extra_preg_min_days_pyears <- full_join(poipresc_annual_counts_final_extra_preg_min_pdays, poipresc_annual_counts_final_extra_preg_min_pyears, by = c("patid" = "patid"))
# select patients in cohort file based on preg min assumption and add on recalculated pdays/pyears
poi_cohort_final_pregmin <- poi_cohort_final %>% filter(patid %in% poipresc_annual_counts_final_extra_preg_min$patid) %>% dplyr::select(-poi_pdays_min, -poi_pdays_min, -poi_pyears_min, -poi_pyears_min)
poi_cohort_final_pregmin <- left_join(poi_cohort_final_pregmin, poipresc_annual_counts_final_extra_preg_min_days_pyears, by = c("patid" = "patid"))

## Totals ---

## Whole cohort 
total_cohort_n <- count(poi_cohort_final_pregmin)
total_poi_pyears_min <- sum(poi_cohort_final_pregmin$poi_pyears_min)
total_wholecohort <- bind_cols(total_cohort_n, total_poi_pyears_min)
colnames(total_wholecohort) <- c("individuals", "poi_pyears_min")
total_wholecohort <- total_wholecohort %>% mutate(group = "Whole cohort") 

## By migrant status
total_mvnm_n <- poi_cohort_final_pregmin %>% group_by(migrant_status) %>% 
  count() %>% 
  rename(individuals = n) %>% 
  rename(group = migrant_status)
total_mvnm_poi_pyears_min <- poi_cohort_final_pregmin %>% group_by(migrant_status) %>%
  tally(poi_pyears_min) %>%
  rename(poi_pyears_min = n) %>% 
  rename(group = migrant_status)
total_mvnm <- full_join(total_mvnm_n, total_mvnm_poi_pyears_min,
                        by = c("group" = "group"))

## By migrant certainty
total_dp_n <- poi_cohort_final_pregmin %>% group_by(migcertainty) %>% 
  count() %>% 
  rename(individuals = n) %>% 
  rename(group = migcertainty)
total_dp_poi_pyears_min <- poi_cohort_final_pregmin %>% group_by(migcertainty) %>%
  tally(poi_pyears_min) %>%
  rename(poi_pyears_min = n) %>% 
  rename(group = migcertainty)
total_dp <- full_join(total_dp_n, total_dp_poi_pyears_min,
                      by = c("group" = "group"))

## Totals for export
totals <- full_join(total_mvnm, total_dp,
                    by = c("group" = "group", "individuals" = "individuals", "poi_pyears_min" = "poi_pyears_min")) %>% 
  full_join(total_wholecohort, by = c("group" = "group", "individuals" = "individuals", "poi_pyears_min" = "poi_pyears_min")) %>% 
  mutate(across(is.numeric, ~ round(.,0)))
total_cohort_n<- as.vector(total_cohort_n$n[1])
totals_percent <- totals %>% mutate(percent = (individuals/total_cohort_n)*100) %>% 
  mutate(percent = round(percent, 1))
...
totals_percent$individuals <- paste(totals_percent$individuals, totals_percent$percent, sep =", ")
totals_percent <- dplyr::select(totals_percent, c(group, individuals, poi_pyears_min))
write_csv(totals_percent, "filepath")
...
poi_cohort_final_pregmin_age <- poi_cohort_final_pregmin %>%
  mutate(age_cohort_entry = calc_age(dob, poi_cohort_entry))
mean_ages_overall <- psych::describe(poi_cohort_final_pregmin_age$age_cohort_entry, IQR = TRUE)
mean_ages_overall

nm <- poi_cohort_final_pregmin_age %>% filter(migrant_status == "Non-migrant")
nm_age <- psych::describe(nm$age_cohort_entry, IQR = TRUE)
nm_age

m <- poi_cohort_final_pregmin_age %>% filter(migrant_status == "Migrant")
m_age <- psych::describe(m$age_cohort_entry, IQR = TRUE)
m_age

d <- poi_cohort_final_pregmin_age %>% filter(migcertainty == "Definite")
d_age <- psych::describe(d$age_cohort_entry, IQR = TRUE)
d_age

p <- poi_cohort_final_pregmin_age %>% filter(migcertainty == "Probable")
p_age <- psych::describe(p$age_cohort_entry, IQR = TRUE)
p_age
...
## Age category - No individuals and p years per group 
poi_cohort_final_pregmin_agecat <- poi_cohort_final_pregmin_age %>%
  mutate(agecat_cohort_entry = age_cohort_entry)
poi_cohort_final_pregmin_agecat <- poi_cohort_final_pregmin_agecat %>%
  mutate(agecat_cohort_entry=replace(agecat_cohort_entry, between (agecat_cohort_entry, 15, 19), 1)) %>%
  mutate(agecat_cohort_entry=replace(agecat_cohort_entry, between (agecat_cohort_entry, 20, 24), 2)) %>%
  mutate(agecat_cohort_entry=replace(agecat_cohort_entry, between (agecat_cohort_entry, 25, 29), 3)) %>%
  mutate(agecat_cohort_entry=replace(agecat_cohort_entry, between (agecat_cohort_entry, 30, 34), 4)) %>%
  ...
  mutate(agecat_cohort_entry=replace(agecat_cohort_entry, between (agecat_cohort_entry, 40, 44), 6)) %>%
  mutate(agecat_cohort_entry=replace(agecat_cohort_entry, between (agecat_cohort_entry, 45, 49), 7)) 
poi_cohort_final_pregmin_agecat$agecat_cohort_entry <- factor(poi_cohort_final_pregmin_agecat$agecat_cohort_entry, levels = c(1,2,3,4,5,6,7),
                                                              labels = c("15-19", "20-24", "25-29", "30-34", "35-39", "40-44", "45-49"))

agecat <- poi_cohort_final_pregmin_agecat %>% 
  group_by(agecat_cohort_entry) %>% count() 
agecat <- agecat %>% mutate(percent = (n/sum(agecat$n))*100) %>%
  mutate(group = "Whole Cohort")
agecat_mvnm <- poi_cohort_final_pregmin_agecat %>% 
  group_by(agecat_cohort_entry, migrant_status) %>% count() %>% 
  rename(group = migrant_status)
agecat_m <- agecat_mvnm %>% filter(group == "Migrant")
agecat_m <- agecat_m %>% mutate(percent = (n/sum(agecat_m$n))*100)
agecat_nm <- agecat_mvnm %>% filter(group == "Non-migrant")
agecat_nm <- agecat_nm %>% mutate(percent = (n/sum(agecat_nm$n))*100)
agecat_dp <- poi_cohort_final_pregmin_agecat %>% 
  group_by(agecat_cohort_entry, migcertainty) %>% count() %>% 
  rename(group = migcertainty)
agecat_d <- agecat_dp %>% filter(group == "Definite")
agecat_d <- agecat_d %>% mutate(percent = (n/sum(agecat_d$n))*100)
...
poi_pyears_min_agecat <- aggregate(poi_pyears_min ~ agecat_cohort_entry, poi_cohort_final_pregmin_agecat, sum) %>% 
  mutate(group = "Whole Cohort")
poi_pyears_min_agecat_mvnm <- aggregate(poi_pyears_min ~ agecat_cohort_entry + migrant_status, poi_cohort_final_pregmin_agecat, sum) %>% 
  rename(group = migrant_status)
poi_pyears_min_agecat_dp <- aggregate(poi_pyears_min ~ agecat_cohort_entry + migcertainty, poi_cohort_final_pregmin_agecat, sum) %>% 
  rename(group = migcertainty)
poi_pyears_min_agecat_all <- poi_pyears_min_agecat_mvnm %>% full_join(poi_pyears_min_agecat_dp,by = c("agecat_cohort_entry"="agecat_cohort_entry",
                                                                                                      "group"="group","poi_pyears_min"="poi_pyears_min")) %>% 
  full_join(poi_pyears_min_agecat,by = c("agecat_cohort_entry"="agecat_cohort_entry",
                                         "group"="group","poi_pyears_min"="poi_pyears_min")) %>%
  rename(agecat = agecat_cohort_entry) 

poi_pyears_min_agecat_export <- full_join(poi_pyears_min_agecat_all, age_cat_all, 
                                          by = c("agecat"="agecat","group"="group")) %>% 
  mutate(across(is.numeric, ~ round(.,0))) %>% 
  dplyr::select(c(group, agecat, n_percent, poi_pyears_min)) 

write_csv(poi_pyears_min_agecat_export, "filepath")


## Ethnicity ---


poi_cohort_final_pregmin$ethnicat6 <- poi_cohort_final_pregmin$ethnicat6 %>% fct_explicit_na(na_level = "Not known")
levels(poi_cohort_final_pregmin$ethnicat6)

ethnicat6 <- poi_cohort_final_pregmin %>% 
  group_by(ethnicat6) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
ethnicat6_mvnm <- poi_cohort_final_pregmin %>% 
  group_by(ethnicat6, migrant_status) %>% count() %>% 
  rename(group = migrant_status) %>% 
  full_join(total_mvnm, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1))) %>%
  dplyr::select(c(ethnicat6, group, n, percent))
ethnicat6_dp <- poi_cohort_final_pregmin %>% 
  group_by(ethnicat6, migcertainty) %>% count() %>% 
  rename(group = migcertainty) %>% 
  full_join(total_dp, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
...
poi_pyears_min_ethnicat6 <- poi_cohort_final_pregmin %>% group_by(ethnicat6) %>% tally(poi_pyears_min) %>% 
  mutate(group = "Whole Cohort")
poi_pyears_min_ethnicat6_mvnm <- poi_cohort_final_pregmin %>% group_by(migrant_status, ethnicat6) %>% tally(poi_pyears_min) %>% 
  rename(group = migrant_status)
poi_pyears_min_ethnicat6_dp <- poi_cohort_final_pregmin %>% group_by(migcertainty, ethnicat6) %>% tally(poi_pyears_min) %>% 
  rename(group = migcertainty)
poi_pyears_min_ethnicat6_all <- poi_pyears_min_ethnicat6_mvnm %>% full_join(poi_pyears_min_ethnicat6_dp,by = c("ethnicat6"="ethnicat6",
                                                                                                               "group"="group","n"="n")) %>% 
  full_join(poi_pyears_min_ethnicat6,by = c("ethnicat6"="ethnicat6",
                                            "group"="group","n"="n")) %>%
  rename(poi_pyears_min = n) %>% 
  mutate(across(is.numeric, ~ round(.,0)))

poi_pyears_min_ethnicat6_export <- full_join(poi_pyears_min_ethnicat6_all, ethnicat6_all, 
                                             by = c("ethnicat6"="ethnicat6","group"="group")) %>% 
  relocate(group,ethnicat6,n_percent,poi_pyears_min)  %>% 
  arrange(group)

write_csv(poi_pyears_min_ethnicat6_export, "filepath")


## practice region ---

poi_cohort_final_pregmin$prac_region <- poi_cohort_final_pregmin$prac_region %>% fct_explicit_na(na_level = "Not known")
levels(poi_cohort_final_pregmin$prac_region)

prac_region <- poi_cohort_final_pregmin %>% 
  group_by(prac_region) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
prac_region_mvnm <- poi_cohort_final_pregmin %>% 
  group_by(prac_region, migrant_status) %>% count() %>% 
  rename(group = migrant_status) %>% 
  full_join(total_mvnm, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1))) %>%
  dplyr::select(c(prac_region, group, n, percent))
prac_region_dp <- poi_cohort_final_pregmin %>% 
  group_by(prac_region, migcertainty) %>% count() %>% 
  rename(group = migcertainty) %>% 
  full_join(total_dp, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
...
poi_pyears_min_prac_region <- poi_cohort_final_pregmin %>% group_by(prac_region) %>% tally(poi_pyears_min) %>% 
  mutate(group = "Whole Cohort")
poi_pyears_min_prac_region_mvnm <- poi_cohort_final_pregmin %>% group_by(migrant_status, prac_region) %>% tally(poi_pyears_min) %>% 
  rename(group = migrant_status)
poi_pyears_min_prac_region_dp <- poi_cohort_final_pregmin %>% group_by(migcertainty, prac_region) %>% tally(poi_pyears_min) %>% 
  rename(group = migcertainty)
poi_pyears_min_prac_region_all <- poi_pyears_min_prac_region_mvnm %>% full_join(poi_pyears_min_prac_region_dp,by = c("prac_region"="prac_region",
                                                                                                                     "group"="group","n"="n")) %>% 
  full_join(poi_pyears_min_prac_region,by = c("prac_region"="prac_region",
                                              "group"="group","n"="n")) %>%
  rename(poi_pyears_min = n) %>% 
  mutate(across(is.numeric, ~ round(.,0)))

poi_pyears_min_prac_region_export <- full_join(poi_pyears_min_prac_region_all, prac_region_all, 
                                               by = c("prac_region"="prac_region","group"="group")) %>% 
  relocate(group,prac_region,n_percent,poi_pyears_min)  %>% 
  arrange(group)

write_csv(poi_pyears_min_prac_region_export, "filepath")


## IMD ---

poi_cohort_final_pregmin$imd<- poi_cohort_final_pregmin$imd%>% fct_explicit_na(na_level = "Not known")
levels(poi_cohort_final_pregmin$imd)

imd<- poi_cohort_final_pregmin %>% 
  group_by(imd) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
imd_mvnm <- poi_cohort_final_pregmin %>% 
  group_by(imd, migrant_status) %>% count() %>% 
  rename(group = migrant_status) %>% 
  full_join(total_mvnm, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1))) %>%
  dplyr::select(c(imd, group, n, percent))
imd_dp <- poi_cohort_final_pregmin %>% 
  group_by(imd, migcertainty) %>% count() %>% 
  rename(group = migcertainty) %>% 
  full_join(total_dp, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
...
poi_pyears_min_imd <- poi_cohort_final_pregmin %>% group_by(imd) %>% tally(poi_pyears_min) %>% 
  mutate(group = "Whole Cohort")
poi_pyears_min_imd_mvnm <- poi_cohort_final_pregmin %>% group_by(migrant_status, imd) %>% tally(poi_pyears_min) %>% 
  rename(group = migrant_status)
poi_pyears_min_imd_dp <- poi_cohort_final_pregmin %>% group_by(migcertainty, imd) %>% tally(poi_pyears_min) %>% 
  rename(group = migcertainty)
poi_pyears_min_imd_all <- poi_pyears_min_imd_mvnm %>% full_join(poi_pyears_min_imd_dp,by = c("imd"="imd",
                                                                                             "group"="group","n"="n")) %>% 
  full_join(poi_pyears_min_imd,by = c("imd"="imd",
                                      "group"="group","n"="n")) %>%
  rename(poi_pyears_min = n) %>% 
  mutate(across(is.numeric, ~ round(.,0)))

poi_pyears_min_imd_export <- full_join(poi_pyears_min_imd_all, imd_all, 
                                       by = c("imd"="imd","group"="group")) %>% 
  relocate(group,imd,n_percent,poi_pyears_min)  %>% 
  arrange(group)

write_csv(poi_pyears_min_imd_export, "filepath")



## Year of cohort entry ---

poi_cohort_final_pregmin_yearofentry <- poi_cohort_final_pregmin %>% mutate(yearofentry = year(poi_cohort_entry))
mean_yearofentry_overall <- psych::describe(poi_cohort_final_pregmin_yearofentry$yearofentry, IQR = TRUE)
mean_yearofentry_overall

nm <- poi_cohort_final_pregmin_yearofentry %>% filter(migrant_status == "Non-migrant")
nm_yearofentry <- psych::describe(nm$yearofentry, IQR = TRUE)
nm_yearofentry

m <- poi_cohort_final_pregmin_yearofentry %>% filter(migrant_status == "Migrant")
m_yearofentry <- psych::describe(m$yearofentry, IQR = TRUE)
m_yearofentry

d <- poi_cohort_final_pregmin_yearofentry %>% filter(migcertainty == "Definite")
d_yearofentry  <- psych::describe(d$yearofentry, IQR = TRUE)
d_yearofentry 

p <- poi_cohort_final_pregmin_yearofentry %>% filter(migcertainty == "Probable")
p_yearofentry <- psych::describe(p$yearofentry, IQR = TRUE)
p_yearofentry
...
poi_cohort_final_pregmin_yearofentry$yearofentry <- factor(poi_cohort_final_pregmin_yearofentry$yearofentry,  levels = c(2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018), 
                                                           labels = c("2009", "2010", "2011", "2012", "2013", "2014", "2015", "2016", "2017", "2018"))
yearofentry <- poi_cohort_final_pregmin_yearofentry %>% 
  group_by(yearofentry) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
yearofentry_mvnm <- poi_cohort_final_pregmin_yearofentry %>% 
  group_by(yearofentry, migrant_status) %>% count() %>% 
  rename(group = migrant_status) %>% 
  full_join(total_mvnm, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1))) %>%
  dplyr::select(c(yearofentry, group, n, percent))
yearofentry_dp <- poi_cohort_final_pregmin_yearofentry %>% 
  group_by(yearofentry, migcertainty) %>% count() %>% 
  rename(group = migcertainty) %>% 
  full_join(total_dp, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
...
poi_pyears_min_yearofentry <- poi_cohort_final_pregmin_yearofentry %>% group_by(yearofentry) %>% tally(poi_pyears_min) %>% 
  mutate(group = "Whole Cohort")
poi_pyears_min_yearofentry_mvnm <- poi_cohort_final_pregmin_yearofentry %>% group_by(migrant_status, yearofentry) %>% tally(poi_pyears_min) %>% 
  rename(group = migrant_status)
poi_pyears_min_yearofentry_dp <- poi_cohort_final_pregmin_yearofentry %>% group_by(migcertainty, yearofentry) %>% tally(poi_pyears_min) %>% 
  rename(group = migcertainty)
poi_pyears_min_yearofentry_all <- poi_pyears_min_yearofentry_mvnm %>% full_join(poi_pyears_min_yearofentry_dp,by = c("yearofentry"="yearofentry",
                                                                                                                     "group"="group","n"="n")) %>% 
  full_join(poi_pyears_min_yearofentry,by = c("yearofentry"="yearofentry",
                                              "group"="group","n"="n")) %>%
  rename(poi_pyears_min = n) %>% 
  mutate(across(is.numeric, ~ round(.,0)))

poi_pyears_min_yearofentry_export <- full_join(poi_pyears_min_yearofentry_all, yearofentry_all, 
                                               by = c("group"="group","yearofentry"="yearofentry")) %>% 
  relocate(group,yearofentry,n_percent,poi_pyears_min)  %>% 
  arrange(group)

write_csv(poi_pyears_min_yearofentry_export, "filepath")


## Year ---

eventyear<- poipresc_annual_counts_final_extra_preg_min %>% 
  group_by(eventyear) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
eventyear_mvnm <- poipresc_annual_counts_final_extra_preg_min  %>% 
  group_by(eventyear, migrant_status) %>% count() %>% 
  rename(group = migrant_status) %>% 
  full_join(total_mvnm, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1))) %>%
  dplyr::select(c(eventyear, group, n, percent))
eventyear_dp <- poipresc_annual_counts_final_extra_preg_min  %>% 
  group_by(eventyear, migcertainty) %>% count() %>% 
  rename(group = migcertainty) %>% 
  full_join(total_dp, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
...
pyears_eventyear <- poipresc_annual_counts_final_extra_preg_min  %>% group_by(eventyear) %>% tally(pyears_ey_pregmin) %>% 
  mutate(group = "Whole Cohort")
pyears_eventyear_mvnm <- poipresc_annual_counts_final_extra_preg_min  %>% group_by(migrant_status, eventyear) %>% tally(pyears_ey_pregmin) %>% 
  rename(group = migrant_status)
pyears_eventyear_dp <- poipresc_annual_counts_final_extra_preg_min  %>% group_by(migcertainty, eventyear) %>% tally(pyears_ey_pregmin) %>% 
  rename(group = migcertainty)
pyears_eventyear_all <- pyears_eventyear_mvnm %>% full_join(pyears_eventyear_dp,by = c("eventyear"="eventyear",
                                                                                       "group"="group","n"="n")) %>% 
  full_join(pyears_eventyear,by = c("eventyear"="eventyear",
...
pyears_eventyear_export <- full_join(pyears_eventyear_all, eventyear_all, 
                                     by = c("eventyear"="eventyear","group"="group")) %>% 
  relocate(group,eventyear,n_percent,poi_pyears_min)  %>% 
  arrange(group)
...
mean_poi_pyears_min_overall <- psych::describe(poi_cohort_final_pregmin$poi_pyears_min, IQR = TRUE)
mean_poi_pyears_min_overall

nm <- poi_cohort_final_pregmin %>% filter(migrant_status == "Non-migrant")
nm_poi_pyears_min <- psych::describe(nm$poi_pyears_min, IQR = TRUE)
nm_poi_pyears_min

m <- poi_cohort_final_pregmin %>% filter(migrant_status == "Migrant")
m_poi_pyears_min <- psych::describe(m$poi_pyears_min, IQR = TRUE)
m_poi_pyears_min

d <- poi_cohort_final_pregmin %>% filter(migcertainty == "Definite")
d_poi_pyears_min <- psych::describe(d$poi_pyears_min, IQR = TRUE)
d_poi_pyears_min

p <- poi_cohort_final_pregmin %>% filter(migcertainty == "Probable")
p_poi_pyears_min <- psych::describe(p$poi_pyears_min, IQR = TRUE)
p_poi_pyears_min

## join export mean + median ages + sd + IQR
mean_poi_pyears_min_overall <- as.data.frame((mean_poi_pyears_min_overall))
mean_poi_pyears_min_overall <- dplyr::select(mean_poi_pyears_min_overall, c(mean , sd, median, IQR)) %>% mutate(group = "Whole cohort") 
nm_poi_pyears_min <- as.data.frame((nm_poi_pyears_min))
nm_poi_pyears_min <- dplyr::select(nm_poi_pyears_min, c(mean , sd, median, IQR)) %>% mutate(group = "Non-migrant") 
m_poi_pyears_min <- as.data.frame((m_poi_pyears_min))
m_poi_pyears_min <- dplyr::select(m_poi_pyears_min, c(mean , sd, median, IQR)) %>% mutate(group = "Migrant") 
d_poi_pyears_min <- as.data.frame((d_poi_pyears_min))
d_poi_pyears_min <- dplyr::select(d_poi_pyears_min, c(mean , sd, median, IQR)) %>% mutate(group = "Definite") 
p_poi_pyears_min <- as.data.frame((p_poi_pyears_min))
p_poi_pyears_min <- dplyr::select(p_poi_pyears_min, c(mean , sd, median, IQR)) %>% mutate(group = "Probable") 
all_mean_poi_pyears_min <- full_join(mean_poi_pyears_min_overall, nm_poi_pyears_min,
                                     by = c( "group" = "group", "mean" = "mean", "sd" = "sd" , "median" = "median", "IQR" = "IQR")) %>% 
  full_join(m_poi_pyears_min, by = c( "group" = "group", "mean" = "mean", "sd" = "sd" , "median" = "median", "IQR" = "IQR") ) %>%
  full_join(d_poi_pyears_min, by = c( "group" = "group", "mean" = "mean", "sd" = "sd", "median" = "median",  "IQR" = "IQR" ) ) %>%
  full_join(p_poi_pyears_min, by = c( "group" = "group", "mean" = "mean", "sd" = "sd" , "median" = "median", "IQR" = "IQR") )  %>%
  mutate(across(is.numeric, ~ round(.,2)))
all_mean_poi_pyears_min$mean_sd <- paste(all_mean_poi_pyears_min$mean, all_mean_poi_pyears_min$sd, sep =", ")
all_mean_poi_pyears_min$median_iqr <- paste(all_mean_poi_pyears_min$median, all_mean_poi_pyears_min$IQR, sep =", ")
all_mean_poi_pyears_min <- dplyr::select(all_mean_poi_pyears_min, c(group, mean_sd, median_iqr))
write_csv(all_mean_poi_pyears_min,"filepath" )
...
poipresc_count_overall_whole_cohort <- poipresc_annual_counts_final_extra_preg_max %>% tally(poipresc_n_pregmax) %>% rename(n_events = n)
poipresc_summary_overall_whole_cohort <- poipresc_annual_counts_final_extra_preg_max %>% 
  summarise(mean = mean(poipresc_n_pregmax), sd = sd(poipresc_n_pregmax), median = median(poipresc_n_pregmax), iqr = IQR(poipresc_n_pregmax), 
            min = min(poipresc_n_pregmax), max = max(poipresc_n_pregmax))
poipresc_count_summary_overall_whole_cohort <- merge(poipresc_count_overall_whole_cohort, poipresc_summary_overall_whole_cohort)
poipresc_count_summary_overall_whole_cohort

## Overall by migrant status
poipresc_count_overall_migrant_status <- poipresc_annual_counts_final_extra_preg_max %>% group_by(migrant_status) %>%
  tally(poipresc_n_pregmax) %>% rename(n_events = n)
poipresc_summary_overall_migrant_status <- poipresc_annual_counts_final_extra_preg_max %>% group_by(migrant_status) %>%
  summarise(mean = mean(poipresc_n_pregmax), sd = sd(poipresc_n_pregmax), median = median(poipresc_n_pregmax), iqr = IQR(poipresc_n_pregmax), 
            min = min(poipresc_n_pregmax), max = max(poipresc_n_pregmax))
poipresc_count_summary_overall_migrant_status <- left_join(poipresc_count_overall_migrant_status, poipresc_summary_overall_migrant_status,
                                                             by = c("migrant_status"="migrant_status" ))

## Overall by migcertainty
poipresc_count_overall_migcertainty <- poipresc_annual_counts_final_extra_preg_max %>% group_by(migcertainty) %>%
  tally(poipresc_n_pregmax) %>% rename(n_events = n)
poipresc_summary_overall_migcertainty <- poipresc_annual_counts_final_extra_preg_max %>% group_by(migcertainty) %>%
  summarise(mean = mean(poipresc_n_pregmax), sd = sd(poipresc_n_pregmax), median = median(poipresc_n_pregmax), iqr = IQR(poipresc_n_pregmax), 
            min = min(poipresc_n_pregmax), max = max(poipresc_n_pregmax))
poipresc_count_summary_overall_migcertainty <- left_join(poipresc_count_overall_migcertainty, poipresc_summary_overall_migcertainty,
                                                           by = c("migcertainty"="migcertainty" ))

## Join for overall whole cohort + by migrant status
poipresc_count_summary_overall_final <- full_join(poipresc_count_summary_overall_whole_cohort, poipresc_count_summary_overall_migrant_status, 
                                                    by = c("n_events"="n_events", "mean" = "mean",
                                                           "sd"="sd", "median"="median", "iqr"="iqr", "min"="min", "max"="max")) %>% 
  full_join(poipresc_count_summary_overall_migcertainty, by = c("n_events"="n_events", "mean" = "mean",
                                                                  "sd"="sd", "median"="median", "iqr"="iqr", "min"="min", "max"="max",
                                                                  "migrant_status" = "migcertainty")) 
poipresc_count_summary_overall_final$migrant_status <- fct_explicit_na(poipresc_count_summary_overall_final$migrant_status, na_level = "whole_cohort")
poipresc_count_summary_overall_final <- poipresc_count_summary_overall_final %>% relocate(migrant_status)
poipresc_count_summary_overall_final

write_csv(poipresc_count_summary_overall_final, "filepath" )

## Annual - whole cohort 
poipresc_count_annual_whole_cohort <- poipresc_annual_counts_final_extra_preg_max %>% group_by(eventyear) %>%
  tally(poipresc_n_pregmax) %>% rename(n_events = n)
poipresc_summary_annual_whole_cohort <- poipresc_annual_counts_final_extra_preg_max %>% group_by(eventyear) %>%
  summarise(mean = mean(poipresc_n_pregmax), sd = sd(poipresc_n_pregmax), median = median(poipresc_n_pregmax), iqr = IQR(poipresc_n_pregmax), 
            min = min(poipresc_n_pregmax), max = max(poipresc_n_pregmax))
poipresc_count_summary_annual_whole_cohort <- left_join(poipresc_count_annual_whole_cohort, poipresc_summary_annual_whole_cohort,
                                                          by = c("eventyear"="eventyear"))


## Annual - by migrant status
poipresc_count_annual_migrant_status <- poipresc_annual_counts_final_extra_preg_max %>% group_by(migrant_status, eventyear) %>%
  tally(poipresc_n_pregmax) %>% rename(n_events = n)
poipresc_summary_annual_migrant_status <- poipresc_annual_counts_final_extra_preg_max %>% group_by(migrant_status, eventyear) %>%
  summarise(mean = mean(poipresc_n_pregmax), sd = sd(poipresc_n_pregmax), median = median(poipresc_n_pregmax), iqr = IQR(poipresc_n_pregmax), 
            min = min(poipresc_n_pregmax), max = max(poipresc_n_pregmax))
poipresc_count_summary_annual_migrant_status <- left_join(poipresc_count_annual_migrant_status, poipresc_summary_annual_migrant_status,
                                                            by = c("migrant_status"="migrant_status" , "eventyear"="eventyear"))

## Annual - by migcertainty 
poipresc_count_annual_migcertainty <- poipresc_annual_counts_final_extra_preg_max %>% group_by(migcertainty, eventyear) %>%
  tally(poipresc_n_pregmax) %>% rename(n_events = n)
poipresc_summary_annual_migcertainty <- poipresc_annual_counts_final_extra_preg_max %>% group_by(migcertainty, eventyear) %>%
  summarise(mean = mean(poipresc_n_pregmax), sd = sd(poipresc_n_pregmax), median = median(poipresc_n_pregmax), iqr = IQR(poipresc_n_pregmax), 
            min = min(poipresc_n_pregmax), max = max(poipresc_n_pregmax))
poipresc_count_summary_annual_migcertainty <- left_join(poipresc_count_annual_migcertainty, poipresc_summary_annual_migcertainty,
                                                          by = c("migcertainty"="migcertainty" , "eventyear"="eventyear"))



## Join to give annual for whole cohort + migrant status
poipresc_count_summary_annual_final <- full_join(poipresc_count_summary_annual_whole_cohort, poipresc_count_summary_annual_migrant_status, 
                                                   by = c("eventyear" = "eventyear", "n_events"="n_events", "mean" = "mean",
                                                          "sd"="sd", "median"="median", "iqr"="iqr", "min"="min", "max"="max")) 
poipresc_count_summary_annual_final <-   full_join(poipresc_count_summary_annual_final, poipresc_count_summary_annual_migcertainty,
                                                     by = c("eventyear" = "eventyear", "n_events"="n_events", "mean" = "mean",
                                                            "sd"="sd", "median"="median", "iqr"="iqr", "min"="min", "max"="max",
                                                            "migrant_status" = "migcertainty"))

poipresc_count_summary_annual_final$migrant_status <- fct_explicit_na(poipresc_count_summary_annual_final$migrant_status, na_level = "whole_cohort") 
poipresc_count_summary_annual_final <- poipresc_count_summary_annual_final %>% 
  relocate(migrant_status)


## Export poipresc summary measures
write_csv(poipresc_count_summary_annual_final, "filepath")
...
pyears_overall <- sum(poipresc_annual_counts_final_extra_preg_max$pyears_ey_pregmax)
poipresccount <- sum(poipresc_annual_counts_final_extra_preg_max$poipresc_n_pregmax)
rate_output <- pois.exact(poipresccount, pyears_overall, conf.level=0.95)
...
pyears_mvnm_overall <- aggregate(pyears_ey_pregmax~ migrant_status, poipresc_annual_counts_final_extra_preg_max, sum) 
pyears_mvnm_overall <- pyears_mvnm_overall %>% mutate(across(is.numeric, ~ round(.,)))
poipresccount_mvnm <- aggregate(poipresc_n_pregmax ~ migrant_status, poipresc_annual_counts_final_extra_preg_max, sum) 
IR_mvnm <- inner_join(poipresccount_mvnm,pyears_mvnm_overall, by= c("migrant_status" = "migrant_status"))
IR_mvnm_output <- pois.exact(IR_mvnm$poipresc_n_pregmax, IR_mvnm$pyears_ey_pregmax, conf.level=0.95)
...
x <- glm.nb(poipresc_n_pregmax ~ as.factor(migrant_status) + offset(log(pyears_ey_pregmax)), data = poipresc_annual_counts_final_extra_preg_max)
names <- names(coef(x))
estimate<- exp(coef(x))
...
a<- poipresc_annual_counts_final_extra_preg_max %>% group_by(patid, eventyear_agecat) %>% 
  tally(poipresc_n_pregmax)
b<- poipresc_annual_counts_final_extra_preg_max %>% group_by(patid, eventyear_agecat) %>% 
  tally(pyears_ey_pregmax)
c<- full_join(a,b, by = c("patid"="patid", "eventyear_agecat" = "eventyear_agecat"))
d<- left_join(c, poi_cohort_final  , by = c("patid" = "patid")) %>% 
  rename(poipresc_n_pregmax = n.x) %>% 
  rename(pyears_agecat = n.y)


# IR by age cat - whole cohort
pyears_agecat_overall <- aggregate(pyears_ey_pregmax ~ eventyear_agecat, poipresc_annual_counts_final_extra_preg_max, sum) 
pyears_agecat_overall <- pyears_agecat_overall %>% mutate(across(is.numeric, ~ round(.,)))
poipresccount_agecat <- aggregate(poipresc_n_pregmax ~ eventyear_agecat, poipresc_annual_counts_final_extra_preg_max, sum) 
IR_agecat <- inner_join(poipresccount_agecat,pyears_agecat_overall, by= c("eventyear_agecat" = "eventyear_agecat"))
IR_agecat_output <- pois.exact(IR_agecat$poipresc_n_pregmax, IR_agecat$pyears_ey_pregmax, conf.level=0.95)
...
x <- glm.nb(poipresc_n_pregmax ~ as.factor(eventyear_agecat) + offset(log(pyears_agecat)), data = d)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
pyears_prac_region_overall <- aggregate(pyears_ey_pregmax ~ prac_region, poipresc_annual_counts_final_extra_preg_max, sum) 
pyears_prac_region_overall <- pyears_prac_region_overall %>% mutate(across(is.numeric, ~ round(.,)))
poipresccount_prac_region <- aggregate(poipresc_n_pregmax ~ prac_region, poipresc_annual_counts_final_extra_preg_max, sum) 
IR_prac_region <- inner_join(poipresccount_prac_region,pyears_prac_region_overall, by= c("prac_region" = "prac_region"))
IR_prac_region_output <- pois.exact(IR_prac_region$poipresc_n_pregmax, IR_prac_region$pyears_ey_pregmax, conf.level=0.95)
...
x <- glm.nb(poipresc_n_pregmax ~ as.factor(prac_region) + offset(log(pyears_ey_pregmax)), data = poipresc_annual_counts_final_extra_preg_max)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
# IR by imd  - whole cohort
pyears_imd_overall <- aggregate(pyears_ey_pregmax ~ imd, poipresc_annual_counts_final_extra_preg_max, sum) 
pyears_imd_overall <- pyears_imd_overall %>% mutate(across(is.numeric, ~ round(.,)))
poipresccount_imd <- aggregate(poipresc_n_pregmax ~ imd, poipresc_annual_counts_final_extra_preg_max, sum) 
IR_imd <- inner_join(poipresccount_imd,pyears_imd_overall, by= c("imd" = "imd"))
IR_imd_output <- pois.exact(IR_imd$poipresc_n_pregmax, IR_imd$pyears_ey_pregmax, conf.level=0.95)
...
x <- glm.nb(poipresc_n_pregmax ~ as.factor(imd) + offset(log(pyears_ey_pregmax)), data = poipresc_annual_counts_final_extra_preg_max)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
pyears_annual <- aggregate(pyears_ey_pregmax ~ eventyear, poipresc_annual_counts_final_extra_preg_max, sum) 
pyears_annual <- pyears_annual %>% mutate(across(is.numeric, ~ round(.,)))
poipresc_count_annual <- aggregate(poipresc_n_pregmax ~ eventyear, poipresc_annual_counts_final_extra_preg_max, sum) 
IR_annual <- inner_join(poipresc_count_annual,pyears_annual, by= c("eventyear" = "eventyear"))
IR_annual_output <- pois.exact(IR_annual$poipresc_n_pregmax, IR_annual$pyears_ey_pregmax, conf.level=0.95)
...
## Generate IRR via negative binomial regression univariabe for  year 
x <- glm.nb(poipresc_n_pregmax ~ as.factor(eventyear) + offset(log(pyears_ey_pregmax)), data = poipresc_annual_counts_final_extra_preg_max)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
x <- glm.nb(poipresc_n_pregmax ~ as.factor(migrant_status) + as.factor(eventyear) + as.factor(eventyear_agecat) + as.factor(imd) + as.factor(prac_region) + offset(log(pyears_ey_pregmax)) , 
            data = poipresc_annual_counts_final_extra_preg_max)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
poi_glm_adj_unadj <- glm_adj_unadj
save(poi_glm_adj_unadj, file = "filepath")
...
pyears_dp_overall <- aggregate(pyears_ey_pregmax ~ migcertainty, poipresc_annual_counts_final_extra_preg_max, sum) 
pyears_dp_overall <- pyears_dp_overall %>% mutate(across(is.numeric, ~ round(.,)))
poipresccount_dp <- aggregate(poipresc_n_pregmax ~ migcertainty, poipresc_annual_counts_final_extra_preg_max, sum) 
IR_dp <- inner_join(poipresccount_dp,pyears_dp_overall, by= c("migcertainty" = "migcertainty"))
IR_dp_output <- pois.exact(IR_dp$poipresc_n_pregmax, IR_dp$pyears_ey_pregmax, conf.level=0.95)
...
##  generate IRR by negative binomial regression
poipresc_annual_counts_final_extra_preg_max$migcertainty <- relevel(poipresc_annual_counts_final_extra_preg_max$migcertainty, "Non-migrant")
x <- glm.nb(poipresc_n_pregmax ~ as.factor(migcertainty) + offset(log(pyears_ey_pregmax)), data = poipresc_annual_counts_final_extra_preg_max)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
## Generate IRR via negative binomial regression
poipresc_annual_counts_final_extra_preg_max$migcertainty <- relevel(poipresc_annual_counts_final_extra_preg_max$migcertainty, "Non-migrant")
x <- glm.nb(poipresc_n_pregmax ~ as.factor(migcertainty) + as.factor(eventyear) + as.factor(eventyear_agecat) +  as.factor(imd) + as.factor(prac_region) + offset(log(pyears_ey_pregmax)), 
            data = poipresc_annual_counts_final_extra_preg_max)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
...
glm_dp_adj_unadj <- full_join(glm_dp, glm_all_dp_fp, by = c("names" = "names", "estimate" = "estimate",
                                                            "lower" = "lower", "upper"= "upper",
                                                            "p" = "p", "ci" = "ci", "irr_ci" = "irr_ci"))
poi_glm_dp_adj_unadj <- glm_dp_adj_unadj
save(poi_glm_dp_adj_unadj, file ="filepath")
...
white_british <- poipresc_annual_counts_final_extra_preg_max %>% filter(ethnicat6 == "White British")
...
poipresccount_mvnm_white_british<- aggregate(poipresc_n_pregmax ~ migrant_status, white_british, sum) 
IR_mvnm_white_british <- inner_join(poipresccount_mvnm_white_british,pyears_mvnm_white_british, by= c("migrant_status" = "migrant_status"))
IR_mvnm_white_british_output <- pois.exact(IR_mvnm_white_british$poipresc_n_pregmax, IR_mvnm_white_british$pyears_ey_pregmax, conf.level=0.95)
...
x <- glm.nb(poipresc_n_pregmax ~ as.factor(migrant_status) + offset(log(pyears_ey_pregmax)), data = white_british)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
white_nonbritish <- poipresc_annual_counts_final_extra_preg_max %>% filter(ethnicat6 == "White Non-British")
...
poipresccount_mvnm_white_nonbritish<- aggregate(poipresc_n_pregmax ~ migrant_status, white_nonbritish, sum) 
IR_mvnm_white_nonbritish <- inner_join(poipresccount_mvnm_white_nonbritish,pyears_mvnm_white_nonbritish, by= c("migrant_status" = "migrant_status"))
IR_mvnm_white_nonbritish_output <- pois.exact(IR_mvnm_white_nonbritish$poipresc_n_pregmax, IR_mvnm_white_nonbritish$pyears_ey_pregmax, conf.level=0.95)
...
x <- glm.nb(poipresc_n_pregmax ~ as.factor(migrant_status) + offset(log(pyears_ey_pregmax)), data = white_nonbritish)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
mixed <- poipresc_annual_counts_final_extra_preg_max %>% filter(ethnicat6 == "Mixed")
...
poipresccount_mvnm_mixed<- aggregate(poipresc_n_pregmax ~ migrant_status, mixed, sum) 
IR_mvnm_mixed <- inner_join(poipresccount_mvnm_mixed,pyears_mvnm_mixed, by= c("migrant_status" = "migrant_status"))
IR_mvnm_mixed_output <- pois.exact(IR_mvnm_mixed$poipresc_n_pregmax, IR_mvnm_mixed$pyears_ey_pregmax, conf.level=0.95)
...
x <- glm.nb(poipresc_n_pregmax ~ as.factor(migrant_status) + offset(log(pyears_ey_pregmax)), data = mixed)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
asian <- poipresc_annual_counts_final_extra_preg_max %>% filter(ethnicat6 == "Asian/Asian British")
...
poipresccount_mvnm_asian<- aggregate(poipresc_n_pregmax ~ migrant_status, asian, sum) 
IR_mvnm_asian <- inner_join(poipresccount_mvnm_asian,pyears_mvnm_asian, by= c("migrant_status" = "migrant_status"))
IR_mvnm_asian_output <- pois.exact(IR_mvnm_asian$poipresc_n_pregmax, IR_mvnm_asian$pyears_ey_pregmax, conf.level=0.95)
...
x <- glm.nb(poipresc_n_pregmax ~ as.factor(migrant_status) + offset(log(pyears_ey_pregmax)), data = asian)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
black <- poipresc_annual_counts_final_extra_preg_max %>% filter(ethnicat6 == "Black/Black British")
...
poipresccount_mvnm_black<- aggregate(poipresc_n_pregmax ~ migrant_status, black, sum) 
IR_mvnm_black <- inner_join(poipresccount_mvnm_black,pyears_mvnm_black, by= c("migrant_status" = "migrant_status"))
IR_mvnm_black_output <- pois.exact(IR_mvnm_black$poipresc_n_pregmax, IR_mvnm_black$pyears_ey_pregmax, conf.level=0.95)
...
x <- glm.nb(poipresc_n_pregmax ~ as.factor(migrant_status) + offset(log(pyears_ey_pregmax)), data = black)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
other <- poipresc_annual_counts_final_extra_preg_max %>% filter(ethnicat6 == "Other ethnic group")
...
poipresccount_mvnm_other<- aggregate(poipresc_n_pregmax ~ migrant_status, other, sum) 
IR_mvnm_other <- inner_join(poipresccount_mvnm_other,pyears_mvnm_other, by= c("migrant_status" = "migrant_status"))
IR_mvnm_other_output <- pois.exact(IR_mvnm_other$poipresc_n_pregmax, IR_mvnm_other$pyears_ey_pregmax, conf.level=0.95)
...
x <- glm.nb(poipresc_n_pregmax ~ as.factor(migrant_status) + offset(log(pyears_ey_pregmax)), data = other)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
poi_glm_adj_unadj_ethnicity <- glm_adj_unadj
save(poi_glm_adj_unadj_ethnicity, file = "filepath")
poi_glm_adj_unadj_ethnicity_deduped <- poi_glm_adj_unadj_ethnicity %>% dplyr::select(-p) %>% distinct()
...
tabletext <- cbind(c("Non-migrants", "White British ethnicity - Migrants (unadjusted)", "White British ethnicity - Migrants (fully adjusted)",
                   ...
                   c(poi_glm_adj_unadj_ethnicity_deduped$estimate),
                   c(poi_glm_adj_unadj_ethnicity_deduped$ci))
dev.new()
png(file = "filepath", width = 800, height = 700)
irr_migrant_status_ethnicity <- forestplot(tabletext, mean = poi_glm_adj_unadj_ethnicity_deduped$estimate, lower=poi_glm_adj_unadj_ethnicity_deduped$lower, upper=poi_glm_adj_unadj_ethnicity_deduped$upper,
                                           graph.pos = (ncol(tabletext)-1),
                                           hrzl_lines = TRUE, xlab = "IRR", zero = 1, 
                                           col = fpColors(box="mediumblue", lines = "black", zero="black", hrz_lines = "black"),
                                           xticks = 0:2,
...
# IR by migrant status - whole cohort
pyears_mvnm_overall <- aggregate(pyears_ey_pregmin~ migrant_status, poipresc_annual_counts_final_extra_preg_min, sum) 
pyears_mvnm_overall <- pyears_mvnm_overall %>% mutate(across(is.numeric, ~ round(.,)))
poipresccount_mvnm <- aggregate(poipresc_n_pregmin ~ migrant_status, poipresc_annual_counts_final_extra_preg_min, sum) 
IR_mvnm <- inner_join(poipresccount_mvnm,pyears_mvnm_overall, by= c("migrant_status" = "migrant_status"))
IR_mvnm_output <- pois.exact(IR_mvnm$poipresc_n_pregmin, IR_mvnm$pyears_ey_pregmin, conf.level=0.95)
IR_mvnm_output <- IR_mvnm_output %>%
  mutate(incidence_rate100000=rate*100000)
IR_mvnm_output <- IR_mvnm_output %>%
  mutate(lower100000=lower*100000)
...
x <- glm.nb(poipresc_n_pregmin ~ as.factor(migrant_status) + offset(log(pyears_ey_pregmin)), data = poipresc_annual_counts_final_extra_preg_min)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
## Generate IRR via negative binomial regression
x <- glm.nb(poipresc_n_pregmin ~ as.factor(migrant_status) + as.factor(eventyear) + as.factor(eventyear_agecat) + as.factor(imd) + as.factor(prac_region) + offset(log(pyears_ey_pregmin)) , 
            data = poipresc_annual_counts_final_extra_preg_min)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
glm_adj_unadj <- full_join(glm_mig, glm_all_mvnm_fp, by = c("names" = "names", "estimate" = "estimate",
                                                            "lower" = "lower", "upper"= "upper",
                                                            "p" = "p", "ci" = "ci", "irr_ci" = "irr_ci"))
poi_glm_adj_unadj_pregmin <- glm_adj_unadj
save(poi_glm_adj_unadj_pregmin, file = "results/contraception_prescribing/poi/preg_min/poi_glm_adj_unadj_pregmin.Rdata")
...
exact_match_poipresc_annual_counts_final_4to1$prac_region <- relevel(exact_match_poipresc_annual_counts_final_4to1$prac_region, "London")
exact_match_poipresc_annual_counts_final_extra_4to1$prac_region <- relevel(exact_match_poipresc_annual_counts_final_extra_4to1$prac_region, "London")


# Using preg max assumption

exact_match_poipresc_annual_counts_final_extra_4to1_preg_max <- exact_match_poipresc_annual_counts_final_extra_4to1 %>% filter(pyears_ey_pregmax > 0)


# IR by migrant status - whole cohort
pyears_mvnm_overall <- aggregate(pyears_ey_pregmax ~ migrant_status, exact_match_poipresc_annual_counts_final_extra_4to1_preg_max, sum) 
pyears_mvnm_overall <- pyears_mvnm_overall %>% mutate(across(is.numeric, ~ round(.,0)))
poipresccount_mvnm <- aggregate(poipresc_n_pregmax ~ migrant_status, exact_match_poipresc_annual_counts_final_extra_4to1_preg_max, sum) 
IR_mvnm <- inner_join(poipresccount_mvnm,pyears_mvnm_overall, by= c("migrant_status" = "migrant_status"))
IR_mvnm_output <- pois.exact(IR_mvnm$poipresc_n_pregmax, IR_mvnm$pyears_ey_pregmax, conf.level=0.95)
IR_mvnm_output <- IR_mvnm_output %>%
  mutate(incidence_rate100000=rate*100000)
...
x <- glm.nb(poipresc_n_pregmax ~ as.factor(migrant_status) + offset(log(pyears_ey_pregmax)), data = exact_match_poipresc_annual_counts_final_extra_4to1_preg_max)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
## Generate IRR via negative binomial regression
x <- glm.nb(poipresc_n_pregmax ~ as.factor(migrant_status) + as.factor(eventyear) + as.factor(eventyear_agecat) + as.factor(imd) + offset(log(pyears_ey_pregmax)) , 
            data = exact_match_poipresc_annual_counts_final_extra_4to1_preg_max)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
poi_glm_adj_unadj_em <- glm_adj_unadj
save(poi_glm_adj_unadj_em, file = "filepath")
...
unadj_irr_migrant_status <- forestplot(tabletext, mean = glm_adj_unadj$estimate, lower=glm_adj_unadj$lower, upper=glm_adj_unadj$upper,
                                       ...
                                       ci.vertices = TRUE,
                                       boxsize = 0.05,
                                       title = "poipresc (Exact Matched Cohort)",
                                       graphwidth = unit(80, 'mm'))
dev.off()
