...
## cuiud Analysis for Neha's contraception prescribing chapter in females of reproductive age (15-49yo) 
...
cuiudpresc_annual_counts_final$prac_region <- relevel(cuiudpresc_annual_counts_final$prac_region, "London")
cuiudpresc_annual_counts_final_extra$prac_region <- relevel(cuiudpresc_annual_counts_final_extra$prac_region, "London")
...
cuiudpresc_annual_counts_final_extra_preg_max <- cuiudpresc_annual_counts_final_extra %>% filter(pdays_ey_pregmax > 0) 
# recalculate pdays total based on counts data and join
cuiudpresc_annual_counts_final_extra_preg_max_pdays <- cuiudpresc_annual_counts_final_extra_preg_max %>% group_by(patid) %>% tally(pdays_ey_pregmax) %>% rename(cuiud_pdays_max = n )
cuiudpresc_annual_counts_final_extra_preg_max_pyears <- cuiudpresc_annual_counts_final_extra_preg_max %>% group_by(patid) %>% tally(pyears_ey_pregmax) %>% rename(cuiud_pyears_max = n )
cuiudpresc_annual_counts_final_extra_preg_max_days_pyears <- full_join(cuiudpresc_annual_counts_final_extra_preg_max_pdays, cuiudpresc_annual_counts_final_extra_preg_max_pyears, by = c("patid" = "patid"))
# select patients in cohort file based on preg max assumption and add on recalculated pdays/pyears
cuiud_cohort_final_pregmax <- cuiud_cohort_final %>% filter(patid %in% cuiudpresc_annual_counts_final_extra_preg_max$patid) %>% dplyr::select(-cuiud_pdays_max, -cuiud_pdays_min, -cuiud_pyears_max, -cuiud_pyears_min)
cuiud_cohort_final_pregmax <- left_join(cuiud_cohort_final_pregmax, cuiudpresc_annual_counts_final_extra_preg_max_days_pyears, by = c("patid" = "patid"))

## Totals ---

## Whole cohort 
total_cohort_n <- count(cuiud_cohort_final_pregmax)
total_cuiud_pyears_max <- sum(cuiud_cohort_final_pregmax$cuiud_pyears_max)
total_wholecohort <- bind_cols(total_cohort_n, total_cuiud_pyears_max)
colnames(total_wholecohort) <- c("individuals", "cuiud_pyears_max")
total_wholecohort <- total_wholecohort %>% mutate(group = "Whole cohort") 

## By migrant status
total_mvnm_n <- cuiud_cohort_final_pregmax %>% group_by(migrant_status) %>% 
  count() %>% 
  rename(individuals = n) %>% 
  rename(group = migrant_status)
total_mvnm_cuiud_pyears_max <- cuiud_cohort_final_pregmax %>% group_by(migrant_status) %>%
  tally(cuiud_pyears_max) %>%
  rename(cuiud_pyears_max = n) %>% 
  rename(group = migrant_status)
total_mvnm <- full_join(total_mvnm_n, total_mvnm_cuiud_pyears_max,
                        by = c("group" = "group"))

## By migrant certainty
total_dp_n <- cuiud_cohort_final_pregmax %>% group_by(migcertainty) %>% 
  count() %>% 
  rename(individuals = n) %>% 
  rename(group = migcertainty)
total_dp_cuiud_pyears_max <- cuiud_cohort_final_pregmax %>% group_by(migcertainty) %>%
  tally(cuiud_pyears_max) %>%
  rename(cuiud_pyears_max = n) %>% 
  rename(group = migcertainty)
total_dp <- full_join(total_dp_n, total_dp_cuiud_pyears_max,
                      by = c("group" = "group"))

## Totals for export
totals <- full_join(total_mvnm, total_dp,
                    by = c("group" = "group", "individuals" = "individuals", "cuiud_pyears_max" = "cuiud_pyears_max")) %>% 
  full_join(total_wholecohort, by = c("group" = "group", "individuals" = "individuals", "cuiud_pyears_max" = "cuiud_pyears_max")) %>% 
...
totals_percent <- dplyr::select(totals_percent, c(group, individuals, cuiud_pyears_max))
...
cuiud_cohort_final_pregmax_age <- cuiud_cohort_final_pregmax %>%
  mutate(age_cohort_entry = calc_age(dob, cuiud_cohort_entry))
mean_ages_overall <- psych::describe(cuiud_cohort_final_pregmax_age$age_cohort_entry, IQR = TRUE)
mean_ages_overall

nm <- cuiud_cohort_final_pregmax_age %>% filter(migrant_status == "Non-migrant")
nm_age <- psych::describe(nm$age_cohort_entry, IQR = TRUE)
nm_age

m <- cuiud_cohort_final_pregmax_age %>% filter(migrant_status == "Migrant")
m_age <- psych::describe(m$age_cohort_entry, IQR = TRUE)
m_age

d <- cuiud_cohort_final_pregmax_age %>% filter(migcertainty == "Definite")
d_age <- psych::describe(d$age_cohort_entry, IQR = TRUE)
d_age

p <- cuiud_cohort_final_pregmax_age %>% filter(migcertainty == "Probable")
...
cuiud_cohort_final_pregmax_agecat <- cuiud_cohort_final_pregmax_age %>%
  mutate(agecat_cohort_entry = age_cohort_entry)
cuiud_cohort_final_pregmax_agecat <- cuiud_cohort_final_pregmax_agecat %>%
...
cuiud_cohort_final_pregmax_agecat$agecat_cohort_entry <- factor(cuiud_cohort_final_pregmax_agecat$agecat_cohort_entry, levels = c(1,2,3,4,5,6,7),
                                                              labels = c("15-19", "20-24", "25-29", "30-34", "35-39", "40-44", "45-49"))

agecat <- cuiud_cohort_final_pregmax_agecat %>% 
  group_by(agecat_cohort_entry) %>% count() 
agecat <- agecat %>% mutate(percent = (n/sum(agecat$n))*100) %>%
  mutate(group = "Whole Cohort")
agecat_mvnm <- cuiud_cohort_final_pregmax_agecat %>% 
...
agecat_dp <- cuiud_cohort_final_pregmax_agecat %>% 
...
cuiud_pyears_max_agecat <- aggregate(cuiud_pyears_max ~ agecat_cohort_entry, cuiud_cohort_final_pregmax_agecat, sum) %>% 
  mutate(group = "Whole Cohort")
cuiud_pyears_max_agecat_mvnm <- aggregate(cuiud_pyears_max ~ agecat_cohort_entry + migrant_status, cuiud_cohort_final_pregmax_agecat, sum) %>% 
  rename(group = migrant_status)
cuiud_pyears_max_agecat_dp <- aggregate(cuiud_pyears_max ~ agecat_cohort_entry + migcertainty, cuiud_cohort_final_pregmax_agecat, sum) %>% 
  rename(group = migcertainty)
cuiud_pyears_max_agecat_all <- cuiud_pyears_max_agecat_mvnm %>% full_join(cuiud_pyears_max_agecat_dp,by = c("agecat_cohort_entry"="agecat_cohort_entry",
                                                                                                      "group"="group","cuiud_pyears_max"="cuiud_pyears_max")) %>% 
  full_join(cuiud_pyears_max_agecat,by = c("agecat_cohort_entry"="agecat_cohort_entry",
                                         "group"="group","cuiud_pyears_max"="cuiud_pyears_max")) %>%
  rename(agecat = agecat_cohort_entry) 

cuiud_pyears_max_agecat_export <- full_join(cuiud_pyears_max_agecat_all, age_cat_all, 
                                          by = c("agecat"="agecat","group"="group")) %>% 
  mutate(across(is.numeric, ~ round(.,0))) %>% 
  dplyr::select(c(group, agecat, n_percent, cuiud_pyears_max)) 

write_csv(cuiud_pyears_max_agecat_export, "filepath")


## Ethnicity ---


cuiud_cohort_final_pregmax$ethnicat6 <- cuiud_cohort_final_pregmax$ethnicat6 %>% fct_explicit_na(na_level = "Not known")
levels(cuiud_cohort_final_pregmax$ethnicat6)

ethnicat6 <- cuiud_cohort_final_pregmax %>% 
  group_by(ethnicat6) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
ethnicat6_mvnm <- cuiud_cohort_final_pregmax %>% 
...
ethnicat6_dp <- cuiud_cohort_final_pregmax %>% 
...
cuiud_pyears_max_ethnicat6 <- cuiud_cohort_final_pregmax %>% group_by(ethnicat6) %>% tally(cuiud_pyears_max) %>% 
  mutate(group = "Whole Cohort")
cuiud_pyears_max_ethnicat6_mvnm <- cuiud_cohort_final_pregmax %>% group_by(migrant_status, ethnicat6) %>% tally(cuiud_pyears_max) %>% 
  rename(group = migrant_status)
cuiud_pyears_max_ethnicat6_dp <- cuiud_cohort_final_pregmax %>% group_by(migcertainty, ethnicat6) %>% tally(cuiud_pyears_max) %>% 
  rename(group = migcertainty)
cuiud_pyears_max_ethnicat6_all <- cuiud_pyears_max_ethnicat6_mvnm %>% full_join(cuiud_pyears_max_ethnicat6_dp,by = c("ethnicat6"="ethnicat6",
                                                                                                               "group"="group","n"="n")) %>% 
  full_join(cuiud_pyears_max_ethnicat6,by = c("ethnicat6"="ethnicat6",
                                            "group"="group","n"="n")) %>%
  rename(cuiud_pyears_max = n) %>% 
  mutate(across(is.numeric, ~ round(.,0)))

cuiud_pyears_max_ethnicat6_export <- full_join(cuiud_pyears_max_ethnicat6_all, ethnicat6_all, 
                                             by = c("ethnicat6"="ethnicat6","group"="group")) %>% 
  relocate(group,ethnicat6,n_percent,cuiud_pyears_max)  %>% 
  arrange(group)

write_csv(cuiud_pyears_max_ethnicat6_export, "filepath")


## practice region ---

cuiud_cohort_final_pregmax$prac_region <- cuiud_cohort_final_pregmax$prac_region %>% fct_explicit_na(na_level = "Not known")
levels(cuiud_cohort_final_pregmax$prac_region)

prac_region <- cuiud_cohort_final_pregmax %>% 
  group_by(prac_region) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
prac_region_mvnm <- cuiud_cohort_final_pregmax %>% 
...
prac_region_dp <- cuiud_cohort_final_pregmax %>% 
...
cuiud_pyears_max_prac_region <- cuiud_cohort_final_pregmax %>% group_by(prac_region) %>% tally(cuiud_pyears_max) %>% 
  mutate(group = "Whole Cohort")
cuiud_pyears_max_prac_region_mvnm <- cuiud_cohort_final_pregmax %>% group_by(migrant_status, prac_region) %>% tally(cuiud_pyears_max) %>% 
  rename(group = migrant_status)
cuiud_pyears_max_prac_region_dp <- cuiud_cohort_final_pregmax %>% group_by(migcertainty, prac_region) %>% tally(cuiud_pyears_max) %>% 
  rename(group = migcertainty)
cuiud_pyears_max_prac_region_all <- cuiud_pyears_max_prac_region_mvnm %>% full_join(cuiud_pyears_max_prac_region_dp,by = c("prac_region"="prac_region",
                                                                                                                     "group"="group","n"="n")) %>% 
  full_join(cuiud_pyears_max_prac_region,by = c("prac_region"="prac_region",
                                              "group"="group","n"="n")) %>%
  rename(cuiud_pyears_max = n) %>% 
  mutate(across(is.numeric, ~ round(.,0)))

cuiud_pyears_max_prac_region_export <- full_join(cuiud_pyears_max_prac_region_all, prac_region_all, 
                                               by = c("prac_region"="prac_region","group"="group")) %>% 
  relocate(group,prac_region,n_percent,cuiud_pyears_max)  %>% 
  arrange(group)

write_csv(cuiud_pyears_max_prac_region_export, "filepath")


## IMD ---

cuiud_cohort_final_pregmax$imd<- cuiud_cohort_final_pregmax$imd%>% fct_explicit_na(na_level = "Not known")
levels(cuiud_cohort_final_pregmax$imd)

imd<- cuiud_cohort_final_pregmax %>% 
  group_by(imd) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
imd_mvnm <- cuiud_cohort_final_pregmax %>% 
...
imd_dp <- cuiud_cohort_final_pregmax %>% 
...
cuiud_pyears_max_imd <- cuiud_cohort_final_pregmax %>% group_by(imd) %>% tally(cuiud_pyears_max) %>% 
  mutate(group = "Whole Cohort")
cuiud_pyears_max_imd_mvnm <- cuiud_cohort_final_pregmax %>% group_by(migrant_status, imd) %>% tally(cuiud_pyears_max) %>% 
  rename(group = migrant_status)
cuiud_pyears_max_imd_dp <- cuiud_cohort_final_pregmax %>% group_by(migcertainty, imd) %>% tally(cuiud_pyears_max) %>% 
  rename(group = migcertainty)
cuiud_pyears_max_imd_all <- cuiud_pyears_max_imd_mvnm %>% full_join(cuiud_pyears_max_imd_dp,by = c("imd"="imd",
                                                                                             "group"="group","n"="n")) %>% 
  full_join(cuiud_pyears_max_imd,by = c("imd"="imd",
                                      "group"="group","n"="n")) %>%
  rename(cuiud_pyears_max = n) %>% 
  mutate(across(is.numeric, ~ round(.,0)))

cuiud_pyears_max_imd_export <- full_join(cuiud_pyears_max_imd_all, imd_all, 
                                       by = c("imd"="imd","group"="group")) %>% 
  relocate(group,imd,n_percent,cuiud_pyears_max)  %>% 
  arrange(group)

write_csv(cuiud_pyears_max_imd_export, "filepath")



## Year of cohort entry ---

cuiud_cohort_final_pregmax_yearofentry <- cuiud_cohort_final_pregmax %>% mutate(yearofentry = year(cuiud_cohort_entry))
mean_yearofentry_overall <- psych::describe(cuiud_cohort_final_pregmax_yearofentry$yearofentry, IQR = TRUE)
mean_yearofentry_overall

nm <- cuiud_cohort_final_pregmax_yearofentry %>% filter(migrant_status == "Non-migrant")
nm_yearofentry <- psych::describe(nm$yearofentry, IQR = TRUE)
nm_yearofentry

m <- cuiud_cohort_final_pregmax_yearofentry %>% filter(migrant_status == "Migrant")
m_yearofentry <- psych::describe(m$yearofentry, IQR = TRUE)
m_yearofentry

d <- cuiud_cohort_final_pregmax_yearofentry %>% filter(migcertainty == "Definite")
d_yearofentry  <- psych::describe(d$yearofentry, IQR = TRUE)
d_yearofentry 

p <- cuiud_cohort_final_pregmax_yearofentry %>% filter(migcertainty == "Probable")
...
cuiud_cohort_final_pregmax_yearofentry$yearofentry <- factor(cuiud_cohort_final_pregmax_yearofentry$yearofentry,  levels = c(2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018), 
                                                           labels = c("2009", "2010", "2011", "2012", "2013", "2014", "2015", "2016", "2017", "2018"))
yearofentry <- cuiud_cohort_final_pregmax_yearofentry %>% 
  group_by(yearofentry) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
yearofentry_mvnm <- cuiud_cohort_final_pregmax_yearofentry %>% 
...
yearofentry_dp <- cuiud_cohort_final_pregmax_yearofentry %>% 
...
cuiud_pyears_max_yearofentry <- cuiud_cohort_final_pregmax_yearofentry %>% group_by(yearofentry) %>% tally(cuiud_pyears_max) %>% 
  mutate(group = "Whole Cohort")
cuiud_pyears_max_yearofentry_mvnm <- cuiud_cohort_final_pregmax_yearofentry %>% group_by(migrant_status, yearofentry) %>% tally(cuiud_pyears_max) %>% 
  rename(group = migrant_status)
cuiud_pyears_max_yearofentry_dp <- cuiud_cohort_final_pregmax_yearofentry %>% group_by(migcertainty, yearofentry) %>% tally(cuiud_pyears_max) %>% 
  rename(group = migcertainty)
cuiud_pyears_max_yearofentry_all <- cuiud_pyears_max_yearofentry_mvnm %>% full_join(cuiud_pyears_max_yearofentry_dp,by = c("yearofentry"="yearofentry",
                                                                                                                     "group"="group","n"="n")) %>% 
  full_join(cuiud_pyears_max_yearofentry,by = c("yearofentry"="yearofentry",
                                              "group"="group","n"="n")) %>%
  rename(cuiud_pyears_max = n) %>% 
  mutate(across(is.numeric, ~ round(.,0)))

cuiud_pyears_max_yearofentry_export <- full_join(cuiud_pyears_max_yearofentry_all, yearofentry_all, 
                                               by = c("group"="group","yearofentry"="yearofentry")) %>% 
  relocate(group,yearofentry,n_percent,cuiud_pyears_max)  %>% 
  arrange(group)

write_csv(cuiud_pyears_max_yearofentry_export, "filepath")


## Year ---

eventyear<- cuiudpresc_annual_counts_final_extra_preg_max %>% 
  group_by(eventyear) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
eventyear_mvnm <- cuiudpresc_annual_counts_final_extra_preg_max  %>% 
...
eventyear_dp <- cuiudpresc_annual_counts_final_extra_preg_max  %>% 
...
pyears_eventyear <- cuiudpresc_annual_counts_final_extra_preg_max  %>% group_by(eventyear) %>% tally(pyears_ey_pregmax) %>% 
  mutate(group = "Whole Cohort")
pyears_eventyear_mvnm <- cuiudpresc_annual_counts_final_extra_preg_max  %>% group_by(migrant_status, eventyear) %>% tally(pyears_ey_pregmax) %>% 
  rename(group = migrant_status)
pyears_eventyear_dp <- cuiudpresc_annual_counts_final_extra_preg_max  %>% group_by(migcertainty, eventyear) %>% tally(pyears_ey_pregmax) %>% 
  rename(group = migcertainty)
...
pyears_eventyear_export <- full_join(pyears_eventyear_all, eventyear_all, 
  ...
  relocate(group,eventyear,n_percent,cuiud_pyears_max)  %>% 
  arrange(group)
...
mean_cuiud_pyears_max_overall <- psych::describe(cuiud_cohort_final_pregmax$cuiud_pyears_max, IQR = TRUE)
mean_cuiud_pyears_max_overall

nm <- cuiud_cohort_final_pregmax %>% filter(migrant_status == "Non-migrant")
nm_cuiud_pyears_max <- psych::describe(nm$cuiud_pyears_max, IQR = TRUE)
nm_cuiud_pyears_max

m <- cuiud_cohort_final_pregmax %>% filter(migrant_status == "Migrant")
m_cuiud_pyears_max <- psych::describe(m$cuiud_pyears_max, IQR = TRUE)
m_cuiud_pyears_max

d <- cuiud_cohort_final_pregmax %>% filter(migcertainty == "Definite")
d_cuiud_pyears_max <- psych::describe(d$cuiud_pyears_max, IQR = TRUE)
d_cuiud_pyears_max

p <- cuiud_cohort_final_pregmax %>% filter(migcertainty == "Probable")
p_cuiud_pyears_max <- psych::describe(p$cuiud_pyears_max, IQR = TRUE)
p_cuiud_pyears_max

## join export mean + median ages + sd + IQR
mean_cuiud_pyears_max_overall <- as.data.frame((mean_cuiud_pyears_max_overall))
mean_cuiud_pyears_max_overall <- dplyr::select(mean_cuiud_pyears_max_overall, c(mean , sd, median, IQR)) %>% mutate(group = "Whole cohort") 
nm_cuiud_pyears_max <- as.data.frame((nm_cuiud_pyears_max))
nm_cuiud_pyears_max <- dplyr::select(nm_cuiud_pyears_max, c(mean , sd, median, IQR)) %>% mutate(group = "Non-migrant") 
m_cuiud_pyears_max <- as.data.frame((m_cuiud_pyears_max))
m_cuiud_pyears_max <- dplyr::select(m_cuiud_pyears_max, c(mean , sd, median, IQR)) %>% mutate(group = "Migrant") 
d_cuiud_pyears_max <- as.data.frame((d_cuiud_pyears_max))
d_cuiud_pyears_max <- dplyr::select(d_cuiud_pyears_max, c(mean , sd, median, IQR)) %>% mutate(group = "Definite") 
p_cuiud_pyears_max <- as.data.frame((p_cuiud_pyears_max))
p_cuiud_pyears_max <- dplyr::select(p_cuiud_pyears_max, c(mean , sd, median, IQR)) %>% mutate(group = "Probable") 
all_mean_cuiud_pyears_max <- full_join(mean_cuiud_pyears_max_overall, nm_cuiud_pyears_max,
                                     by = c( "group" = "group", "mean" = "mean", "sd" = "sd" , "median" = "median", "IQR" = "IQR")) %>% 
  full_join(m_cuiud_pyears_max, by = c( "group" = "group", "mean" = "mean", "sd" = "sd" , "median" = "median", "IQR" = "IQR") ) %>%
  full_join(d_cuiud_pyears_max, by = c( "group" = "group", "mean" = "mean", "sd" = "sd", "median" = "median",  "IQR" = "IQR" ) ) %>%
  full_join(p_cuiud_pyears_max, by = c( "group" = "group", "mean" = "mean", "sd" = "sd" , "median" = "median", "IQR" = "IQR") )  %>%
  mutate(across(is.numeric, ~ round(.,2)))
all_mean_cuiud_pyears_max$mean_sd <- paste(all_mean_cuiud_pyears_max$mean, all_mean_cuiud_pyears_max$sd, sep =", ")
all_mean_cuiud_pyears_max$median_iqr <- paste(all_mean_cuiud_pyears_max$median, all_mean_cuiud_pyears_max$IQR, sep =", ")
all_mean_cuiud_pyears_max <- dplyr::select(all_mean_cuiud_pyears_max, c(group, mean_sd, median_iqr))
write_csv(all_mean_cuiud_pyears_max, "filepath" )
...
## change cohort final to be based on preg max assumption
# drop records with 0 or less pdays_ey_pregmax in counts
exact_match_cuiudpresc_annual_counts_final_extra_4to1_preg_max <- exact_match_cuiudpresc_annual_counts_final_extra_4to1 %>% filter(pdays_ey_pregmax > 0) 
# recalculate pdays total based on counts data and join
exact_match_cuiudpresc_annual_counts_final_extra_4to1_preg_max_pdays <- exact_match_cuiudpresc_annual_counts_final_extra_4to1_preg_max %>% group_by(patid) %>% tally(pdays_ey_pregmax) %>% rename(cuiud_pdays_max = n )
exact_match_cuiudpresc_annual_counts_final_extra_4to1_preg_max_pyears <- exact_match_cuiudpresc_annual_counts_final_extra_4to1_preg_max %>% group_by(patid) %>% tally(pyears_ey_pregmax) %>% rename(cuiud_pyears_max = n )
exact_match_cuiudpresc_annual_counts_final_extra_4to1_preg_max_days_pyears <- full_join(exact_match_cuiudpresc_annual_counts_final_extra_4to1_preg_max_pdays, exact_match_cuiudpresc_annual_counts_final_extra_4to1_preg_max_pyears, by = c("patid" = "patid"))
# select patients in cohort file based on preg max assumption and add on recalculated pdays/pyears
exact_match_cuiud_cohort_final_4to1_pregmax <- exact_match_cuiud_cohort_final_4to1 %>% filter(patid %in% exact_match_cuiudpresc_annual_counts_final_extra_4to1_preg_max$patid) %>% dplyr::select(-cuiud_pdays_max, -cuiud_pdays_min, -cuiud_pyears_max, -cuiud_pyears_min)
exact_match_cuiud_cohort_final_4to1_pregmax <- left_join(exact_match_cuiud_cohort_final_4to1_pregmax, exact_match_cuiudpresc_annual_counts_final_extra_4to1_preg_max_days_pyears, by = c("patid" = "patid"))

## Totals ---

## Whole cohort 
total_cohort_n <- count(exact_match_cuiud_cohort_final_4to1_pregmax)
total_cuiud_pyears_max <- sum(exact_match_cuiud_cohort_final_4to1_pregmax$cuiud_pyears_max)
total_wholecohort <- bind_cols(total_cohort_n, total_cuiud_pyears_max)
colnames(total_wholecohort) <- c("individuals", "cuiud_pyears_max")
total_wholecohort <- total_wholecohort %>% mutate(group = "Whole cohort") 

## By migrant status
total_mvnm_n <- exact_match_cuiud_cohort_final_4to1_pregmax %>% group_by(migrant_status) %>% 
  count() %>% 
  rename(individuals = n) %>% 
  rename(group = migrant_status)
total_mvnm_cuiud_pyears_max <- exact_match_cuiud_cohort_final_4to1_pregmax %>% group_by(migrant_status) %>%
  tally(cuiud_pyears_max) %>%
  rename(cuiud_pyears_max = n) %>% 
  rename(group = migrant_status)
total_mvnm <- full_join(total_mvnm_n, total_mvnm_cuiud_pyears_max,
...
## By migrant certainty
total_dp_n <- exact_match_cuiud_cohort_final_4to1_pregmax %>% group_by(migcertainty) %>% 
  count() %>% 
  rename(individuals = n) %>% 
  rename(group = migcertainty)
total_dp_cuiud_pyears_max <- exact_match_cuiud_cohort_final_4to1_pregmax %>% group_by(migcertainty) %>%
  tally(cuiud_pyears_max) %>%
  rename(cuiud_pyears_max = n) %>% 
  rename(group = migcertainty)
total_dp <- full_join(total_dp_n, total_dp_cuiud_pyears_max,
...
exact_match_cuiud_cohort_final_4to1_pregmax_age <- exact_match_cuiud_cohort_final_4to1_pregmax %>%
  mutate(age_cohort_entry = calc_age(dob, cuiud_cohort_entry))
mean_ages_overall <- psych::describe(exact_match_cuiud_cohort_final_4to1_pregmax_age$age_cohort_entry, IQR = TRUE)
mean_ages_overall

nm <- exact_match_cuiud_cohort_final_4to1_pregmax_age %>% filter(migrant_status == "Non-migrant")
nm_age <- psych::describe(nm$age_cohort_entry, IQR = TRUE)
nm_age

m <- exact_match_cuiud_cohort_final_4to1_pregmax_age %>% filter(migrant_status == "Migrant")
m_age <- psych::describe(m$age_cohort_entry, IQR = TRUE)
m_age

d <- exact_match_cuiud_cohort_final_4to1_pregmax_age %>% filter(migcertainty == "Definite")
d_age <- psych::describe(d$age_cohort_entry, IQR = TRUE)
d_age

p <- exact_match_cuiud_cohort_final_4to1_pregmax_age %>% filter(migcertainty == "Probable")
p_age <- psych::describe(p$age_cohort_entry, IQR = TRUE)
p_age
...
## Age category - No individuals and p years per group 
exact_match_cuiud_cohort_final_4to1_pregmax_agecat <- exact_match_cuiud_cohort_final_4to1_pregmax_age %>%
  mutate(agecat_cohort_entry = age_cohort_entry)
exact_match_cuiud_cohort_final_4to1_pregmax_agecat <- exact_match_cuiud_cohort_final_4to1_pregmax_agecat %>%
  mutate(agecat_cohort_entry=replace(agecat_cohort_entry, between (agecat_cohort_entry, 15, 19), 1)) %>%
  mutate(agecat_cohort_entry=replace(agecat_cohort_entry, between (agecat_cohort_entry, 20, 24), 2)) %>%
  mutate(agecat_cohort_entry=replace(agecat_cohort_entry, between (agecat_cohort_entry, 25, 29), 3)) %>%
  mutate(agecat_cohort_entry=replace(agecat_cohort_entry, between (agecat_cohort_entry, 30, 34), 4)) %>%
  ...
  mutate(agecat_cohort_entry=replace(agecat_cohort_entry, between (agecat_cohort_entry, 40, 44), 6)) %>%
  mutate(agecat_cohort_entry=replace(agecat_cohort_entry, between (agecat_cohort_entry, 45, 49), 7)) 
exact_match_cuiud_cohort_final_4to1_pregmax_agecat$agecat_cohort_entry <- factor(exact_match_cuiud_cohort_final_4to1_pregmax_agecat$agecat_cohort_entry, levels = c(1,2,3,4,5,6,7),
                                                                               labels = c("15-19", "20-24", "25-29", "30-34", "35-39", "40-44", "45-49"))

agecat <- exact_match_cuiud_cohort_final_4to1_pregmax_agecat %>% 
  group_by(agecat_cohort_entry) %>% count() 
agecat <- agecat %>% mutate(percent = (n/sum(agecat$n))*100) %>%
  mutate(group = "Whole Cohort")
agecat_mvnm <- exact_match_cuiud_cohort_final_4to1_pregmax_agecat %>% 
  group_by(agecat_cohort_entry, migrant_status) %>% count() %>% 
  rename(group = migrant_status)
agecat_m <- agecat_mvnm %>% filter(group == "Migrant")
agecat_m <- agecat_m %>% mutate(percent = (n/sum(agecat_m$n))*100)
agecat_nm <- agecat_mvnm %>% filter(group == "Non-migrant")
agecat_nm <- agecat_nm %>% mutate(percent = (n/sum(agecat_nm$n))*100)
agecat_dp <- exact_match_cuiud_cohort_final_4to1_pregmax_agecat %>% 
  group_by(agecat_cohort_entry, migcertainty) %>% count() %>% 
  rename(group = migcertainty)
agecat_d <- agecat_dp %>% filter(group == "Definite")
agecat_d <- agecat_d %>% mutate(percent = (n/sum(agecat_d$n))*100)
...
cuiud_pyears_max_agecat <- aggregate(cuiud_pyears_max ~ agecat_cohort_entry, exact_match_cuiud_cohort_final_4to1_pregmax_agecat, sum) %>% 
  mutate(group = "Whole Cohort")
cuiud_pyears_max_agecat_mvnm <- aggregate(cuiud_pyears_max ~ agecat_cohort_entry + migrant_status, exact_match_cuiud_cohort_final_4to1_pregmax_agecat, sum) %>% 
  rename(group = migrant_status)
cuiud_pyears_max_agecat_dp <- aggregate(cuiud_pyears_max ~ agecat_cohort_entry + migcertainty, exact_match_cuiud_cohort_final_4to1_pregmax_agecat, sum) %>% 
  rename(group = migcertainty)
cuiud_pyears_max_agecat_all <- cuiud_pyears_max_agecat_mvnm %>% full_join(cuiud_pyears_max_agecat_dp,by = c("agecat_cohort_entry"="agecat_cohort_entry",
                                                                                                      "group"="group","cuiud_pyears_max"="cuiud_pyears_max")) %>% 
  full_join(cuiud_pyears_max_agecat,by = c("agecat_cohort_entry"="agecat_cohort_entry",
...
exact_match_cuiud_cohort_final_4to1_pregmax$ethnicat6 <- exact_match_cuiud_cohort_final_4to1_pregmax$ethnicat6 %>% fct_explicit_na(na_level = "Not known")
levels(exact_match_cuiud_cohort_final_4to1_pregmax$ethnicat6)

ethnicat6 <- exact_match_cuiud_cohort_final_4to1_pregmax %>% 
  group_by(ethnicat6) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
ethnicat6_mvnm <- exact_match_cuiud_cohort_final_4to1_pregmax %>% 
  group_by(ethnicat6, migrant_status) %>% count() %>% 
  rename(group = migrant_status) %>% 
  full_join(total_mvnm, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1))) %>%
  dplyr::select(c(ethnicat6, group, n, percent))
ethnicat6_dp <- exact_match_cuiud_cohort_final_4to1_pregmax %>% 
  group_by(ethnicat6, migcertainty) %>% count() %>% 
  rename(group = migcertainty) %>% 
  full_join(total_dp, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
...
cuiud_pyears_max_ethnicat6 <- exact_match_cuiud_cohort_final_4to1_pregmax %>% group_by(ethnicat6) %>% tally(cuiud_pyears_max) %>% 
  mutate(group = "Whole Cohort")
cuiud_pyears_max_ethnicat6_mvnm <- exact_match_cuiud_cohort_final_4to1_pregmax %>% group_by(migrant_status, ethnicat6) %>% tally(cuiud_pyears_max) %>% 
  rename(group = migrant_status)
cuiud_pyears_max_ethnicat6_dp <- exact_match_cuiud_cohort_final_4to1_pregmax %>% group_by(migcertainty, ethnicat6) %>% tally(cuiud_pyears_max) %>% 
  rename(group = migcertainty)
cuiud_pyears_max_ethnicat6_all <- cuiud_pyears_max_ethnicat6_mvnm %>% full_join(cuiud_pyears_max_ethnicat6_dp,by = c("ethnicat6"="ethnicat6",
                                                                                                               "group"="group","n"="n")) %>% 
  full_join(cuiud_pyears_max_ethnicat6,by = c("ethnicat6"="ethnicat6",
...
exact_match_cuiud_cohort_final_4to1_pregmax$prac_region <- exact_match_cuiud_cohort_final_4to1_pregmax$prac_region %>% fct_explicit_na(na_level = "Not known")
levels(exact_match_cuiud_cohort_final_4to1_pregmax$prac_region)

prac_region <- exact_match_cuiud_cohort_final_4to1_pregmax %>% 
  group_by(prac_region) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
prac_region_mvnm <- exact_match_cuiud_cohort_final_4to1_pregmax %>% 
  group_by(prac_region, migrant_status) %>% count() %>% 
  rename(group = migrant_status) %>% 
  full_join(total_mvnm, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1))) %>%
  dplyr::select(c(prac_region, group, n, percent))
prac_region_dp <- exact_match_cuiud_cohort_final_4to1_pregmax %>% 
  group_by(prac_region, migcertainty) %>% count() %>% 
  rename(group = migcertainty) %>% 
  full_join(total_dp, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
...
cuiud_pyears_max_prac_region <- exact_match_cuiud_cohort_final_4to1_pregmax %>% group_by(prac_region) %>% tally(cuiud_pyears_max) %>% 
  mutate(group = "Whole Cohort")
cuiud_pyears_max_prac_region_mvnm <- exact_match_cuiud_cohort_final_4to1_pregmax %>% group_by(migrant_status, prac_region) %>% tally(cuiud_pyears_max) %>% 
  rename(group = migrant_status)
cuiud_pyears_max_prac_region_dp <- exact_match_cuiud_cohort_final_4to1_pregmax %>% group_by(migcertainty, prac_region) %>% tally(cuiud_pyears_max) %>% 
  rename(group = migcertainty)
cuiud_pyears_max_prac_region_all <- cuiud_pyears_max_prac_region_mvnm %>% full_join(cuiud_pyears_max_prac_region_dp,by = c("prac_region"="prac_region",
                                                                                                                     "group"="group","n"="n")) %>% 
  full_join(cuiud_pyears_max_prac_region,by = c("prac_region"="prac_region",
...
exact_match_cuiud_cohort_final_4to1_pregmax$imd<- exact_match_cuiud_cohort_final_4to1_pregmax$imd%>% fct_explicit_na(na_level = "Not known")
levels(exact_match_cuiud_cohort_final_4to1_pregmax$imd)

imd<- exact_match_cuiud_cohort_final_4to1_pregmax %>% 
  group_by(imd) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
imd_mvnm <- exact_match_cuiud_cohort_final_4to1_pregmax %>% 
  group_by(imd, migrant_status) %>% count() %>% 
  rename(group = migrant_status) %>% 
  full_join(total_mvnm, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1))) %>%
  dplyr::select(c(imd, group, n, percent))
imd_dp <- exact_match_cuiud_cohort_final_4to1_pregmax %>% 
  group_by(imd, migcertainty) %>% count() %>% 
  rename(group = migcertainty) %>% 
  full_join(total_dp, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
...
cuiud_pyears_max_imd <- exact_match_cuiud_cohort_final_4to1_pregmax %>% group_by(imd) %>% tally(cuiud_pyears_max) %>% 
  mutate(group = "Whole Cohort")
cuiud_pyears_max_imd_mvnm <- exact_match_cuiud_cohort_final_4to1_pregmax %>% group_by(migrant_status, imd) %>% tally(cuiud_pyears_max) %>% 
  rename(group = migrant_status)
cuiud_pyears_max_imd_dp <- exact_match_cuiud_cohort_final_4to1_pregmax %>% group_by(migcertainty, imd) %>% tally(cuiud_pyears_max) %>% 
  rename(group = migcertainty)
cuiud_pyears_max_imd_all <- cuiud_pyears_max_imd_mvnm %>% full_join(cuiud_pyears_max_imd_dp,by = c("imd"="imd",
                                                                                             "group"="group","n"="n")) %>% 
  full_join(cuiud_pyears_max_imd,by = c("imd"="imd",
...
exact_match_cuiud_cohort_final_4to1_pregmax_yearofentry <- exact_match_cuiud_cohort_final_4to1_pregmax %>% mutate(yearofentry = year(cuiud_cohort_entry))
mean_yearofentry_overall <- psych::describe(exact_match_cuiud_cohort_final_4to1_pregmax_yearofentry$yearofentry, IQR = TRUE)
mean_yearofentry_overall

nm <- exact_match_cuiud_cohort_final_4to1_pregmax_yearofentry %>% filter(migrant_status == "Non-migrant")
nm_yearofentry <- psych::describe(nm$yearofentry, IQR = TRUE)
nm_yearofentry

m <- exact_match_cuiud_cohort_final_4to1_pregmax_yearofentry %>% filter(migrant_status == "Migrant")
m_yearofentry <- psych::describe(m$yearofentry, IQR = TRUE)
m_yearofentry

d <- exact_match_cuiud_cohort_final_4to1_pregmax_yearofentry %>% filter(migcertainty == "Definite")
d_yearofentry  <- psych::describe(d$yearofentry, IQR = TRUE)
d_yearofentry 

p <- exact_match_cuiud_cohort_final_4to1_pregmax_yearofentry %>% filter(migcertainty == "Probable")
p_yearofentry <- psych::describe(p$yearofentry, IQR = TRUE)
p_yearofentry
...
exact_match_cuiud_cohort_final_4to1_pregmax_yearofentry$yearofentry <- factor(exact_match_cuiud_cohort_final_4to1_pregmax_yearofentry$yearofentry,  levels = c(2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018), 
                                                                            labels = c("2009", "2010", "2011", "2012", "2013", "2014", "2015", "2016", "2017", "2018"))
yearofentry <- exact_match_cuiud_cohort_final_4to1_pregmax_yearofentry %>% 
  group_by(yearofentry) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
yearofentry_mvnm <- exact_match_cuiud_cohort_final_4to1_pregmax_yearofentry %>% 
  group_by(yearofentry, migrant_status) %>% count() %>% 
  rename(group = migrant_status) %>% 
  full_join(total_mvnm, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1))) %>%
  dplyr::select(c(yearofentry, group, n, percent))
yearofentry_dp <- exact_match_cuiud_cohort_final_4to1_pregmax_yearofentry %>% 
  group_by(yearofentry, migcertainty) %>% count() %>% 
  rename(group = migcertainty) %>% 
  full_join(total_dp, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
...
cuiud_pyears_max_yearofentry <- exact_match_cuiud_cohort_final_4to1_pregmax_yearofentry %>% group_by(yearofentry) %>% tally(cuiud_pyears_max) %>% 
  mutate(group = "Whole Cohort")
cuiud_pyears_max_yearofentry_mvnm <- exact_match_cuiud_cohort_final_4to1_pregmax_yearofentry %>% group_by(migrant_status, yearofentry) %>% tally(cuiud_pyears_max) %>% 
  rename(group = migrant_status)
cuiud_pyears_max_yearofentry_dp <- exact_match_cuiud_cohort_final_4to1_pregmax_yearofentry %>% group_by(migcertainty, yearofentry) %>% tally(cuiud_pyears_max) %>% 
  rename(group = migcertainty)
cuiud_pyears_max_yearofentry_all <- cuiud_pyears_max_yearofentry_mvnm %>% full_join(cuiud_pyears_max_yearofentry_dp,by = c("yearofentry"="yearofentry",
                                                                                                                     "group"="group","n"="n")) %>% 
  full_join(cuiud_pyears_max_yearofentry,by = c("yearofentry"="yearofentry",
...
eventyear<- exact_match_cuiudpresc_annual_counts_final_extra_4to1_preg_max %>% 
  group_by(eventyear) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
eventyear_mvnm <- exact_match_cuiudpresc_annual_counts_final_extra_4to1_preg_max  %>% 
  group_by(eventyear, migrant_status) %>% count() %>% 
  rename(group = migrant_status) %>% 
  full_join(total_mvnm, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1))) %>%
  dplyr::select(c(eventyear, group, n, percent))
eventyear_dp <- exact_match_cuiudpresc_annual_counts_final_extra_4to1_preg_max  %>% 
  group_by(eventyear, migcertainty) %>% count() %>% 
  rename(group = migcertainty) %>% 
  full_join(total_dp, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
...
pyears_eventyear <- exact_match_cuiudpresc_annual_counts_final_extra_4to1_preg_max  %>% group_by(eventyear) %>% tally(pyears_ey_pregmax) %>% 
  mutate(group = "Whole Cohort")
pyears_eventyear_mvnm <- exact_match_cuiudpresc_annual_counts_final_extra_4to1_preg_max  %>% group_by(migrant_status, eventyear) %>% tally(pyears_ey_pregmax) %>% 
  rename(group = migrant_status)
pyears_eventyear_dp <- exact_match_cuiudpresc_annual_counts_final_extra_4to1_preg_max  %>% group_by(migcertainty, eventyear) %>% tally(pyears_ey_pregmax) %>% 
  rename(group = migcertainty)
pyears_eventyear_all <- pyears_eventyear_mvnm %>% full_join(pyears_eventyear_dp,by = c("eventyear"="eventyear",
                                                                                       "group"="group","n"="n")) %>% 
  full_join(pyears_eventyear,by = c("eventyear"="eventyear",
...
mean_cuiud_pyears_max_overall <- psych::describe(exact_match_cuiud_cohort_final_4to1_pregmax$cuiud_pyears_max, IQR = TRUE)
mean_cuiud_pyears_max_overall

nm <- exact_match_cuiud_cohort_final_4to1_pregmax %>% filter(migrant_status == "Non-migrant")
nm_cuiud_pyears_max <- psych::describe(nm$cuiud_pyears_max, IQR = TRUE)
nm_cuiud_pyears_max

m <- exact_match_cuiud_cohort_final_4to1_pregmax %>% filter(migrant_status == "Migrant")
m_cuiud_pyears_max <- psych::describe(m$cuiud_pyears_max, IQR = TRUE)
m_cuiud_pyears_max

d <- exact_match_cuiud_cohort_final_4to1_pregmax %>% filter(migcertainty == "Definite")
d_cuiud_pyears_max <- psych::describe(d$cuiud_pyears_max, IQR = TRUE)
d_cuiud_pyears_max

p <- exact_match_cuiud_cohort_final_4to1_pregmax %>% filter(migcertainty == "Probable")
p_cuiud_pyears_max <- psych::describe(p$cuiud_pyears_max, IQR = TRUE)
p_cuiud_pyears_max
...
cuiudpresc_annual_counts_final_extra_preg_min <- cuiudpresc_annual_counts_final_extra %>% filter(pdays_ey_pregmin > 0) 
# recalculate pdays total based on counts data and join
cuiudpresc_annual_counts_final_extra_preg_min_pdays <- cuiudpresc_annual_counts_final_extra_preg_min %>% group_by(patid) %>% tally(pdays_ey_pregmin) %>% rename(cuiud_pdays_min = n )
cuiudpresc_annual_counts_final_extra_preg_min_pyears <- cuiudpresc_annual_counts_final_extra_preg_min %>% group_by(patid) %>% tally(pyears_ey_pregmin) %>% rename(cuiud_pyears_min = n )
cuiudpresc_annual_counts_final_extra_preg_min_days_pyears <- full_join(cuiudpresc_annual_counts_final_extra_preg_min_pdays, cuiudpresc_annual_counts_final_extra_preg_min_pyears, by = c("patid" = "patid"))
# select patients in cohort file based on preg min assumption and add on recalculated pdays/pyears
cuiud_cohort_final_pregmin <- cuiud_cohort_final %>% filter(patid %in% cuiudpresc_annual_counts_final_extra_preg_min$patid) %>% dplyr::select(-cuiud_pdays_min, -cuiud_pdays_min, -cuiud_pyears_min, -cuiud_pyears_min)
cuiud_cohort_final_pregmin <- left_join(cuiud_cohort_final_pregmin, cuiudpresc_annual_counts_final_extra_preg_min_days_pyears, by = c("patid" = "patid"))

## Totals ---

## Whole cohort 
total_cohort_n <- count(cuiud_cohort_final_pregmin)
total_cuiud_pyears_min <- sum(cuiud_cohort_final_pregmin$cuiud_pyears_min)
total_wholecohort <- bind_cols(total_cohort_n, total_cuiud_pyears_min)
colnames(total_wholecohort) <- c("individuals", "cuiud_pyears_min")
total_wholecohort <- total_wholecohort %>% mutate(group = "Whole cohort") 

## By migrant status
total_mvnm_n <- cuiud_cohort_final_pregmin %>% group_by(migrant_status) %>% 
  count() %>% 
  rename(individuals = n) %>% 
  rename(group = migrant_status)
total_mvnm_cuiud_pyears_min <- cuiud_cohort_final_pregmin %>% group_by(migrant_status) %>%
  tally(cuiud_pyears_min) %>%
  rename(cuiud_pyears_min = n) %>% 
  rename(group = migrant_status)
total_mvnm <- full_join(total_mvnm_n, total_mvnm_cuiud_pyears_min,
                        by = c("group" = "group"))

## By migrant certainty
total_dp_n <- cuiud_cohort_final_pregmin %>% group_by(migcertainty) %>% 
  count() %>% 
  rename(individuals = n) %>% 
  rename(group = migcertainty)
total_dp_cuiud_pyears_min <- cuiud_cohort_final_pregmin %>% group_by(migcertainty) %>%
  tally(cuiud_pyears_min) %>%
  rename(cuiud_pyears_min = n) %>% 
  rename(group = migcertainty)
total_dp <- full_join(total_dp_n, total_dp_cuiud_pyears_min,
                      by = c("group" = "group"))

## Totals for export
totals <- full_join(total_mvnm, total_dp,
                    by = c("group" = "group", "individuals" = "individuals", "cuiud_pyears_min" = "cuiud_pyears_min")) %>% 
  full_join(total_wholecohort, by = c("group" = "group", "individuals" = "individuals", "cuiud_pyears_min" = "cuiud_pyears_min")) %>% 
  mutate(across(is.numeric, ~ round(.,0)))
total_cohort_n<- as.vector(total_cohort_n$n[1])
totals_percent <- totals %>% mutate(percent = (individuals/total_cohort_n)*100) %>% 
  mutate(percent = round(percent, 1))
...
totals_percent$individuals <- paste(totals_percent$individuals, totals_percent$percent, sep =", ")
totals_percent <- dplyr::select(totals_percent, c(group, individuals, cuiud_pyears_min))
write_csv(totals_percent, "filepath")
...
cuiud_cohort_final_pregmin_age <- cuiud_cohort_final_pregmin %>%
  mutate(age_cohort_entry = calc_age(dob, cuiud_cohort_entry))
mean_ages_overall <- psych::describe(cuiud_cohort_final_pregmin_age$age_cohort_entry, IQR = TRUE)
mean_ages_overall

nm <- cuiud_cohort_final_pregmin_age %>% filter(migrant_status == "Non-migrant")
nm_age <- psych::describe(nm$age_cohort_entry, IQR = TRUE)
nm_age

m <- cuiud_cohort_final_pregmin_age %>% filter(migrant_status == "Migrant")
m_age <- psych::describe(m$age_cohort_entry, IQR = TRUE)
m_age

d <- cuiud_cohort_final_pregmin_age %>% filter(migcertainty == "Definite")
d_age <- psych::describe(d$age_cohort_entry, IQR = TRUE)
d_age

p <- cuiud_cohort_final_pregmin_age %>% filter(migcertainty == "Probable")
p_age <- psych::describe(p$age_cohort_entry, IQR = TRUE)
p_age
...
## Age category - No individuals and p years per group 
cuiud_cohort_final_pregmin_agecat <- cuiud_cohort_final_pregmin_age %>%
  mutate(agecat_cohort_entry = age_cohort_entry)
cuiud_cohort_final_pregmin_agecat <- cuiud_cohort_final_pregmin_agecat %>%
  mutate(agecat_cohort_entry=replace(agecat_cohort_entry, between (agecat_cohort_entry, 15, 19), 1)) %>%
  mutate(agecat_cohort_entry=replace(agecat_cohort_entry, between (agecat_cohort_entry, 20, 24), 2)) %>%
  mutate(agecat_cohort_entry=replace(agecat_cohort_entry, between (agecat_cohort_entry, 25, 29), 3)) %>%
  mutate(agecat_cohort_entry=replace(agecat_cohort_entry, between (agecat_cohort_entry, 30, 34), 4)) %>%
  ...
  mutate(agecat_cohort_entry=replace(agecat_cohort_entry, between (agecat_cohort_entry, 40, 44), 6)) %>%
  mutate(agecat_cohort_entry=replace(agecat_cohort_entry, between (agecat_cohort_entry, 45, 49), 7)) 
cuiud_cohort_final_pregmin_agecat$agecat_cohort_entry <- factor(cuiud_cohort_final_pregmin_agecat$agecat_cohort_entry, levels = c(1,2,3,4,5,6,7),
                                                              labels = c("15-19", "20-24", "25-29", "30-34", "35-39", "40-44", "45-49"))

agecat <- cuiud_cohort_final_pregmin_agecat %>% 
  group_by(agecat_cohort_entry) %>% count() 
agecat <- agecat %>% mutate(percent = (n/sum(agecat$n))*100) %>%
  mutate(group = "Whole Cohort")
agecat_mvnm <- cuiud_cohort_final_pregmin_agecat %>% 
  group_by(agecat_cohort_entry, migrant_status) %>% count() %>% 
  rename(group = migrant_status)
agecat_m <- agecat_mvnm %>% filter(group == "Migrant")
agecat_m <- agecat_m %>% mutate(percent = (n/sum(agecat_m$n))*100)
agecat_nm <- agecat_mvnm %>% filter(group == "Non-migrant")
agecat_nm <- agecat_nm %>% mutate(percent = (n/sum(agecat_nm$n))*100)
agecat_dp <- cuiud_cohort_final_pregmin_agecat %>% 
  group_by(agecat_cohort_entry, migcertainty) %>% count() %>% 
  rename(group = migcertainty)
agecat_d <- agecat_dp %>% filter(group == "Definite")
agecat_d <- agecat_d %>% mutate(percent = (n/sum(agecat_d$n))*100)
...
cuiud_pyears_min_agecat <- aggregate(cuiud_pyears_min ~ agecat_cohort_entry, cuiud_cohort_final_pregmin_agecat, sum) %>% 
  mutate(group = "Whole Cohort")
cuiud_pyears_min_agecat_mvnm <- aggregate(cuiud_pyears_min ~ agecat_cohort_entry + migrant_status, cuiud_cohort_final_pregmin_agecat, sum) %>% 
  rename(group = migrant_status)
cuiud_pyears_min_agecat_dp <- aggregate(cuiud_pyears_min ~ agecat_cohort_entry + migcertainty, cuiud_cohort_final_pregmin_agecat, sum) %>% 
  rename(group = migcertainty)
cuiud_pyears_min_agecat_all <- cuiud_pyears_min_agecat_mvnm %>% full_join(cuiud_pyears_min_agecat_dp,by = c("agecat_cohort_entry"="agecat_cohort_entry",
                                                                                                      "group"="group","cuiud_pyears_min"="cuiud_pyears_min")) %>% 
  full_join(cuiud_pyears_min_agecat,by = c("agecat_cohort_entry"="agecat_cohort_entry",
                                         "group"="group","cuiud_pyears_min"="cuiud_pyears_min")) %>%
  rename(agecat = agecat_cohort_entry) 

cuiud_pyears_min_agecat_export <- full_join(cuiud_pyears_min_agecat_all, age_cat_all, 
                                          by = c("agecat"="agecat","group"="group")) %>% 
  mutate(across(is.numeric, ~ round(.,0))) %>% 
  dplyr::select(c(group, agecat, n_percent, cuiud_pyears_min)) 

write_csv(cuiud_pyears_min_agecat_export, "filepath")


## Ethnicity ---


cuiud_cohort_final_pregmin$ethnicat6 <- cuiud_cohort_final_pregmin$ethnicat6 %>% fct_explicit_na(na_level = "Not known")
levels(cuiud_cohort_final_pregmin$ethnicat6)

ethnicat6 <- cuiud_cohort_final_pregmin %>% 
  group_by(ethnicat6) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
ethnicat6_mvnm <- cuiud_cohort_final_pregmin %>% 
  group_by(ethnicat6, migrant_status) %>% count() %>% 
  rename(group = migrant_status) %>% 
  full_join(total_mvnm, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1))) %>%
  dplyr::select(c(ethnicat6, group, n, percent))
ethnicat6_dp <- cuiud_cohort_final_pregmin %>% 
  group_by(ethnicat6, migcertainty) %>% count() %>% 
  rename(group = migcertainty) %>% 
  full_join(total_dp, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
...
cuiud_pyears_min_ethnicat6 <- cuiud_cohort_final_pregmin %>% group_by(ethnicat6) %>% tally(cuiud_pyears_min) %>% 
  mutate(group = "Whole Cohort")
cuiud_pyears_min_ethnicat6_mvnm <- cuiud_cohort_final_pregmin %>% group_by(migrant_status, ethnicat6) %>% tally(cuiud_pyears_min) %>% 
  rename(group = migrant_status)
cuiud_pyears_min_ethnicat6_dp <- cuiud_cohort_final_pregmin %>% group_by(migcertainty, ethnicat6) %>% tally(cuiud_pyears_min) %>% 
  rename(group = migcertainty)
cuiud_pyears_min_ethnicat6_all <- cuiud_pyears_min_ethnicat6_mvnm %>% full_join(cuiud_pyears_min_ethnicat6_dp,by = c("ethnicat6"="ethnicat6",
                                                                                                               "group"="group","n"="n")) %>% 
  full_join(cuiud_pyears_min_ethnicat6,by = c("ethnicat6"="ethnicat6",
                                            "group"="group","n"="n")) %>%
  rename(cuiud_pyears_min = n) %>% 
  mutate(across(is.numeric, ~ round(.,0)))

cuiud_pyears_min_ethnicat6_export <- full_join(cuiud_pyears_min_ethnicat6_all, ethnicat6_all, 
                                             by = c("ethnicat6"="ethnicat6","group"="group")) %>% 
  relocate(group,ethnicat6,n_percent,cuiud_pyears_min)  %>% 
  arrange(group)

write_csv(cuiud_pyears_min_ethnicat6_export, "filepath")


## practice region ---

cuiud_cohort_final_pregmin$prac_region <- cuiud_cohort_final_pregmin$prac_region %>% fct_explicit_na(na_level = "Not known")
levels(cuiud_cohort_final_pregmin$prac_region)

prac_region <- cuiud_cohort_final_pregmin %>% 
  group_by(prac_region) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
prac_region_mvnm <- cuiud_cohort_final_pregmin %>% 
  group_by(prac_region, migrant_status) %>% count() %>% 
  rename(group = migrant_status) %>% 
  full_join(total_mvnm, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1))) %>%
  dplyr::select(c(prac_region, group, n, percent))
prac_region_dp <- cuiud_cohort_final_pregmin %>% 
  group_by(prac_region, migcertainty) %>% count() %>% 
  rename(group = migcertainty) %>% 
  full_join(total_dp, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
...
cuiud_pyears_min_prac_region <- cuiud_cohort_final_pregmin %>% group_by(prac_region) %>% tally(cuiud_pyears_min) %>% 
  mutate(group = "Whole Cohort")
cuiud_pyears_min_prac_region_mvnm <- cuiud_cohort_final_pregmin %>% group_by(migrant_status, prac_region) %>% tally(cuiud_pyears_min) %>% 
  rename(group = migrant_status)
cuiud_pyears_min_prac_region_dp <- cuiud_cohort_final_pregmin %>% group_by(migcertainty, prac_region) %>% tally(cuiud_pyears_min) %>% 
  rename(group = migcertainty)
cuiud_pyears_min_prac_region_all <- cuiud_pyears_min_prac_region_mvnm %>% full_join(cuiud_pyears_min_prac_region_dp,by = c("prac_region"="prac_region",
                                                                                                                     "group"="group","n"="n")) %>% 
  full_join(cuiud_pyears_min_prac_region,by = c("prac_region"="prac_region",
                                              "group"="group","n"="n")) %>%
  rename(cuiud_pyears_min = n) %>% 
  mutate(across(is.numeric, ~ round(.,0)))

cuiud_pyears_min_prac_region_export <- full_join(cuiud_pyears_min_prac_region_all, prac_region_all, 
                                               by = c("prac_region"="prac_region","group"="group")) %>% 
  relocate(group,prac_region,n_percent,cuiud_pyears_min)  %>% 
  arrange(group)

write_csv(cuiud_pyears_min_prac_region_export, "filepath")


## IMD ---

cuiud_cohort_final_pregmin$imd<- cuiud_cohort_final_pregmin$imd%>% fct_explicit_na(na_level = "Not known")
levels(cuiud_cohort_final_pregmin$imd)

imd<- cuiud_cohort_final_pregmin %>% 
  group_by(imd) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
imd_mvnm <- cuiud_cohort_final_pregmin %>% 
  group_by(imd, migrant_status) %>% count() %>% 
  rename(group = migrant_status) %>% 
  full_join(total_mvnm, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1))) %>%
  dplyr::select(c(imd, group, n, percent))
imd_dp <- cuiud_cohort_final_pregmin %>% 
  group_by(imd, migcertainty) %>% count() %>% 
  rename(group = migcertainty) %>% 
  full_join(total_dp, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
...
cuiud_pyears_min_imd <- cuiud_cohort_final_pregmin %>% group_by(imd) %>% tally(cuiud_pyears_min) %>% 
  mutate(group = "Whole Cohort")
cuiud_pyears_min_imd_mvnm <- cuiud_cohort_final_pregmin %>% group_by(migrant_status, imd) %>% tally(cuiud_pyears_min) %>% 
  rename(group = migrant_status)
cuiud_pyears_min_imd_dp <- cuiud_cohort_final_pregmin %>% group_by(migcertainty, imd) %>% tally(cuiud_pyears_min) %>% 
  rename(group = migcertainty)
cuiud_pyears_min_imd_all <- cuiud_pyears_min_imd_mvnm %>% full_join(cuiud_pyears_min_imd_dp,by = c("imd"="imd",
                                                                                             "group"="group","n"="n")) %>% 
  full_join(cuiud_pyears_min_imd,by = c("imd"="imd",
                                      "group"="group","n"="n")) %>%
  rename(cuiud_pyears_min = n) %>% 
  mutate(across(is.numeric, ~ round(.,0)))

cuiud_pyears_min_imd_export <- full_join(cuiud_pyears_min_imd_all, imd_all, 
                                       by = c("imd"="imd","group"="group")) %>% 
  relocate(group,imd,n_percent,cuiud_pyears_min)  %>% 
  arrange(group)

write_csv(cuiud_pyears_min_imd_export, "filepath")



## Year of cohort entry ---

cuiud_cohort_final_pregmin_yearofentry <- cuiud_cohort_final_pregmin %>% mutate(yearofentry = year(cuiud_cohort_entry))
mean_yearofentry_overall <- psych::describe(cuiud_cohort_final_pregmin_yearofentry$yearofentry, IQR = TRUE)
mean_yearofentry_overall

nm <- cuiud_cohort_final_pregmin_yearofentry %>% filter(migrant_status == "Non-migrant")
nm_yearofentry <- psych::describe(nm$yearofentry, IQR = TRUE)
nm_yearofentry

m <- cuiud_cohort_final_pregmin_yearofentry %>% filter(migrant_status == "Migrant")
m_yearofentry <- psych::describe(m$yearofentry, IQR = TRUE)
m_yearofentry

d <- cuiud_cohort_final_pregmin_yearofentry %>% filter(migcertainty == "Definite")
d_yearofentry  <- psych::describe(d$yearofentry, IQR = TRUE)
d_yearofentry 

p <- cuiud_cohort_final_pregmin_yearofentry %>% filter(migcertainty == "Probable")
p_yearofentry <- psych::describe(p$yearofentry, IQR = TRUE)
p_yearofentry
...
cuiud_cohort_final_pregmin_yearofentry$yearofentry <- factor(cuiud_cohort_final_pregmin_yearofentry$yearofentry,  levels = c(2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018), 
                                                           labels = c("2009", "2010", "2011", "2012", "2013", "2014", "2015", "2016", "2017", "2018"))
yearofentry <- cuiud_cohort_final_pregmin_yearofentry %>% 
  group_by(yearofentry) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
yearofentry_mvnm <- cuiud_cohort_final_pregmin_yearofentry %>% 
  group_by(yearofentry, migrant_status) %>% count() %>% 
  rename(group = migrant_status) %>% 
  full_join(total_mvnm, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1))) %>%
  dplyr::select(c(yearofentry, group, n, percent))
yearofentry_dp <- cuiud_cohort_final_pregmin_yearofentry %>% 
  group_by(yearofentry, migcertainty) %>% count() %>% 
  rename(group = migcertainty) %>% 
  full_join(total_dp, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
...
cuiud_pyears_min_yearofentry <- cuiud_cohort_final_pregmin_yearofentry %>% group_by(yearofentry) %>% tally(cuiud_pyears_min) %>% 
  mutate(group = "Whole Cohort")
cuiud_pyears_min_yearofentry_mvnm <- cuiud_cohort_final_pregmin_yearofentry %>% group_by(migrant_status, yearofentry) %>% tally(cuiud_pyears_min) %>% 
  rename(group = migrant_status)
cuiud_pyears_min_yearofentry_dp <- cuiud_cohort_final_pregmin_yearofentry %>% group_by(migcertainty, yearofentry) %>% tally(cuiud_pyears_min) %>% 
  rename(group = migcertainty)
cuiud_pyears_min_yearofentry_all <- cuiud_pyears_min_yearofentry_mvnm %>% full_join(cuiud_pyears_min_yearofentry_dp,by = c("yearofentry"="yearofentry",
                                                                                                                     "group"="group","n"="n")) %>% 
  full_join(cuiud_pyears_min_yearofentry,by = c("yearofentry"="yearofentry",
                                              "group"="group","n"="n")) %>%
  rename(cuiud_pyears_min = n) %>% 
  mutate(across(is.numeric, ~ round(.,0)))

cuiud_pyears_min_yearofentry_export <- full_join(cuiud_pyears_min_yearofentry_all, yearofentry_all, 
                                               by = c("group"="group","yearofentry"="yearofentry")) %>% 
  relocate(group,yearofentry,n_percent,cuiud_pyears_min)  %>% 
  arrange(group)

write_csv(cuiud_pyears_min_yearofentry_export, "filepath")


## Year ---

eventyear<- cuiudpresc_annual_counts_final_extra_preg_min %>% 
  group_by(eventyear) %>% count() %>% 
  mutate(group = "Whole Cohort") %>% 
  mutate(percent = (n/total_cohort_n)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1)))
eventyear_mvnm <- cuiudpresc_annual_counts_final_extra_preg_min  %>% 
  group_by(eventyear, migrant_status) %>% count() %>% 
  rename(group = migrant_status) %>% 
  full_join(total_mvnm, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
  mutate(across(is.numeric, ~ round(.,1))) %>%
  dplyr::select(c(eventyear, group, n, percent))
eventyear_dp <- cuiudpresc_annual_counts_final_extra_preg_min  %>% 
  group_by(eventyear, migcertainty) %>% count() %>% 
  rename(group = migcertainty) %>% 
  full_join(total_dp, by = c("group" = "group")) %>% 
  mutate(percent = (n/individuals)*100) %>% 
...
pyears_eventyear <- cuiudpresc_annual_counts_final_extra_preg_min  %>% group_by(eventyear) %>% tally(pyears_ey_pregmin) %>% 
  mutate(group = "Whole Cohort")
pyears_eventyear_mvnm <- cuiudpresc_annual_counts_final_extra_preg_min  %>% group_by(migrant_status, eventyear) %>% tally(pyears_ey_pregmin) %>% 
  rename(group = migrant_status)
pyears_eventyear_dp <- cuiudpresc_annual_counts_final_extra_preg_min  %>% group_by(migcertainty, eventyear) %>% tally(pyears_ey_pregmin) %>% 
  rename(group = migcertainty)
pyears_eventyear_all <- pyears_eventyear_mvnm %>% full_join(pyears_eventyear_dp,by = c("eventyear"="eventyear",
                                                                                       "group"="group","n"="n")) %>% 
  full_join(pyears_eventyear,by = c("eventyear"="eventyear",
...
pyears_eventyear_export <- full_join(pyears_eventyear_all, eventyear_all, 
                                     by = c("eventyear"="eventyear","group"="group")) %>% 
  relocate(group,eventyear,n_percent,cuiud_pyears_min)  %>% 
  arrange(group)
...
mean_cuiud_pyears_min_overall <- psych::describe(cuiud_cohort_final_pregmin$cuiud_pyears_min, IQR = TRUE)
mean_cuiud_pyears_min_overall

nm <- cuiud_cohort_final_pregmin %>% filter(migrant_status == "Non-migrant")
nm_cuiud_pyears_min <- psych::describe(nm$cuiud_pyears_min, IQR = TRUE)
nm_cuiud_pyears_min

m <- cuiud_cohort_final_pregmin %>% filter(migrant_status == "Migrant")
m_cuiud_pyears_min <- psych::describe(m$cuiud_pyears_min, IQR = TRUE)
m_cuiud_pyears_min

d <- cuiud_cohort_final_pregmin %>% filter(migcertainty == "Definite")
d_cuiud_pyears_min <- psych::describe(d$cuiud_pyears_min, IQR = TRUE)
d_cuiud_pyears_min

p <- cuiud_cohort_final_pregmin %>% filter(migcertainty == "Probable")
p_cuiud_pyears_min <- psych::describe(p$cuiud_pyears_min, IQR = TRUE)
p_cuiud_pyears_min

## join export mean + median ages + sd + IQR
mean_cuiud_pyears_min_overall <- as.data.frame((mean_cuiud_pyears_min_overall))
mean_cuiud_pyears_min_overall <- dplyr::select(mean_cuiud_pyears_min_overall, c(mean , sd, median, IQR)) %>% mutate(group = "Whole cohort") 
nm_cuiud_pyears_min <- as.data.frame((nm_cuiud_pyears_min))
nm_cuiud_pyears_min <- dplyr::select(nm_cuiud_pyears_min, c(mean , sd, median, IQR)) %>% mutate(group = "Non-migrant") 
m_cuiud_pyears_min <- as.data.frame((m_cuiud_pyears_min))
m_cuiud_pyears_min <- dplyr::select(m_cuiud_pyears_min, c(mean , sd, median, IQR)) %>% mutate(group = "Migrant") 
d_cuiud_pyears_min <- as.data.frame((d_cuiud_pyears_min))
d_cuiud_pyears_min <- dplyr::select(d_cuiud_pyears_min, c(mean , sd, median, IQR)) %>% mutate(group = "Definite") 
p_cuiud_pyears_min <- as.data.frame((p_cuiud_pyears_min))
p_cuiud_pyears_min <- dplyr::select(p_cuiud_pyears_min, c(mean , sd, median, IQR)) %>% mutate(group = "Probable") 
all_mean_cuiud_pyears_min <- full_join(mean_cuiud_pyears_min_overall, nm_cuiud_pyears_min,
                                     by = c( "group" = "group", "mean" = "mean", "sd" = "sd" , "median" = "median", "IQR" = "IQR")) %>% 
  full_join(m_cuiud_pyears_min, by = c( "group" = "group", "mean" = "mean", "sd" = "sd" , "median" = "median", "IQR" = "IQR") ) %>%
  full_join(d_cuiud_pyears_min, by = c( "group" = "group", "mean" = "mean", "sd" = "sd", "median" = "median",  "IQR" = "IQR" ) ) %>%
  full_join(p_cuiud_pyears_min, by = c( "group" = "group", "mean" = "mean", "sd" = "sd" , "median" = "median", "IQR" = "IQR") )  %>%
  mutate(across(is.numeric, ~ round(.,2)))
all_mean_cuiud_pyears_min$mean_sd <- paste(all_mean_cuiud_pyears_min$mean, all_mean_cuiud_pyears_min$sd, sep =", ")
all_mean_cuiud_pyears_min$median_iqr <- paste(all_mean_cuiud_pyears_min$median, all_mean_cuiud_pyears_min$IQR, sep =", ")
all_mean_cuiud_pyears_min <- dplyr::select(all_mean_cuiud_pyears_min, c(group, mean_sd, median_iqr))
write_csv(all_mean_cuiud_pyears_min, "filepath")
...
cuiudpresc_count_overall_whole_cohort <- cuiudpresc_annual_counts_final_extra_preg_max %>% tally(cuiudpresc_n_pregmax) %>% rename(n_events = n)
cuiudpresc_summary_overall_whole_cohort <- cuiudpresc_annual_counts_final_extra_preg_max %>% 
  summarise(mean = mean(cuiudpresc_n_pregmax), sd = sd(cuiudpresc_n_pregmax), median = median(cuiudpresc_n_pregmax), iqr = IQR(cuiudpresc_n_pregmax), 
            min = min(cuiudpresc_n_pregmax), max = max(cuiudpresc_n_pregmax))
cuiudpresc_count_summary_overall_whole_cohort <- merge(cuiudpresc_count_overall_whole_cohort, cuiudpresc_summary_overall_whole_cohort)
cuiudpresc_count_summary_overall_whole_cohort

## Overall by migrant status
cuiudpresc_count_overall_migrant_status <- cuiudpresc_annual_counts_final_extra_preg_max %>% group_by(migrant_status) %>%
  tally(cuiudpresc_n_pregmax) %>% rename(n_events = n)
cuiudpresc_summary_overall_migrant_status <- cuiudpresc_annual_counts_final_extra_preg_max %>% group_by(migrant_status) %>%
  summarise(mean = mean(cuiudpresc_n_pregmax), sd = sd(cuiudpresc_n_pregmax), median = median(cuiudpresc_n_pregmax), iqr = IQR(cuiudpresc_n_pregmax), 
            min = min(cuiudpresc_n_pregmax), max = max(cuiudpresc_n_pregmax))
cuiudpresc_count_summary_overall_migrant_status <- left_join(cuiudpresc_count_overall_migrant_status, cuiudpresc_summary_overall_migrant_status,
                                                           by = c("migrant_status"="migrant_status" ))

## Overall by migcertainty
cuiudpresc_count_overall_migcertainty <- cuiudpresc_annual_counts_final_extra_preg_max %>% group_by(migcertainty) %>%
  tally(cuiudpresc_n_pregmax) %>% rename(n_events = n)
cuiudpresc_summary_overall_migcertainty <- cuiudpresc_annual_counts_final_extra_preg_max %>% group_by(migcertainty) %>%
  summarise(mean = mean(cuiudpresc_n_pregmax), sd = sd(cuiudpresc_n_pregmax), median = median(cuiudpresc_n_pregmax), iqr = IQR(cuiudpresc_n_pregmax), 
            min = min(cuiudpresc_n_pregmax), max = max(cuiudpresc_n_pregmax))
cuiudpresc_count_summary_overall_migcertainty <- left_join(cuiudpresc_count_overall_migcertainty, cuiudpresc_summary_overall_migcertainty,
                                                         by = c("migcertainty"="migcertainty" ))

## Join for overall whole cohort + by migrant status
cuiudpresc_count_summary_overall_final <- full_join(cuiudpresc_count_summary_overall_whole_cohort, cuiudpresc_count_summary_overall_migrant_status, 
                                                  by = c("n_events"="n_events", "mean" = "mean",
                                                         "sd"="sd", "median"="median", "iqr"="iqr", "min"="min", "max"="max")) %>% 
  full_join(cuiudpresc_count_summary_overall_migcertainty, by = c("n_events"="n_events", "mean" = "mean",
                                                                "sd"="sd", "median"="median", "iqr"="iqr", "min"="min", "max"="max",
                                                                "migrant_status" = "migcertainty")) 
cuiudpresc_count_summary_overall_final$migrant_status <- fct_explicit_na(cuiudpresc_count_summary_overall_final$migrant_status, na_level = "whole_cohort")
cuiudpresc_count_summary_overall_final <- cuiudpresc_count_summary_overall_final %>% relocate(migrant_status)
cuiudpresc_count_summary_overall_final

write_csv(cuiudpresc_count_summary_overall_final, "filepath")

## Annual - whole cohort 
cuiudpresc_count_annual_whole_cohort <- cuiudpresc_annual_counts_final_extra_preg_max %>% group_by(eventyear) %>%
  tally(cuiudpresc_n_pregmax) %>% rename(n_events = n)
cuiudpresc_summary_annual_whole_cohort <- cuiudpresc_annual_counts_final_extra_preg_max %>% group_by(eventyear) %>%
  summarise(mean = mean(cuiudpresc_n_pregmax), sd = sd(cuiudpresc_n_pregmax), median = median(cuiudpresc_n_pregmax), iqr = IQR(cuiudpresc_n_pregmax), 
            min = min(cuiudpresc_n_pregmax), max = max(cuiudpresc_n_pregmax))
cuiudpresc_count_summary_annual_whole_cohort <- left_join(cuiudpresc_count_annual_whole_cohort, cuiudpresc_summary_annual_whole_cohort,
                                                        by = c("eventyear"="eventyear"))


## Annual - by migrant status
cuiudpresc_count_annual_migrant_status <- cuiudpresc_annual_counts_final_extra_preg_max %>% group_by(migrant_status, eventyear) %>%
  tally(cuiudpresc_n_pregmax) %>% rename(n_events = n)
cuiudpresc_summary_annual_migrant_status <- cuiudpresc_annual_counts_final_extra_preg_max %>% group_by(migrant_status, eventyear) %>%
  summarise(mean = mean(cuiudpresc_n_pregmax), sd = sd(cuiudpresc_n_pregmax), median = median(cuiudpresc_n_pregmax), iqr = IQR(cuiudpresc_n_pregmax), 
            min = min(cuiudpresc_n_pregmax), max = max(cuiudpresc_n_pregmax))
cuiudpresc_count_summary_annual_migrant_status <- left_join(cuiudpresc_count_annual_migrant_status, cuiudpresc_summary_annual_migrant_status,
                                                          by = c("migrant_status"="migrant_status" , "eventyear"="eventyear"))

## Annual - by migcertainty 
cuiudpresc_count_annual_migcertainty <- cuiudpresc_annual_counts_final_extra_preg_max %>% group_by(migcertainty, eventyear) %>%
  tally(cuiudpresc_n_pregmax) %>% rename(n_events = n)
cuiudpresc_summary_annual_migcertainty <- cuiudpresc_annual_counts_final_extra_preg_max %>% group_by(migcertainty, eventyear) %>%
  summarise(mean = mean(cuiudpresc_n_pregmax), sd = sd(cuiudpresc_n_pregmax), median = median(cuiudpresc_n_pregmax), iqr = IQR(cuiudpresc_n_pregmax), 
            min = min(cuiudpresc_n_pregmax), max = max(cuiudpresc_n_pregmax))
cuiudpresc_count_summary_annual_migcertainty <- left_join(cuiudpresc_count_annual_migcertainty, cuiudpresc_summary_annual_migcertainty,
                                                        by = c("migcertainty"="migcertainty" , "eventyear"="eventyear"))



## Join to give annual for whole cohort + migrant status
cuiudpresc_count_summary_annual_final <- full_join(cuiudpresc_count_summary_annual_whole_cohort, cuiudpresc_count_summary_annual_migrant_status, 
                                                 by = c("eventyear" = "eventyear", "n_events"="n_events", "mean" = "mean",
                                                        "sd"="sd", "median"="median", "iqr"="iqr", "min"="min", "max"="max")) 
cuiudpresc_count_summary_annual_final <-   full_join(cuiudpresc_count_summary_annual_final, cuiudpresc_count_summary_annual_migcertainty,
                                                   by = c("eventyear" = "eventyear", "n_events"="n_events", "mean" = "mean",
                                                          "sd"="sd", "median"="median", "iqr"="iqr", "min"="min", "max"="max",
                                                          "migrant_status" = "migcertainty"))

cuiudpresc_count_summary_annual_final$migrant_status <- fct_explicit_na(cuiudpresc_count_summary_annual_final$migrant_status, na_level = "whole_cohort") 
cuiudpresc_count_summary_annual_final <- cuiudpresc_count_summary_annual_final %>% 
  relocate(migrant_status)


## Export cuiudpresc summary measures
write_csv(cuiudpresc_count_summary_annual_final,"filepath")
...
pyears_overall <- sum(cuiudpresc_annual_counts_final_extra_preg_max$pyears_ey_pregmax)
cuiudpresccount <- sum(cuiudpresc_annual_counts_final_extra_preg_max$cuiudpresc_n_pregmax)
rate_output <- pois.exact(cuiudpresccount, pyears_overall, conf.level=0.95)
...
pyears_mvnm_overall <- aggregate(pyears_ey_pregmax~ migrant_status, cuiudpresc_annual_counts_final_extra_preg_max, sum) 
pyears_mvnm_overall <- pyears_mvnm_overall %>% mutate(across(is.numeric, ~ round(.,)))
cuiudpresccount_mvnm <- aggregate(cuiudpresc_n_pregmax ~ migrant_status, cuiudpresc_annual_counts_final_extra_preg_max, sum) 
IR_mvnm <- inner_join(cuiudpresccount_mvnm,pyears_mvnm_overall, by= c("migrant_status" = "migrant_status"))
IR_mvnm_output <- pois.exact(IR_mvnm$cuiudpresc_n_pregmax, IR_mvnm$pyears_ey_pregmax, conf.level=0.95)
...
x <- glm.nb(cuiudpresc_n_pregmax ~ as.factor(migrant_status) + offset(log(pyears_ey_pregmax)), data = cuiudpresc_annual_counts_final_extra_preg_max)
names <- names(coef(x))
estimate<- exp(coef(x))
...
a<- cuiudpresc_annual_counts_final_extra_preg_max %>% group_by(patid, eventyear_agecat) %>% 
  tally(cuiudpresc_n_pregmax)
b<- cuiudpresc_annual_counts_final_extra_preg_max %>% group_by(patid, eventyear_agecat) %>% 
  tally(pyears_ey_pregmax)
c<- full_join(a,b, by = c("patid"="patid", "eventyear_agecat" = "eventyear_agecat"))
d<- left_join(c, cuiud_cohort_final  , by = c("patid" = "patid")) %>% 
  rename(cuiudpresc_n_pregmax = n.x) %>% 
  rename(pyears_agecat = n.y)


# IR by age cat - whole cohort
pyears_agecat_overall <- aggregate(pyears_ey_pregmax ~ eventyear_agecat, cuiudpresc_annual_counts_final_extra_preg_max, sum) 
pyears_agecat_overall <- pyears_agecat_overall %>% mutate(across(is.numeric, ~ round(.,)))
cuiudpresccount_agecat <- aggregate(cuiudpresc_n_pregmax ~ eventyear_agecat, cuiudpresc_annual_counts_final_extra_preg_max, sum) 
IR_agecat <- inner_join(cuiudpresccount_agecat,pyears_agecat_overall, by= c("eventyear_agecat" = "eventyear_agecat"))
IR_agecat_output <- pois.exact(IR_agecat$cuiudpresc_n_pregmax, IR_agecat$pyears_ey_pregmax, conf.level=0.95)
...
x <- glm.nb(cuiudpresc_n_pregmax ~ as.factor(eventyear_agecat) + offset(log(pyears_agecat)), data = d)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
pyears_prac_region_overall <- aggregate(pyears_ey_pregmax ~ prac_region, cuiudpresc_annual_counts_final_extra_preg_max, sum) 
pyears_prac_region_overall <- pyears_prac_region_overall %>% mutate(across(is.numeric, ~ round(.,)))
cuiudpresccount_prac_region <- aggregate(cuiudpresc_n_pregmax ~ prac_region, cuiudpresc_annual_counts_final_extra_preg_max, sum) 
IR_prac_region <- inner_join(cuiudpresccount_prac_region,pyears_prac_region_overall, by= c("prac_region" = "prac_region"))
IR_prac_region_output <- pois.exact(IR_prac_region$cuiudpresc_n_pregmax, IR_prac_region$pyears_ey_pregmax, conf.level=0.95)
...
x <- glm.nb(cuiudpresc_n_pregmax ~ as.factor(prac_region) + offset(log(pyears_ey_pregmax)), data = cuiudpresc_annual_counts_final_extra_preg_max)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
# IR by imd  - whole cohort
pyears_imd_overall <- aggregate(pyears_ey_pregmax ~ imd, cuiudpresc_annual_counts_final_extra_preg_max, sum) 
pyears_imd_overall <- pyears_imd_overall %>% mutate(across(is.numeric, ~ round(.,)))
cuiudpresccount_imd <- aggregate(cuiudpresc_n_pregmax ~ imd, cuiudpresc_annual_counts_final_extra_preg_max, sum) 
IR_imd <- inner_join(cuiudpresccount_imd,pyears_imd_overall, by= c("imd" = "imd"))
IR_imd_output <- pois.exact(IR_imd$cuiudpresc_n_pregmax, IR_imd$pyears_ey_pregmax, conf.level=0.95)
...
x <- glm.nb(cuiudpresc_n_pregmax ~ as.factor(imd) + offset(log(pyears_ey_pregmax)), data = cuiudpresc_annual_counts_final_extra_preg_max)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
pyears_annual <- aggregate(pyears_ey_pregmax ~ eventyear, cuiudpresc_annual_counts_final_extra_preg_max, sum) 
pyears_annual <- pyears_annual %>% mutate(across(is.numeric, ~ round(.,)))
cuiudpresc_count_annual <- aggregate(cuiudpresc_n_pregmax ~ eventyear, cuiudpresc_annual_counts_final_extra_preg_max, sum) 
IR_annual <- inner_join(cuiudpresc_count_annual,pyears_annual, by= c("eventyear" = "eventyear"))
IR_annual_output <- pois.exact(IR_annual$cuiudpresc_n_pregmax, IR_annual$pyears_ey_pregmax, conf.level=0.95)
...
## Generate IRR via negative binomial regression univariabe for  year 
x <- glm.nb(cuiudpresc_n_pregmax ~ as.factor(eventyear) + offset(log(pyears_ey_pregmax)), data = cuiudpresc_annual_counts_final_extra_preg_max)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
x <- glm.nb(cuiudpresc_n_pregmax ~ as.factor(migrant_status) + as.factor(eventyear) + as.factor(eventyear_agecat) + as.factor(imd) + as.factor(prac_region) + offset(log(pyears_ey_pregmax)) , 
            data = cuiudpresc_annual_counts_final_extra_preg_max)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
cuiud_glm_adj_unadj <- glm_adj_unadj
save(cuiud_glm_adj_unadj, file = "filepath")
...
pyears_dp_overall <- aggregate(pyears_ey_pregmax ~ migcertainty, cuiudpresc_annual_counts_final_extra_preg_max, sum) 
pyears_dp_overall <- pyears_dp_overall %>% mutate(across(is.numeric, ~ round(.,)))
cuiudpresccount_dp <- aggregate(cuiudpresc_n_pregmax ~ migcertainty, cuiudpresc_annual_counts_final_extra_preg_max, sum) 
IR_dp <- inner_join(cuiudpresccount_dp,pyears_dp_overall, by= c("migcertainty" = "migcertainty"))
IR_dp_output <- pois.exact(IR_dp$cuiudpresc_n_pregmax, IR_dp$pyears_ey_pregmax, conf.level=0.95)
...
##  generate IRR by negative binomial regression
cuiudpresc_annual_counts_final_extra_preg_max$migcertainty <- relevel(cuiudpresc_annual_counts_final_extra_preg_max$migcertainty, "Non-migrant")
x <- glm.nb(cuiudpresc_n_pregmax ~ as.factor(migcertainty) + offset(log(pyears_ey_pregmax)), data = cuiudpresc_annual_counts_final_extra_preg_max)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
## Generate IRR via negative binomial regression
cuiudpresc_annual_counts_final_extra_preg_max$migcertainty <- relevel(cuiudpresc_annual_counts_final_extra_preg_max$migcertainty, "Non-migrant")
x <- glm.nb(cuiudpresc_n_pregmax ~ as.factor(migcertainty) + as.factor(eventyear) + as.factor(eventyear_agecat) +  as.factor(imd) + as.factor(prac_region) + offset(log(pyears_ey_pregmax)), 
            data = cuiudpresc_annual_counts_final_extra_preg_max)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
...
glm_dp_adj_unadj <- full_join(glm_dp, glm_all_dp_fp, by = c("names" = "names", "estimate" = "estimate",
                                                            "lower" = "lower", "upper"= "upper",
                                                            "p" = "p", "ci" = "ci", "irr_ci" = "irr_ci"))
cuiud_glm_dp_adj_unadj <- glm_dp_adj_unadj
save(cuiud_glm_dp_adj_unadj, file = "filepath")
...
white_british <- cuiudpresc_annual_counts_final_extra_preg_max %>% filter(ethnicat6 == "White British")
...
cuiudpresccount_mvnm_white_british<- aggregate(cuiudpresc_n_pregmax ~ migrant_status, white_british, sum) 
IR_mvnm_white_british <- inner_join(cuiudpresccount_mvnm_white_british,pyears_mvnm_white_british, by= c("migrant_status" = "migrant_status"))
IR_mvnm_white_british_output <- pois.exact(IR_mvnm_white_british$cuiudpresc_n_pregmax, IR_mvnm_white_british$pyears_ey_pregmax, conf.level=0.95)
...
x <- glm.nb(cuiudpresc_n_pregmax ~ as.factor(migrant_status) + offset(log(pyears_ey_pregmax)), data = white_british)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
white_nonbritish <- cuiudpresc_annual_counts_final_extra_preg_max %>% filter(ethnicat6 == "White Non-British")
...
cuiudpresccount_mvnm_white_nonbritish<- aggregate(cuiudpresc_n_pregmax ~ migrant_status, white_nonbritish, sum) 
IR_mvnm_white_nonbritish <- inner_join(cuiudpresccount_mvnm_white_nonbritish,pyears_mvnm_white_nonbritish, by= c("migrant_status" = "migrant_status"))
IR_mvnm_white_nonbritish_output <- pois.exact(IR_mvnm_white_nonbritish$cuiudpresc_n_pregmax, IR_mvnm_white_nonbritish$pyears_ey_pregmax, conf.level=0.95)
...
x <- glm.nb(cuiudpresc_n_pregmax ~ as.factor(migrant_status) + offset(log(pyears_ey_pregmax)), data = white_nonbritish)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
mixed <- cuiudpresc_annual_counts_final_extra_preg_max %>% filter(ethnicat6 == "Mixed")
...
cuiudpresccount_mvnm_mixed<- aggregate(cuiudpresc_n_pregmax ~ migrant_status, mixed, sum) 
IR_mvnm_mixed <- inner_join(cuiudpresccount_mvnm_mixed,pyears_mvnm_mixed, by= c("migrant_status" = "migrant_status"))
IR_mvnm_mixed_output <- pois.exact(IR_mvnm_mixed$cuiudpresc_n_pregmax, IR_mvnm_mixed$pyears_ey_pregmax, conf.level=0.95)
...
x <- glm.nb(cuiudpresc_n_pregmax ~ as.factor(migrant_status) + offset(log(pyears_ey_pregmax)), data = mixed)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
asian <- cuiudpresc_annual_counts_final_extra_preg_max %>% filter(ethnicat6 == "Asian/Asian British")
...
cuiudpresccount_mvnm_asian<- aggregate(cuiudpresc_n_pregmax ~ migrant_status, asian, sum) 
IR_mvnm_asian <- inner_join(cuiudpresccount_mvnm_asian,pyears_mvnm_asian, by= c("migrant_status" = "migrant_status"))
IR_mvnm_asian_output <- pois.exact(IR_mvnm_asian$cuiudpresc_n_pregmax, IR_mvnm_asian$pyears_ey_pregmax, conf.level=0.95)
...
x <- glm.nb(cuiudpresc_n_pregmax ~ as.factor(migrant_status) + offset(log(pyears_ey_pregmax)), data = asian)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
black <- cuiudpresc_annual_counts_final_extra_preg_max %>% filter(ethnicat6 == "Black/Black British")
...
cuiudpresccount_mvnm_black<- aggregate(cuiudpresc_n_pregmax ~ migrant_status, black, sum) 
IR_mvnm_black <- inner_join(cuiudpresccount_mvnm_black,pyears_mvnm_black, by= c("migrant_status" = "migrant_status"))
IR_mvnm_black_output <- pois.exact(IR_mvnm_black$cuiudpresc_n_pregmax, IR_mvnm_black$pyears_ey_pregmax, conf.level=0.95)
...
x <- glm.nb(cuiudpresc_n_pregmax ~ as.factor(migrant_status) + offset(log(pyears_ey_pregmax)), data = black)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
other <- cuiudpresc_annual_counts_final_extra_preg_max %>% filter(ethnicat6 == "Other ethnic group")
...
cuiudpresccount_mvnm_other<- aggregate(cuiudpresc_n_pregmax ~ migrant_status, other, sum) 
IR_mvnm_other <- inner_join(cuiudpresccount_mvnm_other,pyears_mvnm_other, by= c("migrant_status" = "migrant_status"))
IR_mvnm_other_output <- pois.exact(IR_mvnm_other$cuiudpresc_n_pregmax, IR_mvnm_other$pyears_ey_pregmax, conf.level=0.95)
...
x <- glm.nb(cuiudpresc_n_pregmax ~ as.factor(migrant_status) + offset(log(pyears_ey_pregmax)), data = other)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
cuiud_glm_adj_unadj_ethnicity <- glm_adj_unadj
save(cuiud_glm_adj_unadj_ethnicity, file = "filepath")
...
# IR by migrant status - whole cohort
pyears_mvnm_overall <- aggregate(pyears_ey_pregmin~ migrant_status, cuiudpresc_annual_counts_final_extra_preg_min, sum) 
pyears_mvnm_overall <- pyears_mvnm_overall %>% mutate(across(is.numeric, ~ round(.,)))
cuiudpresccount_mvnm <- aggregate(cuiudpresc_n_pregmin ~ migrant_status, cuiudpresc_annual_counts_final_extra_preg_min, sum) 
IR_mvnm <- inner_join(cuiudpresccount_mvnm,pyears_mvnm_overall, by= c("migrant_status" = "migrant_status"))
IR_mvnm_output <- pois.exact(IR_mvnm$cuiudpresc_n_pregmin, IR_mvnm$pyears_ey_pregmin, conf.level=0.95)
IR_mvnm_output <- IR_mvnm_output %>%
  mutate(incidence_rate100000=rate*100000)
IR_mvnm_output <- IR_mvnm_output %>%
  mutate(lower100000=lower*100000)
...
x <- glm.nb(cuiudpresc_n_pregmin ~ as.factor(migrant_status) + offset(log(pyears_ey_pregmin)), data = cuiudpresc_annual_counts_final_extra_preg_min)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
## Generate IRR via negative binomial regression
x <- glm.nb(cuiudpresc_n_pregmin ~ as.factor(migrant_status) + as.factor(eventyear) + as.factor(eventyear_agecat) + as.factor(imd) + as.factor(prac_region) + offset(log(pyears_ey_pregmin)) , 
            data = cuiudpresc_annual_counts_final_extra_preg_min)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
glm_adj_unadj <- full_join(glm_mig, glm_all_mvnm_fp, by = c("names" = "names", "estimate" = "estimate",
                                                            "lower" = "lower", "upper"= "upper",
                                                            "p" = "p", "ci" = "ci", "irr_ci" = "irr_ci"))
cuiud_glm_adj_unadj_pregmin <- glm_adj_unadj
save(cuiud_glm_adj_unadj_pregmin, file = "filepath")
...
exact_match_cuiudpresc_annual_counts_final_4to1$prac_region <- relevel(exact_match_cuiudpresc_annual_counts_final_4to1$prac_region, "London")
exact_match_cuiudpresc_annual_counts_final_extra_4to1$prac_region <- relevel(exact_match_cuiudpresc_annual_counts_final_extra_4to1$prac_region, "London")

# Using preg max assumption

exact_match_cuiudpresc_annual_counts_final_extra_4to1_preg_max <- exact_match_cuiudpresc_annual_counts_final_extra_4to1 %>% filter(pyears_ey_pregmax > 0)


# IR by migrant status - whole cohort
pyears_mvnm_overall <- aggregate(pyears_ey_pregmax ~ migrant_status, exact_match_cuiudpresc_annual_counts_final_extra_4to1_preg_max, sum) 
pyears_mvnm_overall <- pyears_mvnm_overall %>% mutate(across(is.numeric, ~ round(.,0)))
cuiudpresccount_mvnm <- aggregate(cuiudpresc_n_pregmax ~ migrant_status, exact_match_cuiudpresc_annual_counts_final_extra_4to1_preg_max, sum) 
IR_mvnm <- inner_join(cuiudpresccount_mvnm,pyears_mvnm_overall, by= c("migrant_status" = "migrant_status"))
IR_mvnm_output <- pois.exact(IR_mvnm$cuiudpresc_n_pregmax, IR_mvnm$pyears_ey_pregmax, conf.level=0.95)
IR_mvnm_output <- IR_mvnm_output %>%
  mutate(incidence_rate100000=rate*100000)
...
x <- glm.nb(cuiudpresc_n_pregmax ~ as.factor(migrant_status) + offset(log(pyears_ey_pregmax)), data = exact_match_cuiudpresc_annual_counts_final_extra_4to1_preg_max)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
## Generate IRR via negative binomial regression
x <- glm.nb(cuiudpresc_n_pregmax ~ as.factor(migrant_status) + as.factor(eventyear) + as.factor(eventyear_agecat) + as.factor(imd) + offset(log(pyears_ey_pregmax)) , 
            data = exact_match_cuiudpresc_annual_counts_final_extra_4to1_preg_max)
names <- names(coef(x))
estimate<- exp(coef(x))
confint.default <- exp(confint.default(x))
p <- coef(summary(x))[,4]
...
cuiud_glm_adj_unadj_em <- glm_adj_unadj
save(cuiud_glm_adj_unadj_em, file = "filepath")
...
unadj_irr_migrant_status <- forestplot(tabletext, mean = glm_adj_unadj$estimate, lower=glm_adj_unadj$lower, upper=glm_adj_unadj$upper,
                                       ...
                                       ci.vertices = TRUE,
                                       boxsize = 0.05,
                                       title = "cuiudpresc (Exact Matched Cohort)",
                                       graphwidth = unit(80, 'mm'))
dev.off()
